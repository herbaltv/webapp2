...
/*-----------------------------------------------------------------------------------*/
add_action( 'init', 'herbal_extensions_slider_register' );
function herbal_extensions_slider_register() {

	$args = array(
		'labels' => array(
			'name'          => esc_html__( 'Custom Sliders', 'herbal-extensions' ),
			'singular_name' => esc_html__( 'Slider',         'herbal-extensions' ),
			'add_new_item'  => esc_html__( 'Add New Slider', 'herbal-extensions' ),
		),
		'public'              => false,
		'show_ui'             => true,
...
/*-----------------------------------------------------------------------------------*/
add_action( 'edit_form_after_title', 'herbal_extensions_slider_add_slides' );
function herbal_extensions_slider_add_slides() {
  ...
  <script>
	  jQuery(document).ready(function() {
			...
			jQuery(document).on( 'click', '#upload_add_slide' , function(event){
				...
				tie_uploader = wp.media.frames.tie_uploader = wp.media({
					title: '<?php esc_html_e( 'Insert Images', 'herbal-extensions' ) ?>',
					library: {
						type: 'image'
					},
					button: {
						text: '<?php esc_html_e( 'Select', 'herbal-extensions' ) ?>',
					},
					multiple: true
				});
								...
								<div class="slider-content">\
									...
									<div class="slider-option-item">\
										<label for="custom_slider['+ nextCell +'][title]">\
											<span class="tie-slider-label"><?php esc_html_e( 'Slide Title:', 'herbal-extensions' ) ?></span>\
									...
									<div class="slider-option-item">\
										<label for="custom_slider['+ nextCell +'][link]">\
											<span class="tie-slider-label"><?php esc_html_e( 'Slide Link:', 'herbal-extensions' ) ?></span>\
									...
									<div class="slider-option-item">\
										<label for="custom_slider['+ nextCell +'][caption]">\
											<span class="tie-slider-label"><?php esc_html_e( 'Slide Caption:', 'herbal-extensions' ) ?></span>\
	...
	<div id="tie-slider-items">
		...
		<input id="upload_add_slide" type="button" class="tie-primary-button button button-primary button-large" value="<?php esc_html_e( 'Add New Slide', 'herbal-extensions' ) ?>">
	</div>
...
/*-----------------------------------------------------------------------------------*/
add_action( 'save_post', 'herbal_extensions_save_slider' );
function herbal_extensions_save_slider( $post_id ){
...
# Slider page columns
/*-----------------------------------------------------------------------------------*/
add_filter( 'manage_edit-tie_slider_columns', 'herbal_extensions_slider_edit_columns' );
function herbal_extensions_slider_edit_columns($columns){

	$columns = array(
		'cb'     => '<input type="checkbox" />',
		'title'  => esc_html__( 'Title',            'herbal-extensions' ),
		'slides' => esc_html__( 'Number of slides', 'herbal-extensions' ),
		'id'     => esc_html__( 'ID',               'herbal-extensions' ),
    'date'   => esc_html__( 'Date',             'herbal-extensions' ),
  );
...
/*-----------------------------------------------------------------------------------*/
add_action( 'manage_tie_slider_posts_custom_column', 'herbal_extensions_slider_custom_columns' );
function herbal_extensions_slider_custom_columns($column){
	$post_id = get_the_id();
