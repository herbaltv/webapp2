...
class WP_User_Avatar_Shortcode {
  ...
  public function wpua_shortcode($atts, $content=null) {
    ...
    if(!empty($link)) {
      ...
      $html = '<a href="'.$link.'" class="herbal-avatar-link herbal-avatar-'.$link_class.'"'.$target_link.'>'.get_wp_user_avatar($id_or_email, $get_size, $align).'</a>';
    } else {
    ...
    if($wp_user_avatar->wpua_is_author_or_above() || ((bool) $wpua_allow_upload == 1 && is_user_logged_in())) {
      ...
      if($valid_user) {
        ...
        if(isset($errors) && is_wp_error($errors)) {
          ...
          echo '<div class="updated"><p><strong>'.__('Profile updated.','herbal-avatar').'</strong></p></div>';
        }
        // Edit form
  ...
  ?>
    <form id="wpua-edit-<?php echo $user->ID; ?>" class="wpua-edit" action="" method="post" enctype="multipart/form-data">
      ...
      <?php submit_button(__('Update Profile','herbal-avatar')); ?>
    </form>
  <?php
    return ob_get_clean();
  }
