...
class WP_User_Avatar_List_Table extends WP_List_Table {
  ...
  ?>
    <p class="search-box">
      ...
      <input type="hidden" id="page" name="page" value="herbal-avatar-library" />
  ...
  public function get_views() {
    ...
    $type_links['all'] = sprintf('<a href="%s">', esc_url(add_query_arg(array('page' => 'herbal-avatar-library'), 'admin.php'))).sprintf(_nx('All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $_total_posts, 'uploaded files'), number_format_i18n($_total_posts)).'</a>';
    return $type_links;
  }
  ...
  public function get_bulk_actions() {
    $actions = array();
    $actions['delete'] = __('Delete Permanently','herbal-avatar');
    return $actions;
  }
  ...
  public function no_items() {
    _e('No media attachments found.','herbal-avatar');
  }
  ...
  public function get_columns() {
    ...
    $columns['author'] = __('Author','herbal-avatar');
            ...
            ?>
              <th scope="row" class="check-column">
                <?php if($user_can_edit) { ?>
                  <label class="screen-reader-text" for="cb-select-<?php the_ID(); ?>"><?php echo sprintf(__('Select %s','herbal-avatar'), $att_title);?></label>
      ...
      <?php
        ...
        case 'date':
          if('0000-00-00 00:00:00' == $post->post_date) {
            $h_time = __('Unpublished','herbal-avatar');
          } else {
            $m_time = $post->post_date;
            $time = get_post_time('G', true, $post, false);
            if ((abs($t_diff = time() - $time)) < DAY_IN_SECONDS) {
              if ($t_diff < 0)
                $h_time = sprintf(__('%s from now','herbal-avatar'), human_time_diff($time));
              else
                $h_time = sprintf(__('%s ago','herbal-avatar'), human_time_diff($time));
            } else {
              $h_time = mysql2date(__('Y/m/d','herbal-avatar'), $m_time);
            }
          }
      ?>
  ...
  public function _get_row_actions($post, $att_title) {
    ...
    if(current_user_can('edit_post', $post->ID) && !$this->is_trash) {
      $actions['edit'] = '<a href="'.get_edit_post_link($post->ID, true).'">'.__('Edit','herbal-avatar').'</a>';
    }
    if(current_user_can('delete_post', $post->ID)) {
      if($this->is_trash) {
        $actions['untrash'] = "<a class='submitdelete' href='".wp_nonce_url("post.php?action=untrash&amp;post=$post->ID", 'untrash-post_'.$post->ID)."'>".__('Restore','herbal-avatar')."</a>";
      } elseif (EMPTY_TRASH_DAYS && MEDIA_TRASH) {
        $actions['trash'] = "<a class='submitdelete' href='".wp_nonce_url("post.php?action=trash&amp;post=$post->ID", 'trash-post_'.$post->ID)."'>".__('Trash','herbal-avatar')."</a>";
      }
      if($this->is_trash || !EMPTY_TRASH_DAYS || !MEDIA_TRASH) {
        $delete_ays = (!$this->is_trash && !MEDIA_TRASH) ? " onclick='return showNotice.warn();'" : "";
        $actions['delete'] = "<a class='submitdelete'$delete_ays href='".wp_nonce_url("post.php?action=delete&amp;post=$post->ID", 'delete-post_'.$post->ID)."'>".__('Delete Permanently','herbal-avatar')."</a>";
      }
    }
    if(!$this->is_trash) {
      $title = _draft_or_post_title($post->post_parent);
      $actions['view'] = '<a href="'.get_permalink($post->ID).'" title="'.esc_attr(sprintf(__('View &#8220;%s&#8221;'), $title)).'" rel="permalink">'.__('View','herbal-avatar').'</a>';
    }
    return $actions;
  }
}
