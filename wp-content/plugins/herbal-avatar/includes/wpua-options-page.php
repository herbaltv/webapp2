...
$wpua_options_page_title = __('Herbal Avatar', 'herbal-avatar');
/**
...
<div class="wrap">
  ...
  <form method="post" action="<?php echo admin_url('options.php'); ?>">
          ...
          <?php
            ...
            $wpua_settings['tinymce'] = '<fieldset>
              <label for="wp_user_avatar_tinymce">
                ...
                .__('Add avatar button to Visual Editor', 'herbal-avatar').'
              </label>
            </fieldset>';
            $wpua_settings['upload'] ='<fieldset>
              <label for="wp_user_avatar_allow_upload">
                ...
                .__('Allow Contributors & Subscribers to upload avatars', 'herbal-avatar').'
              </label>
            </fieldset>';
            $wpua_settings['gravatar'] ='<fieldset>
              <label for="wp_user_avatar_disable_gravatar">
                ...
                .__('Disable Gravatar and use only local avatars', 'herbal-avatar').'
              </label>
            </fieldset>';
            /**
          ...
          <tr valign="top">
            <th scope="row">
              <label for="wp_user_avatar_upload_size_limit">'
                .__('Upload Size Limit', 'herbal-avatar').' '.__('(only for Contributors & Subscribers)', 'herbal-avatar').'
              </label>
            </th>
            <td>
              <fieldset>
                <legend class="screen-reader-text"><span>'.__('Upload Size Limit', 'herbal-avatar').' '. __('(only for Contributors & Subscribers)', 'herbal-avatar').'</span></legend>
                ...
                <span id="wpua-readable-size-error">'.sprintf(__('%s exceeds the maximum upload size for this site.','herbal-avatar'), "").'</span>
                <div id="wpua-slider"></div>
                <span class="description">'.sprintf(__('Maximum upload file size: %d%s.','herbal-avatar'), esc_html(wp_max_upload_size()), esc_html(' bytes ('.$upload_size_limit_with_units.')')).'</span>
              </fieldset>
              <fieldset>
                <label for="wp_user_avatar_edit_avatar">
                  ...
                  .__('Allow users to edit avatars', 'herbal-avatar').'
                </label>
              </fieldset>
              <fieldset>
                <label for="wp_user_avatar_resize_upload">
                  ...
                  .__('Resize avatars on upload', 'herbal-avatar').'
                </label>
              </fieldset>
              <fieldset id="wpua-resize-sizes"'.$hide_resize.'>
                <label for="wp_user_avatar_resize_w">'.__('Width','herbal-avatar').'</label>
                ...
                <label for="wp_user_avatar_resize_h">'.__('Height','herbal-avatar').'</label>
                ...
                <label for="wp_user_avatar_resize_crop">'.__('Crop avatars to exact dimensions', 'herbal-avatar').'</label>
              </fieldset>
            </td>
          </tr>
        </table>
    ...
    <table class="form-table">
      <tr valign="top">
      <th scope="row"><?php _e('Avatar Display','herbal-avatar'); ?></th>
      <td>
        <fieldset>
          <legend class="screen-reader-text"><span><?php _e('Avatar Display','herbal-avatar'); ?></span></legend>
          <label for="show_avatars">
          <input type="checkbox" id="show_avatars" name="show_avatars" value="1" <?php checked($show_avatars, 1); ?> />
          <?php _e('Show Avatars','herbal-avatar'); ?>
          </label>
        </fieldset>
        </td>
      </tr>
        <tr valign="top" id="avatar-rating" <?php echo ((bool) $wpua_disable_gravatar == 1) ? 'style="display:none"' : ''?>>
          <th scope="row"><?php _e('Maximum Rating','herbal-avatar'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Maximum Rating','herbal-avatar'); ?></span></legend>
              <?php
                $ratings = array(
                  'G' => __('G &#8212; Suitable for all audiences','herbal-avatar'),
                  'PG' => __('PG &#8212; Possibly offensive, usually for audiences 13 and above','herbal-avatar'),
                  'R' => __('R &#8212; Intended for adult audiences above 17','herbal-avatar'),
                  'X' => __('X &#8212; Even more mature than above','herbal-avatar')
                );
      ...
      <tr valign="top">
        <th scope="row"><?php _e('Default Avatar','herbal-avatar') ?></th>
        <td class="defaultavatarpicker">
          <fieldset>
            <legend class="screen-reader-text"><span><?php _e('Default Avatar','herbal-avatar'); ?></span></legend>
            <?php _e('For users without a custom avatar of their own, you can either display a generic logo or a generated one based on their e-mail address.','herbal-avatar'); ?><br />
