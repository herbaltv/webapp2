 ...
 */
  ...
  if(!current_user_can('upload_files'))
    wp_die(__('You do not have permission to upload files.','herbal-avatar'));
  ...
  if($doaction) {
    ...
    $location = esc_url(add_query_arg(array('page' => 'herbal-avatar-library'), 'admin.php'));
      ...
      case 'delete':
        ...
        foreach((array) $post_ids as $post_id_del) {
          if(!current_user_can('delete_post', $post_id_del)) {
            wp_die(__('You are not allowed to delete this post.','herbal-avatar'));
          }
          if(!wp_delete_attachment($post_id_del)) {
            wp_die(__('Error in deleting.','herbal-avatar'));
          }
        }
...
<div class="wrap">
  <h2>
    <?php _e('Avatars','herbal-avatar');
      if(!empty($_REQUEST['s'])) {
        printf('<span class="subtitle">'.__('Search results for &#8220;%s&#8221;','herbal-avatar').'</span>', get_search_query());
      }
    ?>
  </h2>
  <?php
  ...
  <form id="posts-filter" action="" method="get">
    <?php $wp_list_table->search_box(__('Search','herbal-avatar'), 'media'); ?>
