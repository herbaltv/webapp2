...
class WP_User_Avatar_Admin {
  ...
  public function __construct() {
    ...
    register_activation_hook(WPUA_DIR.'herbal-avatar.php', array($this, 'wpua_options'));
    // Settings saved to wp_options
    add_action('admin_init', array($this, 'wpua_options'));
    // Remove subscribers edit_posts capability
    // Translations
    load_plugin_textdomain('herbal-avatar', "", WPUA_FOLDER.'/lang');
    // Admin menu settings
  ...
  public function wpua_admin() {
    add_menu_page(__('Herbal Avatar', 'herbal-avatar'), __('Avatars', 'herbal-avatar'), 'manage_options', 'herbal-avatar', array($this, 'wpua_options_page'), WPUA_URL.'images/wpua-icon.png');
    add_submenu_page('herbal-avatar', __('Settings' , 'herbal-avatar'), __('Settings' , 'herbal-avatar'), 'manage_options', 'herbal-avatar', array($this, 'wpua_options_page'));
    $hook = add_submenu_page('herbal-avatar', __('Library','herbal-avatar'), __('Library', 'herbal-avatar'), 'manage_options', 'herbal-avatar-library', array($this, 'wpua_media_page'));
  ...
  public function wpua_is_menu_page() {
    global $pagenow;
    $is_menu_page = ($pagenow == 'admin.php' && isset($_GET['page']) && $_GET['page'] == 'herbal-avatar') ? true : false;
  ...
  public function wpua_media_screen_option() {
    $option = 'per_page';
    $args = array(
      'label' => __('Avatars','herbal-avatar'),
      'default' => 10,
  ...
  public function wpua_add_default_avatar() {
    ...
    $avatar_defaults = array(
      'mystery' => __('Mystery Man','herbal-avatar'),
      'blank' => __('Blank','herbal-avatar'),
      'gravatar_default' => __('Gravatar Logo','herbal-avatar'),
      'identicon' => __('Identicon (Generated)','herbal-avatar'),
      'wavatar' => __('Wavatar (Generated)','herbal-avatar'),
      'monsterid' => __('MonsterID (Generated)','herbal-avatar'),
      'retro' => __('Retro (Generated)','herbal-avatar')
    );
    ...
    $wpua_list .= ' '.__('Herbal Avatar', 'herbal-avatar').'</label>';
    $wpua_list .= '<p id="wpua-edit"><button type="button" class="button" id="wpua-add" name="wpua-add" data-avatar_default="true" data-title="'.__('Choose Image').': '.__('Default Avatar').'">'.__('Choose Image','herbal-avatar').'</button>';
    $wpua_list .= '<span id="wpua-remove-button"'.$hide_remove.'><a href="#" id="wpua-remove">'.__('Remove','herbal-avatar').'</a></span><span id="wpua-undo-button"><a href="#" id="wpua-undo">'.__('Undo','herbal-avatar').'</a></span></p>';
    $wpua_list .= '<input type="hidden" id="herbal-avatar" name="avatar_default_wp_user_avatar" value="'.$wpua_avatar_default.'">';
  ...
  public function wpua_action_links($links, $file) { 
    if(basename(dirname($file)) == 'herbal-avatar') {
      $links[] = '<a href="'.esc_url(add_query_arg(array('page' => 'herbal-avatar'), admin_url('admin.php'))).'">'.__('Settings','herbal-avatar').'</a>';
    }
    return $links;
  }
  ...
  public function wpua_add_column($columns) {
    return $columns + array('herbal-avatar' => __('Avatar','herbal-avatar'));
  }
  ...
  public function _wpua_get_list_table($class, $args = array()) {
    require_once(WPUA_INC.'class-herbal-avatar-list-table.php');
    $args['screen'] = 'herbal-avatar';
  ...
  public function wpua_add_media_state($states) {
    ...
    if(!empty($is_wpua)) {
      $states[] = __('Avatar','herbal-avatar');
    }
    if(!empty($wpua_avatar_default) && ($wpua_avatar_default == $post->ID)) {
      $states[] = __('Default Avatar','herbal-avatar');
    }
    /**
     * Filter media states
     * @since 1.4
