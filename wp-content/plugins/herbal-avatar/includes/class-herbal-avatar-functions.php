...
class WP_User_Avatar_Functions {
  ...
  public function __construct() {
    ...
    // Filter to display Herbal Avatar at Buddypress
    add_filter('bp_core_fetch_avatar', array($this, 'wpua_bp_core_fetch_avatar_filter'), 10, 5);
    // Filter to display Herbal Avatar by URL at Buddypress
  ...
  /**
   * Returns Herbal Avatar or Gravatar-hosted image if user doesn't have Buddypress-uploaded image
   * @param string $avatar
   * @param array $params
   * @param int $item_id
  ...
  */
  ...
  public function wpua_get_avatar_filter($avatar, $id_or_email="", $size="", $default="", $alt="") {
    ...
    if( $alt == '' ) {
       $alt = apply_filters('wpua_default_alt_tag',__("Avatar",'herbal-avatar'));
    }
    ...
    if(is_object($id_or_email)) {
      ...
      // User has Gravatar and Gravatar is not disabled
       ...
       //$avatar = $avatar;
       $avatar = '<img src="'.$wpua_final_avatar_image_src.'"'.$default_image_details['dimensions'].' alt="'.$alt.'" class="avatar avatar-'.$size.' herbal-avatar herbal-avatar-'.$size.' photo avatar-default" />';
      ...
      // User doesn't have WPUA or Gravatar and Default Avatar is wp_user_avatar, show custom Default Avatar
       ...
       $default_image_details = $this->wpua_default_image($size); 
       $avatar = '<img src="'.$default_image_details['src'].'"'.$default_image_details['dimensions'].' alt="'.$alt.'" class="avatar avatar-'.$size.' herbal-avatar herbal-avatar-'.$size.' photo avatar-default" />';
...
//      $wpua_avatar_default_image = $wpua_functions->wpua_get_attachment_image_src($wpua_avatar_default, array($size,$size));
  ...
  public function get_wp_user_avatar($id_or_email="", $size='96', $align="", $alt="") {
    ...
    if(!empty($wpua_meta) && $wpua_functions->wpua_attachment_is_image($wpua_meta)) {
      ...
      // Construct the img tag
      $avatar = '<img src="'.$wpua_image[0].'"'.$dimensions.' alt="'.$alt.'" class="avatar avatar-'.$size.' herbal-avatar herbal-avatar-'.$size.$alignclass.' photo" />';
    } else {
      ...
      $replace = array('herbal-avatar ', 'herbal-avatar-'.$get_size.' ', 'herbal-avatar-'.$size.' ', 'avatar-'.$get_size, ' photo');
      $replacements = array("", "", "", 'avatar-'.$size, 'herbal-avatar herbal-avatar-'.$size.$alignclass.' photo');
