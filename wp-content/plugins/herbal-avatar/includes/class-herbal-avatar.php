...
class WP_User_Avatar {
  ...
  public static function wpua_media_upload_scripts($user="") {
    ...
    wp_enqueue_style('herbal-avatar', WPUA_URL.'css/herbal-avatar.css', "", WPUA_VERSION);
    wp_enqueue_script('jquery');
    if($wp_user_avatar->wpua_is_author_or_above()) {
      wp_enqueue_script('admin-bar');
      wp_enqueue_media(array('post' => $post));
      wp_enqueue_script('herbal-avatar', WPUA_URL.'js/herbal-avatar.js', array('jquery', 'media-editor'), WPUA_VERSION, true);
    } else {
      wp_enqueue_script('herbal-avatar', WPUA_URL.'js/herbal-avatar-user.js', array('jquery'), WPUA_VERSION, true);
    }
    // Admin scripts
    if($pagenow == 'options-discussion.php' || $wpua_admin->wpua_is_menu_page()) {
      // Size limit slider
      wp_enqueue_script('jquery-ui-slider');
      wp_enqueue_style('herbal-avatar-jqueryui', WPUA_URL.'css/jquery.ui.slider.css', "", null);
      // Default avatar
      wp_localize_script('herbal-avatar', 'wpua_custom', array('avatar_thumb' => $mustache_admin));
      // Settings control
      wp_enqueue_script('herbal-avatar-admin', WPUA_URL.'js/herbal-avatar-admin.js', array('herbal-avatar'), WPUA_VERSION, true);
      wp_localize_script('herbal-avatar-admin', 'wpua_admin', array('upload_size_limit' => $wpua_upload_size_limit, 'max_upload_size' => wp_max_upload_size()));
    } else {
      ...
      wp_localize_script('herbal-avatar', 'wpua_custom', array('avatar_thumb' => $avatar_medium_src));
    }
  }
  ...
  ?>
    ...
    <input type="hidden" name="herbal-avatar" id="<?php echo ($user=='add-new-user') ? 'herbal-avatar' : 'herbal-avatar-existing'?>" value="<?php echo $wpua; ?>" />
    <?php if($wp_user_avatar->wpua_is_author_or_above()) : // Button to launch Media Uploader ?>
      
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-add-button' : 'wpua-add-button-existing'?>"><button type="button" class="button" id="<?php echo ($user=='add-new-user') ? 'wpua-add' : 'wpua-add-existing'?>" name="<?php echo ($user=='add-new-user') ? 'wpua-add' : 'wpua-add-existing'?>" data-title="<?php _e('Choose Image','herbal-avatar'); ?>: <?php echo ( ! empty( $user->display_name ) ? $user->display_name:''); ?>"><?php _e('Choose Image','herbal-avatar'); ?></button></p>
    ...
    <?php elseif(!$wp_user_avatar->wpua_is_author_or_above()) : // Upload button ?>
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-upload-button' : 'wpua-upload-button-existing'?>">
        ...
        <button type="submit" class="button" id="<?php echo ($user=='add-new-user') ? 'wpua-upload' : 'wpua-upload-existing'?>" name="submit" value="<?php _e('Upload','herbal-avatar'); ?>"><?php _e('Upload','herbal-avatar'); ?></button>
      </p>
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-upload-messages' : 'wpua-upload-messages-existing'?>">
        <span id="<?php echo ($user=='add-new-user') ? 'wpua-max-upload' : 'wpua-max-upload-existing'?>" class="description"><?php printf(__('Maximum upload file size: %d%s.','herbal-avatar'), esc_html($wpua_upload_size_limit_with_units), esc_html('KB')); ?></span>
        <span id="<?php echo ($user=='add-new-user') ? 'wpua-allowed-files' : 'wpua-allowed-files-existing'?>" class="description"><?php _e('Allowed Files','herbal-avatar'); ?>: <?php _e('<code>jpg jpeg png gif</code>','herbal-avatar'); ?></span>
      </p>
    <?php endif; ?>
    <div id="<?php echo ($user=='add-new-user') ? 'wpua-images' : 'wpua-images-existing'?>" class="<?php echo $hide_images; ?>">
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-preview' : 'wpua-preview-existing'?>">
        ...
        <span class="description"><?php _e('Original Size','herbal-avatar'); ?></span>
      </p>
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-thumbnail' : 'wpua-thumbnail-existing'?>">
        <img src="<?php echo $avatar_thumbnail; ?>" alt="" />
        <span class="description"><?php _e('Thumbnail','herbal-avatar'); ?></span>
      </p>
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-remove-button' : 'wpua-remove-button-existing'?>" class="<?php echo $hide_remove; ?>">
        <button type="button" class="button" id="<?php echo ($user=='add-new-user') ? 'wpua-remove' : 'wpua-remove-existing'?>" name="wpua-remove"><?php _e('Remove Image','herbal-avatar'); ?></button>
        <?php if((bool) $wpua_edit_avatar == 1 && !$wp_user_avatar->wpua_is_author_or_above() && has_wp_user_avatar($current_user->ID) && $wp_user_avatar->wpua_author($wpua, $current_user->ID)) : // Edit button ?>
          <span id="<?php echo ($user=='add-new-user') ? 'wpua-edit-attachment' : 'wpua-edit-attachment-existing'?>"><a href="<?php echo $edit_attachment_link; ?>" class="edit-attachment" target="_blank"><?php _e('Edit Image','herbal-avatar'); ?></a></span>
        <?php endif; ?>
      </p>
      <p id="<?php echo ($user=='add-new-user') ? 'wpua-undo-button' : 'wpua-undo-button-existing'?>"><button type="button" class="button" id="<?php echo ($user=='add-new-user') ? 'wpua-undo' : 'wpua-undo-existing'?>" name="wpua-undo"><?php _e('Undo','herbal-avatar'); ?></button></p>
    </div>
  ...
  public static function wpua_upload_errors($errors, $update, $user) {
    ...
    if($update && !empty($_FILES['wpua-file'])) {
      ...
      if(!empty($type) && !preg_match('/(jpe?g|gif|png)$/i', $type)) {
        $errors->add('wpua_file_type', __('This file is not an image. Please try another.','herbal-avatar'));
      }
      // Upload size limit
      if(!empty($size) && $size > $wpua_upload_size_limit) {
        $errors->add('wpua_file_size', __('Memory exceeded. Please try another smaller file.','herbal-avatar'));
      }
      // Check if directory is writeable
      if(!is_writeable($upload_dir['path'])) {
        $errors->add('wpua_file_directory', sprintf(__('Unable to create directory %s. Is its parent directory writable by the server?','herbal-avatar'), $upload_dir['path']));
      }
    }
  }
  ...
  public static function wpua_action_process_option_update($user_id) {
    ...
    if($wp_user_avatar->wpua_is_author_or_above()) {
      $wpua_id = isset($_POST['herbal-avatar']) ? strip_tags($_POST['herbal-avatar']) : "";
      ...
      if(isset($_POST['herbal-avatar']) && empty($_POST['herbal-avatar'])) {
