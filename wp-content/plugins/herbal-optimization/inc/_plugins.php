...
class HERBAL_OPTIMIZATION_PLUGINS {
	...
	function __construct(){
		...
		add_action( 'Herbs/builder/after_save', array( $this, 'check_sections' ), 10, 2 );
	...
	function dequeue_scripts_styles(){
		...
		if( ! HERBS_HELPER::has_builder() || ! is_singular('post') ){
			return;
		}

		// WooCommerce
		if( HERBS_WOOCOMMERCE_IS_ACTIVE && ! $this->is_plugin_load('woocommerce') ){
		...
		// BuddyPress
		if( HERBS_BUDDYPRESS_IS_ACTIVE && ! $this->is_plugin_load('buddypress') ){
		...
		// BBPress
		if( HERBS_BBPRESS_IS_ACTIVE && ! $this->is_plugin_load('bbpress') ){
...
//
add_filter( 'init', 'herbal_optimization_plugins_init' );
function herbal_optimization_plugins_init(){

	// This method available in v4.0.0 and above
	if( method_exists( 'HERBS_HELPER','has_builder' ) ){
		new HERBAL_OPTIMIZATION_PLUGINS();
	}
}
