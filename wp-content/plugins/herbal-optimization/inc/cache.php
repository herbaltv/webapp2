...
class HERBAL_OPTIMIZATION_CACHE{
	...
	function __construct(){
		...
		$this->cache_key  = apply_filters( 'Herbs/cache_key', '' );
		...
		add_action( 'Herbs/Options/updated',   array( $this, 'transient_flusher' ) );
		add_action( 'Herbs/after_db_update',   array( $this, 'transient_flusher' ) );
...
//
add_filter( 'init', 'herbal_optimization_cache_init' );
function herbal_optimization_cache_init(){

	// This method available in v4.0.0 and above
	if( method_exists( 'HERBS_HELPER','has_builder' ) ){
		new HERBAL_OPTIMIZATION_CACHE();
	}
}
