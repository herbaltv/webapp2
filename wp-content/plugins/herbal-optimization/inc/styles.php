...
class HERBAL_OPTIMIZATION_STYLES {
	...
	function __construct(){
		...
		add_action( 'Herbs/after_db_update',    array( $this, 'delete_critical_css' ) );

		// Load Style files
		add_action( 'Herbs/after_header',       array( $this, 'after_header' ),  1 );
		...
		if( tie_get_option( 'jso_css_delivery' ) ){
			...
			add_filter( 'Herbs/Styles/is_inline_css',  '__return_true' );
			add_filter( 'Herbs/CSS/after_theme_color',   array( $this, 'inline_css_code' ), 100 );
			add_filter( 'Herbs/CSS/Builder/block_style', array( $this, 'inline_css_code' ), 100 );
		}
	}
	...
	function save_critical_css(){
		...
		if( is_wp_error( $request ) ){
			...
			if( ! empty( $css ) ){
				...
				if( ! HERBS_STYLES::is_minified() ){
					$css = HERBS_STYLES::minify_css( $css );
				}

				// Store the code
				set_transient( 'tie_critical_css_'.HERBS_THEME_ID, $css, YEAR_IN_SECONDS );
	...
	function print_critical_css(){
		...
		if ( false === ( $css = get_transient( 'tie_critical_css_'.HERBS_THEME_ID ) ) ) {
	...
	function after_header(){
		...
		// Shortcodes
		if( ! HERBAL_OPTIMIZATION_RESOURCES::plugins_resources_disabled( 'shortcodes' ) ){
			$this->do_style( 'tie-css-shortcodes' );
			wp_enqueue_script( 'tie-js-shortcodes' ); // Will be loaded in the footer
		}

		// Taqyeem
		if( HERBS_TAQYEEM_IS_ACTIVE && is_singular() ){
	...
	function rtl_theme_styles(){
		...
		// Taqyeem
		if( ! HERBS_TAQYEEM_IS_ACTIVE || ! is_singular() || ! tie_get_postdata( 'taq_review_position' ) ){
			wp_dequeue_style( 'taqyeem-styles' );
			wp_dequeue_style( 'taqyeem-buttons-style' );
		}
...
//
add_filter( 'init', 'herbal_optimization_styles_init' );
function herbal_optimization_styles_init(){

	// This method available in v4.0.0 and above
	if( method_exists( 'HERBS_HELPER','has_builder' ) ){
		new HERBAL_OPTIMIZATION_STYLES();
	}
}
