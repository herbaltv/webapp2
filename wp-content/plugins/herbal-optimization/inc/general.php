...
class HERBAL_OPTIMIZATION_GENERAL {
	...
	function __construct(){
		...
		// Track Usage
		add_filter( 'Herbs/api_connect_body', array( $this, 'api_connect_body' ) );
		...
		if( tie_get_option( 'jso_disable_fonts_2g' ) ){
			add_filter( 'Herbs/google_fonts/js_code', array( $this, 'google_fonts_disable_2g' ), 10, 2 );
		}

		// Disable Google Fonts On Mobiles
		if( tie_get_option( 'jso_disable_fonts_mobile' ) ){
			add_filter( 'Herbs/google_fonts/js_code', array( $this, 'google_fonts_disable_mobile' ), 15, 2 );
		}
		...
		if( tie_get_option( 'jso_ajax' ) ){
			add_filter( 'Herbs/js_main_vars', array( $this, 'ajax_file_path' ) );
		}
		*/
	}
	...
	function preload_resources(){
		...
		$fonts = apply_filters( 'Herbs/preload_resources/fonts', $fonts );

		foreach ( $fonts as $font => $type ) {
			echo "<link rel='preload' as='font' href='".HERBS_TEMPLATE_URL."/assets/fonts/$font' type='font/$type' crossorigin='anonymous' />\n";
		}
...
//
add_filter( 'init', 'herbal_optimization_general_init' );
function herbal_optimization_general_init(){

	// This method available in v4.0.0 and above
	if( method_exists( 'HERBS_HELPER','has_builder' ) ){
		new HERBAL_OPTIMIZATION_GENERAL();
	}
}
