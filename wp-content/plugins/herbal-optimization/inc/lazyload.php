...
class HERBAL_OPTIMIZATION_LAZYLOAD {
	...
	function __construct() {
		...
		// Ads Lazyload images
		add_filter( 'Herbs/Ad_widget/code',  array( $this, 'lazyload_ads' ) );
		add_filter( 'Herbs/Ad_widget/image', array( $this, 'lazyload_ads' ) );
		add_filter( 'Herbs/block/ad_code',   array( $this, 'lazyload_ads' ) );
		add_filter( 'Herbs/block/ad_image',  array( $this, 'lazyload_ads' ) );
	}
	...
	function is_lazyload_active(){
		...
		if( HERBS_JETPACK_IS_ACTIVE && in_array( 'photon', Jetpack::get_active_modules() ) && in_array( 'the_content', $GLOBALS['wp_current_filter'] ) ){
			return false;
		}

		// WooCommerce
		if( HERBS_WOOCOMMERCE_IS_ACTIVE && in_array( 'woocommerce_review_before', $GLOBALS['wp_current_filter'] ) ){
			return false;
		}
	...
	function lazyload_image_attributes( $attr, $attachment, $size = false ) {
		...
		$blank_image = ( $size == HERBS_THEME_SLUG.'-image-small' ) ? tie_lazyload_placeholder('small') : tie_lazyload_placeholder();
...
//
add_filter( 'init', 'herbal_optimization_lazyload_init' );
function herbal_optimization_lazyload_init(){

	// This method available in v4.0.0 and above
	if( method_exists( 'HERBS_HELPER','has_builder' ) ){
		new HERBAL_OPTIMIZATION_LAZYLOAD();
	}
}
