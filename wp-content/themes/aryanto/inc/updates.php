...
# Debug
//update_option( 'tie_ver_'. HERBS_THEME_ID, '2.0.0' );

$current_version = get_option( 'tie_ver_'. HERBS_THEME_ID ) ? get_option( 'tie_ver_'. HERBS_THEME_ID ) : get_option( 'tie_herbal_ver' );

// Apply updates
if( $current_version ){

	if( version_compare( $current_version, HERBS_DB_VERSION, '<' ) ){

		// ChangeLog
		$changelog = '';

		// Custom Versions updates
		$updated_options = $original_options = get_option( apply_filters( 'Herbs/theme_options', '' ) );
		...
		if( version_compare( $current_version, '1.1.0', '<' ) ){
			...
			delete_transient( 'tie-data-'.HERBS_THEME_SLUG );

			// Chnagelog
			$changelog .= '
				NEW: Introducing our Herbal Switcher Plugin now you can migrating your posts from 17 themes to Herbal.
		...
		if( version_compare( $current_version, '1.2.0', '<' ) ){
			...
			$changelog .= "
				...
				NEW: The new Audio, Video and Image widgets are now available in the Herbs Page builder Widgets section.
		...
		if( version_compare( $current_version, '2.1.0', '<' ) ){
			...
			$options_to_update = array(
				'tie_herbal_installed_demo' => 'tie_installed_demo',
				'tie_herbal_installed_demo' => 'tie_installed_demo_'.  HERBS_THEME_ID,
				'herbal_published_posts'    => 'tie_published_posts_'. HERBS_THEME_ID,
				'tie_herbal_install_date'   => 'tie_install_date_'. HERBS_THEME_ID ,
				'herbal_chnagelog'          => 'tie_chnagelog_'. HERBS_THEME_ID,
				'herbal_foxpush_code'       => 'tie_foxpush_code_'. HERBS_THEME_ID,
				'switch_to_herbal'          => 'tie_switch_to_'. HERBS_THEME_ID,
				'tie_herbal_ver'            => 'tie_ver_'. HERBS_THEME_ID,
			);
			...
			$translation_texts = apply_filters( 'Herbs/translation_texts', array() );
		...
		if( version_compare( $current_version, '2.1.1', '<' ) ){
			...
			if( get_option('switch_to_herbal') ){

				$theme_switched = get_option('switch_to_herbal');
				update_option( 'tie_switch_to_'. HERBS_THEME_ID, $theme_switched, false );
				delete_option('switch_to_herbal');
			}
		}
		...
		if( version_compare( $current_version, '3.0.0', '<' ) ){
			...
			// Chnagelog
			$changelog .= "
				- NEW: Fitness Demo https://herbaltv.co.id
				- NEW: Salad Dash Demo https://herbaltv.co.id
		...
		if( version_compare( $current_version, '4.0.0', '<' ) ){
			...
			// Chnagelog
			$changelog .= "
				- Performance: Introduced the new Herbal Speed Optimization plugin (beta).
		...
		if( ! empty( $changelog ) ){
			...
			// Store the new data
			update_option( 'tie_chnagelog_'. HERBS_THEME_ID, trim( $changelog ), false );

			// Remove the pointer from the dismissed array
			$dismissed = array_filter( explode( ',', (string) get_user_meta( get_current_user_id(), 'dismissed_wp_pointers', true ) ) );
			$pointer   = 'tie_new_updates_'. HERBS_THEME_ID;
		...
		if( $updated_options != $original_options ){
			update_option( apply_filters( 'Herbs/theme_options', '' ), $updated_options );
		}
		...
		do_action( 'Herbs/after_db_update' );
