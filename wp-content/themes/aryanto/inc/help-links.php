...
if( ! class_exists( 'HERBS_EXTERNAL_LINKS' )){

	class HERBS_EXTERNAL_LINKS{
		...
		function __construct(){
			...
			add_action( 'Herbs/admin_after_tab_title',  array( $this, 'help_icons' ), 10, 1 );
			add_action( 'Herbs/admin_after_head_title', array( $this, 'help_icons' ), 10, 1 );
			...
			add_filter( 'Herbs/External/hosting',               array( $this, 'get_hosting' ));
			add_filter( 'Herbs/External/support_center',        array( $this, 'support_center' ));
			add_filter( 'Herbs/External/update_manually',       array( $this, 'update_manually' ));
			add_filter( 'Herbs/External/theme_footer',          array( $this, 'theme_footer' ));
			add_filter( 'Herbs/External/knowledge_base',        array( $this, 'knowledge_base' ));
			add_filter( 'Herbs/External/troubleshooting',       array( $this, 'troubleshooting' ));
			add_filter( 'Herbs/External/max_input_vars',        array( $this, 'max_input_vars' ));
			add_filter( 'Herbs/External/licenses_article',      array( $this, 'licenses_article' ));
			add_filter( 'Herbs/External/renew_support_article', array( $this, 'renew_support_article' ));
			add_filter( 'Herbs/External/open_ticket',           array( $this, 'open_ticket' ));
			add_filter( 'Herbs/External/share_idea',            array( $this, 'share_idea' ));
			add_filter( 'Herbs/External/portfolio',             array( $this, 'portfolio' ));
			add_filter( 'Herbs/External/foxpush',               array( $this, 'foxpush' ));
			add_filter( 'Herbs/External/changelog',             array( $this, 'changelog' ));
			add_filter( 'Herbs/translations_panel_url',         array( $this, 'translations_panel_url' ));
		}
		...
		function get_hosting( $url ){
			return esc_url( 'https://herbaltv.co.id' );
		}
		...
		function changelog( $url ){
			return esc_url( 'https://herbaltv.co.id?id='. HERBS_THEME_ID );
		}
		...
		function help_icons( $id = false ){
			...
			$options = array(
				...
				//
				'speed-optimization-tab' => '211-herbal-speed-optimization-plugin-settings',

			);

			add_thickbox();

			$docs_url = 'https://herbaltv.co.id';

			if( ! empty( $id ) && ! empty( $options[ $id ] ) ){
				echo '<a href="'. esc_url( $docs_url . $options[ $id ] ) .'?tiedocs=true&TB_iframe=true&width=650&height=750" id="help-icon-'. $id .'" class="docs-link thickbox" target="_blank" rel="nofollow noopener" title="'. esc_html__( 'Need Help?', HERBS_TEXTDOMAIN ) .'"><span class="dashicons dashicons-editor-help"></span></a>';
			}
