...
if( ! class_exists( 'HERBS_EXTENSIONS' )){

	class HERBS_EXTENSIONS{
		...
		function __construct(){
			...
			add_filter( 'Herbs/shortcodes_check', array( $this, 'shortcodes_check' ), 10, 2 );
			...
			add_action( 'Herbs/before_single_post_title', array( $this, 'post_index' ) );
		}
			...
			foreach( $shortcodes_list as $shortcode ){
				if( strpos( $content, $shortcode ) !== false ){
					$message .= HERBS_HELPER::notice_message( sprintf(
						esc_html__( 'This section contains some shortcodes that requries the %s Plugin. Install it from the Theme Menu &gt; Install Plugins.', HERBS_TEXTDOMAIN ),
						'<strong>Herbal Extinsions</strong>'
					), false );
