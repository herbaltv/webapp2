...
if( ! class_exists( 'HERBS_WOOCOMMERCE' )){

	class HERBS_WOOCOMMERCE{
		...
		function __construct(){
			...
			add_action( 'Herbs/wc_cart_menu_content', array( $this, 'header_cart_content' ) );

			// WooCommerce pages logo
			add_filter( 'Herbs/Logo/args', array( $this, 'logo_args' ), 10, 2 );

			// Blocks Query
			add_filter( 'Herbs/Query/args', array( $this, 'block_query_args' ), 10, 2 );

			// Post Sidebars Settings
			add_filter( 'Herbs/Settings/Post/sidebar/defaults', array( $this, 'post_sidebar_settings' ) );
		...
		function enqueue_styles( $styles ) {
			...
			$styles['tie-css-woocommerce'] = array(
				'src'     => HERBS_TEMPLATE_URL.'/assets/css/plugins/woocommerce'. HERBS_STYLES::is_minified() .'.css',
				'deps'    => '',
				'version' => HERBS_DB_VERSION,
				'media'   => 'all',
			);
		...
		public static function categories( $label = false ){
			...
			if( ! empty( $label )){
				$categories = array( '' => esc_html__( '- Select a category -', HERBS_TEXTDOMAIN ));
			}
		...
		function product_thumbnails_slider(){
			...
			HERBS_HELPER::inline_script( 'tie-scripts', $products_script );
		}
		...
		function cart_items_details( $fragments ){
			...
			do_action( 'Herbs/wc_cart_menu_content' );
		...
		function header_cart_content(){
			...
			if( ! empty( $cart_items ) ){ ?>
							...
							<li>
								...
								<div class="product-meta">
									<div class="product-quantity-price">
										<?php printf(
												esc_html__( '%1$s x %2$s.', HERBS_TEXTDOMAIN ),
												$details['quantity'],
								...
								<a href="<?php echo esc_url( $remove_url  ) ?>" class="remove"><span class="screen-reader-text"><?php esc_html_e( 'Remove', HERBS_TEXTDOMAIN ); ?></span></a>
							</li>
				...
				<div class="shopping-subtotal">
					<?php esc_html_e( 'Subtotal:', HERBS_TEXTDOMAIN ); ?> <?php echo WC()->cart->get_total(); ?>
				</div><!-- .shopping-subtotal /-->

				<a href="<?php echo wc_get_checkout_url() ?>" class="checkout-button button"><?php esc_html_e( 'Checkout', HERBS_TEXTDOMAIN ); ?></a>
				<a href="<?php echo wc_get_cart_url() ?>" class="view-cart-button button guest-btn"><?php esc_html_e( 'View Cart', HERBS_TEXTDOMAIN ); ?></a>
				<?php
			}
			else{ ?>
				<div class="cart-empty-message">
					<?php esc_html_e( 'Your cart is currently empty.', HERBS_TEXTDOMAIN ); ?>
				</div>
				<a href="<?php echo get_permalink( wc_get_page_id( 'shop' ) ); ?>" class="checkout-button button"><?php esc_html_e( 'Go to the shop', HERBS_TEXTDOMAIN ); ?></a>
				<?php
			}
		...
		function post_sidebar_settings( $current_settings ){
			...
			return array(
				...
				array(
					'text' =>  sprintf(
						esc_html__( 'Control WooCommerce sidebar settings from the theme options page &gt; %1$sWooCommerce settings%2$s.', HERBS_TEXTDOMAIN ),
