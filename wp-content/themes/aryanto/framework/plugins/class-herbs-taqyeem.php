...
if( ! class_exists( 'HERBS_TAQYEEM' )){
	...
	class HERBS_TAQYEEM{
		...
		function __construct(){
			...
			add_filter( 'taqyeem_exclude_content', 'HERBS_HELPER::strip_shortcodes' );
			...
			// Alter the Queries
			add_filter( 'Herbs/Query/args', array( $this, 'best_reviews_query' ), 10, 2 );

			// Alter the Related Posts Query
			add_filter( 'Herbs/related_posts_query', array( $this, 'related_posts_query' ) );

			// Alter the Check Also box Query
			add_filter( 'Herbs/checkalso_query', array( $this, 'checkalso_query' ) );
			...
			add_filter( 'Herbs/is_page_rich_snippet', array( $this, 'rich_snippet_for_page' ) );

			// Change the Review Rich Snippet
			add_filter( 'Herbs/rich_snippet_schema', array( $this, 'review_rich_snippet' ) );

			// Add Best Reviews to the post order option menu.
			add_filter( 'Herbs/Options/Related/post_order_args',   array( $this, 'posts_order_args' ) );
			add_filter( 'Herbs/Options/Checkalso/post_order_args', array( $this, 'posts_order_args' ) );
			add_filter( 'Herbs/Builder/Block/post_order_args',     array( $this, 'posts_order_args' ) );
			add_filter( 'Herbs/Widget/Slider/post_order_args',     array( $this, 'posts_order_args' ) );
			add_filter( 'Herbs/Widget/Posts/post_order_args',      array( $this, 'posts_order_args_2' ) );

			// Prevent Taqyeem from displaying the review box in the content block in the builder
			add_action( 'Herbs/Builder/before', array( $this, 'remove_review_block' ) );
		...
		function posts_order_args( $args ){
			...
			$args['best'] = esc_html__( 'Best Reviews', HERBS_TEXTDOMAIN );
		...
		function posts_order_args_2( $args ){
			...
			$args['standard']['best'] = esc_html__( 'Best Reviews', HERBS_TEXTDOMAIN );
		...
		function reviews_thumb_size(){
			return HERBS_THEME_SLUG.'-image-small';
		}
		...
		function enqueue_scripts(){
			...
			wp_enqueue_style( 'taqyeem-styles', HERBS_TEMPLATE_URL.'/assets/css/plugins/taqyeem'. HERBS_STYLES::is_minified() .'.css', array(), HERBS_DB_VERSION, 'all' );
