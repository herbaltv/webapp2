...
if( ! class_exists( 'HERBS_BUDDYPRESS' )){

	class HERBS_BUDDYPRESS{
		...
		function __construct(){
			...
			add_filter( 'Herbs/BuddyPress/notifications', array( $this, 'get_notifications' ) );

			// Js Vars
			add_filter( 'Herbs/js_main_vars', array( $this, 'js_var' ) );
		...
		function enqueue_resources(){
			...
			wp_enqueue_style( 'tie-css-buddypress', HERBS_TEMPLATE_URL.'/assets/css/plugins/buddypress'. HERBS_STYLES::is_minified() .'.css', array('dashicons'), HERBS_DB_VERSION, 'all' );
			...
			HERBS_HELPER::inline_script( 'jquery-masonry', $masonry_js );
		}
		...
		function get_notifications(){
			...
			if ( ! empty( $notifications ) ){
				...
				$out_data .= '<li id="no-notifications" class="notifications-item"><a href="'. $menu_link .'"><span class="fa fa-bell-o"></span>  '. esc_html__( 'No new notifications', HERBS_TEXTDOMAIN ) .'</a></li>';
			}
		...
		function cover_image_css( $settings = array() ){
			...
			$settings['width']         = 1400;
			$settings['height']        = 440;
			$settings['default_cover'] = HERBS_TEMPLATE_URL. '/assets/images/default-cover-image.jpg';
		...
		function cover_image_callback( $params = array() ){
			...
			if( $params['cover_image'] == HERBS_TEMPLATE_URL. '/assets/images/default-cover-image.jpg' ){
				$background_attr = '
