...
if( ! empty( $sections ) && is_array( $sections ) ){
	...
	do_action( 'Herbs/Builder/before' );
...
<div id="<?php echo esc_attr( $section_id ) ?>" class="section-wrapper container<?php echo esc_attr( $outer_class ) ?>">
	<div class="section-item <?php echo join( ' ', $classes ); ?>" style="<?php echo join( ' ', $section_styles ); ?>" <?php echo ( $section_video_bg.$parallax_options ) ?>>
		...
		if( $section_settings['section_title'] && ( $section_settings['title'] || $section_settings['title_icon'] ) ){
			...
			echo apply_filters( 'Herbs/before_section_title', $before_section_title, $section_settings );

			echo '<h2 class="'. $title_class .'">';
			echo '<span class="the-section-title">' .$before_section_link . $title . $after_section_link.'</span>';
			echo '</h2>';

			echo apply_filters( 'Herbs/after_section_title', $after_section_title, $section_settings );
		}
		...
		if( ! empty( $section['blocks'] ) && is_array( $section['blocks'] )){
			...
			foreach( $section['blocks'] as $block ){
				...
				if( ( strpos( $style, 'slider-' ) !== false ) || $style == 'videos-list' || $style == 'lsslider' || $style == 'revslider' ){
					...
					HERBS_HELPER::get_template_part( 'templates/featured', '', array(
				...
				elseif( strpos( $style, 'apps-' ) !== false ){
					switch ( $style ){
						case 'apps-bmi':
							HERBS_HELPER::get_template_part( 'templates/apps-bmi', '', $args );
						break;
					}
				 ...
				 */
					...
					if( $custom_block_style = apply_filters( 'Herbs/block_style_'. $style .'_args', false ) ){
						...
						switch ( $style ){
							...
							case 'woocommerce':
								...
								if( $sidebar == 'full-width' ){
									...
									add_filter( 'loop_shop_columns', array( 'HERBS_WOOCOMMERCE', 'full_width_loop_shop_columns' ), 99, 1 );
								}
								else{
									...
									remove_filter( 'loop_shop_columns', array( 'HERBS_WOOCOMMERCE', 'full_width_loop_shop_columns'), 99, 1 );
								}
					...
					// Get the block query
					$block = apply_filters( 'Herbs/Builder/Block/args', $block );
					...
					// Before Block Action
					do_action( 'Herbs/Builder/before_block' );
			...
			if( ! empty( $style ) ):
													...
													if( $cat_query->have_posts() ){
														echo ( $before_items );
															while ( $cat_query->have_posts() ){
																...
																HERBS_HELPER::get_template_part( 'templates/loops/loop', 'large-first', $b_args );

																// Do not dublicate posts
																if( $is_do_not_dublicate ){
																	HERBS_HELPER::do_not_dublicate( get_the_ID() );
																}
															}
														echo ( $after_items );
													}
				...
				elseif( $style == 'breaking' ):
					...
					HERBS_HELPER::get_template_part( 'templates/breaking-news', '', array(
						'type'            => 'block',
					...
					<div class="container-wrapper">
						<?php
							...
							if( ! empty( $block['ad_img'] ) ){
								...
								$url      = apply_filters( 'Herbs/ads_url', empty( $block['ad_url'] ) ? '' : esc_url( $block['ad_url'] ) );
								$alt 		  = empty( $block['ad_alt'] ) ? '' : esc_attr( $block['ad_alt'] );

								echo "<a href=\"$url\" title=\"$alt\"$target$nofollow>";
								echo apply_filters( "Herbs/block/ad_image", "<img src=\"$ad_image\" alt=\"$alt\" width=\"728\" height=\"90\">", $ad_image ) ;
								echo"</a>";
							}

							// Get the Ad Custom Code
							elseif( ! empty( $block['ad_code'] ) ){
								echo do_shortcode( apply_filters( 'Herbs/block/ad_code', $block['ad_code'] ) );
							}
						?>
					</div><!-- .container-wrapper /-->
				<?php
				...
				/*-----------------------------------------------------------------------------------*/
					...
					<div class="container-wrapper">
						...
						<div class="mag-box-container clearfix">
							...
							elseif( $style == 'trendslider'){
								// Get the custom content code and apply the content filters
								if( ! empty( $block['custom_content'] ) ){
									echo '
										<div class="entry clearfix">'. $block['custom_content'] . '</div>
									';
							...
							elseif( ( $style == 'woocommerce' || $style == 'woocommerce-slider' ) ){
								...
								if( HERBS_WOOCOMMERCE_IS_ACTIVE ){
									...
									if ( $block_query->have_posts() ){
										...
										esc_html_e( 'No products found', HERBS_TEXTDOMAIN );
									}

									echo '</ul>';
								}
								else{
									HERBS_HELPER::notice_message( esc_html__( 'This Block requires the WooCoomerce plugin.', HERBS_TEXTDOMAIN ) );
								}
							...
							// Posts Blocks
								...
								if( $block_query->have_posts() ){
									...
									while ( $block_query->have_posts() ){
										...
										HERBS_HELPER::get_template_part( 'templates/loops/loop', $style, $b_args );
						...
						<?php
							...
							if ( ! empty( $block['pagi'] ) && $max_page > 1 ){
								...
								if( $block['pagi'] == 'numeric' ){
									HERBS_PAGINATION::show( array( 'query' => $block_query, 'type' => 'numeric' ) );
								}

								// Show more button Pagination
								elseif( $block['pagi'] == 'show-more' ){
									echo'<a class="block-pagination next-posts show-more-button" data-text="'. esc_html__( 'Show More', HERBS_TEXTDOMAIN ) .'">'. esc_html__( 'Show More', HERBS_TEXTDOMAIN ) .'</a>';
								}

								// Load more button Pagination
								elseif( $block['pagi'] == 'load-more' ){
									echo '<a class="block-pagination next-posts show-more-button load-more-button" data-text="'. esc_html__( 'Load More', HERBS_TEXTDOMAIN ) .'">'. esc_html__( 'Load More', HERBS_TEXTDOMAIN ) .'</a>';
								}
							}
						?>
			...
			if( ! empty( $sidebar_position ) && $sidebar_position != 'full' ):
				...
				if( ! empty( $section['settings']['predefined_sidebar'] ) ){
					if( ! empty( $section['settings']['sidebar_id'] ) ){
						...
						if( empty( $sidebar ) || ( ! empty( $sidebar ) && ! HERBS_HELPER::is_sidebar_registered( $sidebar ) )){
				...
				?>
					...
					<aside class="<?php echo esc_attr( $sidebar_class ); ?>" aria-label="<?php esc_html_e( 'Primary Sidebar', HERBS_TEXTDOMAIN ); ?>">
