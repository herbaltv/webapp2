...
if( ! function_exists( 'tie_breadcrumbs' )){
	...
	add_action( 'Herbs/before_archive_title', 'tie_breadcrumbs' );
	add_action( 'Herbs/before_entry_head',    'tie_breadcrumbs' );

	function tie_breadcrumbs(){

		// If the breadcrumbs is disabled OR is hidden on mobiles
		if( ! tie_get_option( 'breadcrumbs' ) || HERBS_HELPER::is_mobile_and_hidden( 'breadcrumbs' ) ){
			return;
		}
		...
		$home_text   = esc_html__( 'Home', HERBS_TEXTDOMAIN );
		$before      = '<span class="current">';
		$after       = '</span>';
		$breadcrumbs = array();

		// bbPress breadcrumbs
		if( HERBS_BBPRESS_IS_ACTIVE && is_bbpress() && ( ! HERBS_BUDDYPRESS_IS_ACTIVE || ( HERBS_BUDDYPRESS_IS_ACTIVE && ! is_buddypress() ))){
		...
		elseif ( ! is_home() && ! is_front_page() || is_paged() ){
			...
			if ( is_category() ){
				...
				if( $category->parent !== 0 ){
					...
					foreach ( $parent_categories as $parent_category ) {
						$breadcrumbs[] = array(
							'url'  => HERBS_WP_HELPER::get_term_link( $parent_category, 'category' ),
			...
			elseif ( is_search() ){
				...
				$breadcrumbs[] = array(
					'name' => sprintf( esc_html__( 'Search Results for: %s', HERBS_TEXTDOMAIN ),  get_search_query() ),
				);
			}
			...
			elseif ( is_404() ){
				...
				$breadcrumbs[] = array(
					'name' => esc_html__( 'Nothing Found', HERBS_TEXTDOMAIN ),
				);
			}
			...
			elseif ( is_singular() ){
				...
				if ( get_post_type() == 'post' ){
					...
					if( ! empty( $category ) ){
						...
						$breadcrumbs[] = array(
							'url'  => HERBS_WP_HELPER::get_term_link( $category->term_id, 'category' ),
