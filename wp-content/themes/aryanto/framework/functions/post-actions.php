...
if( ! function_exists( 'tie_show_page_head' )){
	...
	add_action( 'Herbs/before_single_post_title', 'tie_show_page_head', 10 );
	function tie_show_page_head(){


		$exclude_post_types = apply_filters( 'Herbs/page_head/exc_post_types', HERBS_HELPER::get_supported_post_types() );

		if( is_page() || ( HERBS_BBPRESS_IS_ACTIVE && is_bbpress() ) || ( is_singular() && ! in_array( get_post_type(), $exclude_post_types ) ) ){

			HERBS_HELPER::get_template_part( 'templates/page', 'head' );
		}
	}
}
...
if( ! function_exists( 'tie_show_post_head_featured' )){
	...
	add_action( 'Herbs/before_single_post_title', 'tie_show_post_head_featured', 10 );
	function tie_show_post_head_featured(){
		...
		if( $post_layout == 2 || $post_layout == 3 || $post_layout == 4 || $post_layout == 5 || $post_layout == 8 ){
			HERBS_HELPER::get_template_part( 'templates/single-post/featured' );
		}

		if( $post_layout == 1 || $post_layout == 2 || $post_layout == 6 ){
			HERBS_HELPER::get_template_part( 'templates/single-post/head' );
		}

		// Get the top share buttons
		HERBS_HELPER::get_template_part( 'templates/single-post/share', '', array( 'share_position' => 'top' ) );
...
if( ! function_exists( 'tie_story_highlights' )){
	...
	add_action( 'Herbs/before_post_content', 'tie_story_highlights', 10 );
			...
			echo '
				<div id="story-highlights">
					<div '. tie_box_class( 'widget-title', false ) .'>
						<div class="the-subtitle">'. esc_html__( 'Story Highlights', HERBS_TEXTDOMAIN ) .'</div>
					</div>
					<ul>';
...
if( ! function_exists( 'tie_post_multi_pages' )){
	...
	add_action( 'Herbs/after_post_content', 'tie_post_multi_pages', 10 );
...
if( ! function_exists( 'tie_post_source_via' )){
	...
	add_action( 'Herbs/after_post_content', 'tie_post_source_via', 20 );
...
if( ! function_exists( 'tie_post_tags' )){
	...
	add_action( 'Herbs/after_post_content', 'tie_post_tags', 30 );
	function tie_post_tags(){
		...
		if(( tie_get_option( 'post_tags' ) && ! tie_get_postdata( 'tie_hide_tags' ) ) || tie_get_postdata( 'tie_hide_tags' ) == 'no' ){
			the_tags( '<div class="post-bottom-meta"><div class="post-bottom-meta-title"><span class="fa fa-tags" aria-hidden="true"></span> '. esc_html__( 'Tags', HERBS_TEXTDOMAIN ) .'</div><span class="tagcloud">', ' ', '</span></div>' );
		}
	}
}
...
if( ! function_exists( 'tie_edit_post_button' )){
	...
	add_action( 'Herbs/after_post_content', 'tie_edit_post_button', 40 );
	function tie_edit_post_button(){
		...
		edit_post_link(
			'<span class="fa fa-edit" aria-hidden="true"></span> '. esc_html__( 'Edit Post', HERBS_TEXTDOMAIN ),
...
if( ! function_exists( 'tie_post_share_bottom' )){
	...
	add_action( 'Herbs/after_post_entry', 'tie_post_share_bottom', 20 );
	function tie_post_share_bottom(){

		HERBS_HELPER::get_template_part( 'templates/single-post/share', '', array( 'share_position' => 'bottom' ) );
	}
}
...
if( ! function_exists( 'tie_read_next_posts' )){
	...
	add_action( 'Herbs/post_components', 'tie_read_next_posts', 20 );
	function tie_read_next_posts(){
		HERBS_HELPER::get_template_part( 'templates/single-post/read-next' );
	}
}
...
if( ! function_exists( 'tie_post_about_author' )){
	...
	add_action( 'Herbs/post_components', 'tie_post_about_author', 30 );
	function tie_post_about_author(){
		...
		if( (( tie_get_option( 'post_authorbio' ) && ! tie_get_postdata( 'tie_hide_author' ) ) || tie_get_postdata( 'tie_hide_author' ) == 'no' ) && ! HERBS_HELPER::is_mobile_and_hidden( 'post_authorbio' ) ){
...
if( ! function_exists( 'tie_post_newsletter' )){
	...
	add_action( 'Herbs/post_components', 'tie_post_newsletter', 40 );
	function tie_post_newsletter(){
		...
		if( HERBS_HELPER::is_mobile_and_hidden( 'post_newsletter' ) ){
			return;
		}

		if( ( ( tie_get_option( 'post_newsletter' ) && ! tie_get_postdata( 'tie_hide_newsletter' ) ) || tie_get_postdata( 'tie_hide_newsletter' ) == 'no' ) ){

			HERBS_HELPER::get_template_part( 'templates/single-post/newsletter' );
		}
	}
}
...
if( ! function_exists( 'tie_post_next_prev' )){
	...
	add_action( 'Herbs/post_components', 'tie_post_next_prev', 50 );
	function tie_post_next_prev(){
		...
		if( (( tie_get_option( 'post_nav' ) && ! tie_get_postdata( 'tie_hide_nav' ) ) || tie_get_postdata( 'tie_hide_nav' ) == 'no' ) && ! HERBS_HELPER::is_mobile_and_hidden( 'post_nav' ) ){
...
if( tie_get_option( 'related_position') == 'post' ){
	add_action( 'Herbs/post_components', 'tie_related_posts', 60 );
}
elseif( tie_get_option( 'related_position') == 'comments' ){
	add_action( 'Herbs/post_components', 'tie_related_posts', 80 );
}
elseif( tie_get_option( 'related_position') == 'footer' ){
	add_action( 'Herbs/after_main_content', 'tie_related_posts', 10 );
}
...
if( ! function_exists( 'tie_related_posts' )){
	...
	function tie_related_posts(){
		HERBS_HELPER::get_template_part( 'templates/single-post/related' );
	}
}
...
if( ! function_exists( 'tie_post_comments' )){
	...
	add_action( 'Herbs/post_components', 'tie_post_comments', 70 );
...
if( ! function_exists( 'tie_post_fly_box' )){
	...
	add_action( 'Herbs/after_post_column', 'tie_post_fly_box', 10 );
	function tie_post_fly_box(){
		...
		if( (( tie_get_option( 'check_also' ) && ! tie_get_postdata( 'tie_hide_check_also' )) || ( tie_get_postdata( 'tie_hide_check_also' ) == 'no' )) && ! tie_is_mobile() ){
			...
			HERBS_HELPER::get_template_part( 'templates/single-post/fly-box' );
		}
	}
}
...
if( ! function_exists( 'tie_mobile_share_buttons' )){
	...
	add_action( 'Herbs/after_footer', 'tie_mobile_share_buttons' );
	function tie_mobile_share_buttons(){
		...
		HERBS_HELPER::get_template_part( 'templates/single-post/share', '', array( 'share_position' => 'mobile' ) );
	}
}
...
if( ! function_exists( 'tie_mobile_toggle_content_button' )){
	...
	add_action( 'Herbs/after_post_entry', 'tie_mobile_toggle_content_button' );
	function tie_mobile_toggle_content_button(){

		if( ! HERBS_HELPER::is_supported_post_type() || ! tie_get_option( 'mobile_post_show_more' )){
			return;
		} ?>

		<div class="toggle-post-content clearfix">
			<a id="toggle-post-button" class="button" href="#">
				<?php esc_html_e( 'Show More', HERBS_TEXTDOMAIN ); ?> <span class="fa fa-chevron-down"></span>
			</a>
