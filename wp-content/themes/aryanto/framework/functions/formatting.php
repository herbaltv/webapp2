...
if( ! function_exists( 'tie_body_class' )){
	...
	function tie_body_class( $classes ){
		...
		if( HERBS_HELPER::has_builder() ){
			...
			elseif( is_page() || ( HERBS_BBPRESS_IS_ACTIVE && is_bbpress() ) || is_singular() ){
		...
		if( is_page() ){
			...
			if( tie_get_postdata( 'tie_hide_header' ) ){
				...
				add_filter('Herbs/is_header_active', '__return_false');
			}
			...
			if( tie_get_postdata( 'tie_hide_footer' )){
				...
				add_filter('Herbs/is_footer_active', '__return_false');
			}
		}
...
if( ! function_exists( 'tie_get_sidebar_position' )){
	...
	function tie_get_sidebar_position(){
		...
		if( HERBS_WOOCOMMERCE_IS_ACTIVE && is_product() && tie_get_option( 'woo_product_sidebar_pos' )){
			$sidebar_position = tie_get_option( 'woo_product_sidebar_pos' );
		}

		// WooCommerce sidebar position
		elseif( HERBS_WOOCOMMERCE_IS_ACTIVE && is_woocommerce() && tie_get_option( 'woo_sidebar_pos' )){
			$sidebar_position = tie_get_option( 'woo_sidebar_pos' );
		}

		// buddyPress Sidebar Settings
		elseif( HERBS_BUDDYPRESS_IS_ACTIVE && is_buddypress() ){
			$sidebar_position = HERBS_BUDDYPRESS::get_page_data( 'tie_sidebar_pos' );
		}

		// bbPress Sidebar Settings
		elseif( HERBS_BBPRESS_IS_ACTIVE && is_bbpress() ){
...
if( ! function_exists( 'tie_get_post_class' ) ){
	...
	function tie_get_post_class( $classes = false, $post_id = null, $standard = false ){
		...
		$classes = apply_filters( 'Herbs/post_classes', $classes );
...
if( ! function_exists( 'tie_comment_form_before' )){
	...
	function tie_comment_form_before(){
		...
		if( HERBS_WOOCOMMERCE_IS_ACTIVE && is_woocommerce() ){
			return;
		}
...
if( ! function_exists( 'tie_comment_form_after' )){
	...
	function tie_comment_form_after(){
		...
		//|| ( HERBS_JETPACK_IS_ACTIVE && Jetpack::is_active() && in_array( 'comments', Jetpack::get_active_modules() ) ) ){
...
if( ! function_exists( 'tie_content_column_attr' )){
	...
	function tie_content_column_attr( $echo = true ){
		...
		$attr = apply_filters( 'Herbs/content_column_attr', 'class="main-content '. $columns_classes .'" role="main"' );
...
if( ! function_exists( 'tie_before_main_content' )){
	...
	add_action( 'Herbs/before_main_content', 'tie_before_main_content' );
	function tie_before_main_content(){

		if( ( HERBS_BUDDYPRESS_IS_ACTIVE && is_buddypress() ) || ( HERBS_HELPER::has_builder() && ! post_password_required() )){
			return;
		}
...
if( ! function_exists( 'tie_after_main_content' )){
	...
	add_action( 'Herbs/after_main_content', 'tie_after_main_content' );
