 ...
 */
define( 'HERBS_SMALL_IMAGE',  HERBS_THEME_SLUG.'-image-small' );
...
if( ! function_exists( 'tie_post_thumbnail' )){
	...
	function tie_post_thumbnail( $thumb = HERBS_SMALL_IMAGE, $review = 'small', $cat = false, $trending = true, $media_icon = false ){
		...
		echo '
			...
			if( $review == 'large' ){
				...
				$hide_category_meta = apply_filters( 'Herbs/Archive_Thumbnail/category_meta', $hide_category_meta );
			...
			if( ! empty( $thumb ) ){
				the_post_thumbnail( $thumb, array( 'alt' => sprintf( esc_html__( 'Photo of %s', HERBS_TEXTDOMAIN ), the_title_attribute( 'echo=0' ) )));
			}
...
if( ! function_exists( 'tie_lazyload_placeholder' )){
	...
	function tie_lazyload_placeholder( $size = '' ){
		...
		// Image Data
		if( apply_filters( 'Herbs/images/use_data', true ) ){
		...
		// Normal Images
			...
			return HERBS_TEMPLATE_URL.'/assets/images/tie-empty'.$size.'.png';
		}
...
if( ! function_exists( 'tie_thumb_src' )){
	...
	function tie_thumb_src( $size = HERBS_SMALL_IMAGE ){
...
if( ! function_exists( 'tie_thumb_src_bg' )){
	...
	function tie_thumb_src_bg( $size = HERBS_SMALL_IMAGE ){
...
if( ! function_exists( 'tie_google_maps' )){
	...
	function tie_google_maps( $url ){
		...
		elseif( tie_get_option( 'api_google_maps' ) ){
			...
			$api_key  = HERBS_HELPER::remove_spaces( tie_get_option( 'api_google_maps' ) );
			...
			return HERBS_HELPER::notice_message( esc_html__( 'You need to set the Google Map API Key in the theme options page > API Keys.', HERBS_TEXTDOMAIN ) );
		}
