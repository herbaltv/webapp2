	...
	function tie_get_time( $return = false ){
		...
		if( $time_format == 'modern' ){
			...
			if ( $post_time > ( $time_now - MONTH_IN_SECONDS ) ){
				$since = sprintf( esc_html__( '%s ago', HERBS_TEXTDOMAIN ), human_time_diff( $post_time, $time_now ) );
			}
			else {
		...
		$since = apply_filters( 'Herbs/post_date', $since );
...
if( ! function_exists( 'tie_get_score' )){
	...
	function tie_get_score( $size = 'small' ){
		...
		if( ! HERBS_TAQYEEM_IS_ACTIVE || ! tie_get_postdata( 'taq_review_position' ) ){
			return;
		}
...
if( ! function_exists( 'tie_get_primary_category' )){
	...
	function tie_get_primary_category(){
		...
		if( ! empty( $category ) && HERBS_WP_HELPER::term_exists( $category, 'category' ) ){
			$get_the_category = HERBS_WP_HELPER::get_term_by( 'id', $category, 'category' );
		...
		if( ! empty( $categories ) && is_array( $categories )){
			foreach ( $categories as $category ){
				...
				if( $plain ){
					...
					$output .= '<a class="post-cat tie-cat-'.$category->term_id.'" href="' . HERBS_WP_HELPER::get_term_link( $category->term_id, $taxonomy_slug ) . '">' . $category->name.'</a>';
				}
			}
		}
 ...
 * Get the Post Category HTML
 */
if( ! function_exists( 'tie_get_category_herbal' )){

	function tie_get_category_herbal( $before = '<span class="post-cat-wrap">', $after = '</span>', $primary = true, $plain = false ){
		...
		if( ! empty( $categories ) && is_array( $categories )){
			foreach ( $categories as $category ){
				...
				if( $plain ){
					$output .= '<span class="post-cat-herbal herbal-tie-cat-'.$category->term_id.'">' . $category->name.'</span>';
				}
				else{
					$output .= '<a class="post-cat-herbal herbal-tie-cat-'.$category->term_id.'" href="' . HERBS_WP_HELPER::get_term_link( $category->term_id, $taxonomy_slug ) . '">' . $category->name.'</a>';
				}
			}
		}
 ...
 * Print the post category HTML
 */
if( ! function_exists( 'tie_the_category_herbal' )){

	function tie_the_category_herbal( $before = false, $after = false, $primary = true, $plain = false ){
		echo tie_get_category_herbal( $before, $after, $primary, $plain );
	}
}
...
if( ! function_exists( 'tie_get_title' )){
	...
	function tie_get_title( $limit = false, $trim_type = false ){
		...
		if( $title == '' ){
			 $title = esc_html__( '(no title)', HERBS_TEXTDOMAIN );
		}
...
if( ! function_exists( 'tie_get_post_meta' )){
	...
	function tie_get_post_meta( $args = '', $before = false, $after = false ){
		...
		$disable_on = apply_filters( 'Herbs/post_meta/disable_post_types', array(
			'page',
			'product'
		));
		...
		$args = apply_filters( 'Herbs/post_meta/args', $args );
		...
		if( ! empty( $comments ) || ! empty( $views ) || ! empty( $reading ) ){
			...
			$post_meta .= apply_filters( 'Herbs/post_meta_before_extra_info', '<div class="tie-alignright">' );
			...
			// Number of views
			if( ! empty( $views ) ){
				$post_meta .= HERBS_POSTVIEWS::get_views();
			}
			...
			$post_meta .= apply_filters( 'Herbs/post_meta_after_extra_info', '</div>' );
		}
	...
	if ( is_array( $post_authors ) && ! empty( $post_authors ) ) {
		...
		foreach ( $post_authors as $author ) {
			...
			if( $show_avatars ){
				$post_meta .= '
					<span class="meta-author-avatar">
						<a href="'. $profile .'">'. get_avatar( $author->user_email, 140, '', sprintf( esc_html__( 'Photo of %s', HERBS_TEXTDOMAIN ), $display_name ) ) .'</a>
					</span>
				';
			}
			...
			if( ! empty( $args['twitter'] ) && ! empty( $author_twitter )){
				$post_meta .= '
					<a href="'. esc_url( $author_twitter ) .'" class="author-twitter-link" target="_blank" rel="nofollow noopener" title="'. esc_html__( 'Follow on Twitter', HERBS_TEXTDOMAIN ) .'">
						<span class="fa fa-twitter" aria-hidden="true"></span>
						<span class="screen-reader-text">'. esc_html__( 'Follow on Twitter', HERBS_TEXTDOMAIN ) .'</span>
					</a>
				';
			}
			...
			if( ! empty( $args['email'] ) && ! empty( $author->user_email ) ){
				$post_meta .= '
					<a href="mailto:'. $author->user_email .'" class="author-email-link" target="_blank" rel="nofollow noopener" title="'. esc_html__( 'Send an email', HERBS_TEXTDOMAIN ) .'">
						<span class="fa fa-envelope" aria-hidden="true"></span>
						<span class="screen-reader-text">'. esc_html__( 'Send an email', HERBS_TEXTDOMAIN ) .'</span>
					</a>
				';
			}
			...
			if( is_singular( 'post' ) && ! $show_avatars ){
				if( $authors_count == $authors_number - 1 ){
					$post_meta .= '<span class="co-plus-and-sep meta-item">'. esc_html__( 'and', HERBS_TEXTDOMAIN ) .'</span>';
				}
			}
		}
...
if( ! function_exists( 'tie_adjacent_post' )){
	...
	function tie_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ){
		...
		if( ! empty( $adjacent_post ) ){
			...
			$image_data = wp_get_attachment_image_src( $image_id, HERBS_THEME_SLUG.'-image-large' );
...
if( ! function_exists( 'tie_reading_time' )){
	...
	function tie_reading_time(){
		...
		$post_content  = HERBS_HELPER::strip_shortcodes( $post_content );
		...
		$words_per_min = apply_filters( 'Herbs/words_per_min', 250 );

		$reading_time = floor( $words_count / $words_per_min );

		if( $reading_time < 1){
			$result = esc_html__( 'Less than a minute', HERBS_TEXTDOMAIN );
		}
		elseif( $reading_time > 60 ){
			$result = sprintf( esc_html__( '%s hours read', HERBS_TEXTDOMAIN ), number_format_i18n( floor( $reading_time / 60 ) ) );
		}
		else if ( $reading_time == 1 ){
			$result = esc_html__( '1 minute read', HERBS_TEXTDOMAIN );
		}
		else {
			$result = sprintf( esc_html__( '%s minutes read', HERBS_TEXTDOMAIN ), number_format_i18n( $reading_time ) );
		}

		$result = apply_filters( 'Herbs/reading_time', $result, $reading_time, $words_per_min, $words_count );
...
if( ! function_exists( 'tie_get_more_button' )){
	...
	function tie_get_more_button(){
		...
		if( HERBS_HELPER::is_mobile_and_hidden( 'read_more_buttons' ) ){
			return;
		}

		return apply_filters( 'Herbs/more_link_button', '<a class="more-link button" href="' . get_permalink() . '">'. esc_html__( 'Read More &raquo;', HERBS_TEXTDOMAIN ) .'</a>' );
	}
}
