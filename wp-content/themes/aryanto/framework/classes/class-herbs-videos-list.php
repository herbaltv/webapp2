...
if( ! class_exists( 'HERBS_VIDEOS_LIST' )){

	class HERBS_VIDEOS_LIST {
		...
		function __construct() {
			...
			add_filter( 'Herbs/save_block', array( $this, 'save_block' ) );

			// Save Videos list category
			add_filter( 'Herbs/save_category', array( $this, 'save_category' ) );
		}
		...
		public static function get_video_info( $videos_list ){
			...
			foreach ( $videos_list as $video ){
				...
				if( preg_match( "#(?<=v=)[a-zA-Z0-9-]+(?=&)|(?<=v\/)[^&\n]+(?=\?)|(?<=v=)[^&\n]+|(?<=youtu.be/)[^&\n]+#", $video, $matches )){
					...
					$video_id = HERBS_HELPER::remove_spaces( $matches[0] );
				...
				elseif( preg_match( "/(https?:\/\/)?(www\.)?(player\.)?vimeo\.com\/([a-z]*\/)*([0-9]{6,11})[?]?.*/", $video, $matches )){
					...
					$video_id = HERBS_HELPER::remove_spaces( $matches[5] );
		...
		private static function get_youtube_info( $vid ){
			...
			$params = array(
				...
				'id'   => $vid,
				'key'  => HERBS_HELPER::remove_spaces( tie_get_option( 'api_youtube' ) ),
			);
