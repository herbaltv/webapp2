...
if( ! class_exists( 'HERBS_STYLES' )){
	...
	class HERBS_STYLES {
		...
		public static function init(){
			...
			add_action( 'Herbs/Options/updated',   array( __CLASS__, 'update_styles_file' ) );
			add_action( 'Herbs/after_db_update',   array( __CLASS__, 'update_styles_file' ) );
		...
		public static function localize_script(){
			...
			$js_vars = array(
				...
				'is_taqyeem_active'      => HERBS_TAQYEEM_IS_ACTIVE,
				...
				'lang_no_results'        => esc_html__( 'Nothing Found', HERBS_TEXTDOMAIN ),
			);

			wp_localize_script( 'tie-scripts', 'tie', apply_filters( 'Herbs/js_main_vars', $js_vars ) );
		}
		...
		public static function fonts_css(){
			...
			$out = '';
			$fonts_sections  = apply_filters( 'Herbs/fonts_sections_array', '' );
		...
		public static function typography_css(){
			...
			$text_sections = apply_filters( 'Herbs/typography_elements', array() );
	  ...
	  */
		public static function insert_google_fonts_head(){
			...
			echo '<script>';
			echo apply_filters( 'Herbs/google_fonts/js_code', $js_code, $GLOBALS['tie_google_fonts'] );
			echo '</script>';
		}
	  ...
	  */
		...
		public static function is_inline_css(){
			return apply_filters( 'Herbs/Styles/is_inline_css', tie_get_option( 'inline_css' ) );
		}
		...
		public static function style_file_path( $absolute = false ){
			...
			$path = $absolute ? HERBS_TEMPLATE_PATH : HERBS_TEMPLATE_URL;
			$path = $path . '/assets/custom-css';

			$path = apply_filters( 'Herbs/CSS/custom_file_path', $path, $absolute );
		...
		public static function get_background( $is_custom_css_file = false ){
			...
			if( HERBS_BUDDYPRESS_IS_ACTIVE && is_buddypress() && HERBS_BUDDYPRESS::get_page_data( 'tie_theme_layout' ) ){
				$theme_layout = HERBS_BUDDYPRESS::get_page_data( 'tie_theme_layout' );
			}
			...
			if( $theme_layout != 'border' ){
				...
				elseif( $background_type == 'pattern' ){
					...
					$background_code   .= ! empty( $background_pattern ) ? 'background-image: url('.  HERBS_TEMPLATE_URL .'/assets/images/patterns/' .$background_pattern .'.png);' : '';
				}
			}
			...
			if( ! empty( $background_dots )){
				$background_overlay .= ! empty( $background_dots ) ? 'background-image: url('.  HERBS_TEMPLATE_URL .'/assets/images/bg-dots.png);' : '';
			}
		...
		public static function enqueue_external_css(){
			...
			wp_enqueue_style( 'tie-css-style-custom', self::style_file_path(), array(), get_option( 'style-custom-ver', HERBS_DB_VERSION ), 'all' );
		}
		...
		public static function get_custom_styles(){
			...
			$inline_css_code = apply_filters( 'Herbs/Styles/inline_css_code', self::minify_css( $inline_css_code ) );
		...
		public static function get_all_custom_css(){
			...
			$elements_styling .= apply_filters( 'Herbs/Styles/custom_theme_color', self::custom_theme_color() );

			// Early Filter after getting the custom theme color
			$elements_styling .= apply_filters( 'Herbs/CSS/after_theme_color', $elements_styling );
			...
			$out = apply_filters( 'Herbs/CSS/after_all_styles', $out );
			...
			if( empty( $theme_color ) ){
				$theme_color = apply_filters( 'Herbs/default_theme_color', '#000' );
			}

			$out = str_replace( 'primary-color', $theme_color, $out );

			// Prepare the CSS codes
			return apply_filters( 'Herbs/CSS/output', $out );
		}
		...
		public static function custom_theme_color( $color = false ){
			...
			return apply_filters( 'Herbs/CSS/custom_theme_color', $css_code, $color, $dark_color, $bright, $rgb_color );
		}
		...
		public static function builder_section_style( $section ){
			...
			return apply_filters( 'Herbs/CSS/Builder/section_style', $section_css, $section_id, $section_settings );
		}
		...
		public static function builder_block_style( $block ){
			...
			return apply_filters( 'Herbs/CSS/Builder/block_style', '', $id_css, $block, $color, $bright, $darker );
		}
		...
		public static function meta_viewport(){
			...
			if( tie_get_option( 'disable_responsive' )){
				echo '<meta name="viewport" content="width='. apply_filters( 'Herbs/non_responsive_viewport', '1200' ) .'" />';
			}
			else{
