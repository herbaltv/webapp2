...
class HERBS_FILTERS {
	...
	function __construct() {
		...
		add_filter( 'Herbs/cache_key',     array( $this, 'cache_key' ) );
		...
		add_filter( 'Herbs/exclude_content',            'HERBS_HELPER::strip_shortcodes' );
	...
	function shortcodes_notice( $content ){
		...
		if( ! HERBS_MPTIMETABLE_IS_ACTIVE ){
			...
			if( strpos( $content, '[mp-timetable' ) !== false ){
				...
				$message .= HERBS_HELPER::notice_message( sprintf(
					esc_html__( 'This section contains some shortcodes that requries the %2$s%1$s%3$s Plugin.', HERBS_TEXTDOMAIN ),
		...
		return apply_filters( 'Herbs/shortcodes_check', $message, $content ) . $content;
	}
	...
	function meta_description(){
		...
		if( is_singular() && ! HERBS_HELPER::has_builder() ){
			...
			if( ! empty( $post->post_content ) ){
				$description = apply_filters( 'Herbs/exclude_content', $post->post_content );
			}
		}
		// Archives
	...
	function theme_translation( $translation, $text, $domain ){
		...
		if( $domain == HERBS_TEXTDOMAIN || $domain == 'amp' ){
	...
	function footer_inline_scripts(){
		...
		if( ! empty( $GLOBALS['tie_inline_scripts'] ) && HERBS_HELPER::is_js_minified() ){
	...
	function popup_module(){
		...
		HERBS_HELPER::get_template_part( 'templates/popup' );
	}
	...
	function small_thumb_image_class( $attr, $attachment, $size = false ) {
		...
		if( ! empty( $size ) && $size == HERBS_THEME_SLUG.'-image-small' ){
	...
	function post_excerpt( $text = '' ){
		...
		if ( '' == $text ){
			...
			$text = apply_filters( 'Herbs/exclude_content', $text );
	...
	function cache_key( $key ){
		return 'tie-cache-'. HERBS_HELPER::get_locale() . $key;
	}
...
// Single instance.
$HERBS_FILTERS = new HERBS_FILTERS();
