...
if( ! class_exists( 'HERBS_FOXPUSH' )){
	...
	class HERBS_FOXPUSH{
		...
		function get_embed_code(){
			...
			if( ! tie_get_option( 'web_notifications' ) || HERBS_HELPER::is_bot() ){
        ...
        (function(){
			...
			HERBS_HELPER::inline_script( 'tie-scripts', apply_filters( 'Herbs/foxpush_embedcode', $code ) );
		}
		...
		function post_meta_boxes(){
			...
			if( tie_get_option( 'web_notifications' ) && tie_get_option( 'foxpush_domain' ) && tie_get_option( 'foxpush_api' ) ){
				...
				if( 'publish' === get_post_status( get_the_ID() ) ){
					...
					if( $get_campaigns && is_array( $get_campaigns ) ){
						...
						add_meta_box(
							...
							esc_html__( 'Web Notification Campaigns', HERBS_TEXTDOMAIN ),
				...
				add_meta_box(
					...
					esc_html__( 'Send a Web Notification', HERBS_TEXTDOMAIN ),
		...
		function get_meta_box_content( $post_id = false, $meta_box_atts = false ){
			...
			if( $get_campaigns && is_array( $get_campaigns ) ){
				...
				if( $meta_box_atts ){
					echo '<div class="campaigns-statistics">'. esc_html__( 'Statistics (Updated Hourly)', HERBS_TEXTDOMAIN ) .' <a href="#" id="update-campaign-status" class="button">'. esc_html__( 'Update', HERBS_TEXTDOMAIN ) .'</a></div>';
				}
				...
				foreach ( $get_campaigns as $campaign_id => $campaign_data ) {
					...
					echo '
						...
						<table class="wp-list-table widefat striped">
							...
							<tbody>
								<tr>
									<td colspan="3">'. esc_html__( 'Date', HERBS_TEXTDOMAIN ) .': '. $campaign_data['created_time'] .'</td>
								</tr>
								<tr>
									<td>'. esc_html__( 'Sent', HERBS_TEXTDOMAIN ) .'</td>
									<td>'. esc_html__( 'Views', HERBS_TEXTDOMAIN ) .'</td>
									<td>'. esc_html__( 'Clicks', HERBS_TEXTDOMAIN ) .'</td>
								</tr>
								<tr>
		...
		function send_meta_box_content(){
			...
			if( ! get_option( 'tie_token_'.HERBS_THEME_ID ) ){
				...
				tie_build_theme_option(
					array(
						'text' => esc_html__( 'You need to validated your license to use this feature.', HERBS_TEXTDOMAIN ),
						'type' => 'error',
					));
			...
			elseif( 'publish' !== get_post_status( get_the_ID() ) ){
				...
				tie_build_theme_option(
					array(
						'text' => esc_html__( 'You need to publish the post first.', HERBS_TEXTDOMAIN ),
						'type' => 'message',
					));
			...
			echo '<div id="send-notification-options">';
				...
				tie_build_option(
					array(
						'placeholder' => esc_html__( 'Title', HERBS_TEXTDOMAIN ),
				...
				tie_build_option(
					array(
						'placeholder' => esc_html__( 'Message', HERBS_TEXTDOMAIN ),
						'id'          => 'tie_foxpush_msg',
						'type'        => 'text',
					),
					'tie_foxpush_msg',
				...
				tie_build_option(
					array(
						'custom_text' => esc_html__( 'Upload Icon', HERBS_TEXTDOMAIN ),
						'hint'        => sprintf( esc_html__( 'Recommended size is %1spx x %2spx', HERBS_TEXTDOMAIN ), 250, 250 ),
						'id'          => 'tie_foxpush_icon',
						'type'        => 'upload',
					),
					'tie_foxpush_icon',
				...
				echo '
					...
					<div id="send-notification-actions">
						<a id="send-notification" class="button button-primary button-large">'. esc_html__( 'Send', HERBS_TEXTDOMAIN ) .'</a>
					</div>
				';
		...
		function get_statistics( $type = 'chart' ){
			...
			if( empty( $data )){
				...
				$args = array(
					'headers'     => array(
						'FOXPUSH_DOMAIN' => HERBS_HELPER::remove_spaces( $foxpush_domain ),
						'FOXPUSH_TOKEN'  => HERBS_HELPER::remove_spaces( $foxpush_apikey ),
					)
				);
		...
		function create_campaign(){
			...
			if( ! $domain || ! $api_key || empty( $_REQUEST['title'] ) || empty( $_REQUEST['message'] ) || empty( $_REQUEST['id'] ) ){
				...
				tie_build_theme_option(
					array(
						'text' => esc_html__( 'Requried data Missing', HERBS_TEXTDOMAIN ),
						'type' => 'error',
					));
			...
			if( ! empty( $request['code'] ) ){
				if( $request['code'] == '411' && $request['error_message'] == 'hotlink_image' ){
					...
					tie_build_theme_option(
						array(
							'text' => esc_html__( 'Can not access the image from your server, send the Campaign from your account on FoxPush.com', HERBS_TEXTDOMAIN ),
							'type' => 'error',
						));
				...
				elseif( $request['code'] == '200' && ! empty( $request['campaign_id'] ) ){
					...
					tie_build_theme_option(
						array(
							'text' => esc_html__( 'Campaign has been sent', HERBS_TEXTDOMAIN ),
							'type' => 'success',
						));
