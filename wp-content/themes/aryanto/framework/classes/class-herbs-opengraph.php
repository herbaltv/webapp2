...
if( ! class_exists( 'HERBS_OPENGRAPH' )){

	class HERBS_OPENGRAPH {
		...
		function insert_opengraph(){
			...
			$og_description = apply_filters( 'Herbs/exclude_content', $post->post_content );
...
<meta property="og:site_name" content="'. get_bloginfo( 'name' ) .'" />
			...
			if ( has_post_thumbnail() || tie_get_option( 'post_og_cards_image' ) ){
				...
				$image = has_post_thumbnail() ? tie_thumb_src( HERBS_THEME_SLUG.'-image-post' ) : tie_get_option( 'post_og_cards_image' );
		...
		public static function is_active(){
			...
			return apply_filters( 'Herbs/is_opengraph_active', $is_active );
		}

	}

	// Single instance
	$HERBS_OPENGRAPH = new HERBS_OPENGRAPH();
}
