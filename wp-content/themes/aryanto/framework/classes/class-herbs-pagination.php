...
if( ! class_exists( 'HERBS_PAGINATION' )){

	class HERBS_PAGINATION {
			...
			if( $type == 'numeric' ){
				...
				if( $type == 'load-more' || $type == 'infinite'  ){
					...
					if( $wp_query->max_num_pages > $paged ){
						...
						// --
						echo '<a data-url="'. get_pagenum_link( 99999 ) .'" data-text="'. esc_html__( 'Load More', HERBS_TEXTDOMAIN ) .'" data-query="'. $query .'" data-max="'. $wp_query->max_num_pages .'" data-page="'. $paged .'" data-latest="'. $latest_post .'" id="load-more-archives" class="container-wrapper show-more-button load-more-button '. $class .'">'. esc_html__( 'Load More', HERBS_TEXTDOMAIN ) .'</a>';
					}
				}
				...
				// Next / Prev Navigation
					...
					<div class="pages-numbers pages-standard">
						<span class="first-page first-last-pages">
							<?php previous_posts_link( '<span class="fa" aria-hidden="true"></span>'. esc_html__( 'Previous page', HERBS_TEXTDOMAIN ) ); ?>
						</span>

						<span class="last-page first-last-pages">
							<?php next_posts_link( '<span class="fa" aria-hidden="true"></span>'. esc_html__( 'Next page', HERBS_TEXTDOMAIN ) ); ?>
						</span>
					</div>
					<?php
				}
		...
		private static function numeric( $query = false, $before = '', $after = '' ){
			...
			$pagenavi_options = array(
				// 'pages_text' => esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', HERBS_TEXTDOMAIN ),
				'current_text'  => '%PAGE_NUMBER%',
				'page_text'		  => '%PAGE_NUMBER%',
				'first_text' 	  => esc_html__( 'First', HERBS_TEXTDOMAIN ),
				'last_text'		  => esc_html__( 'Last', HERBS_TEXTDOMAIN ),
				'next_text'		  => esc_html__( '&raquo;', HERBS_TEXTDOMAIN ),
				'prev_text'		  => esc_html__( '&laquo;', HERBS_TEXTDOMAIN ),
				'dotright_text' => '...',
				'dotleft_text'  => '...',
				'larger_page_numbers_multiple' => 10,
			);

			$pagenavi_options = apply_filters( 'Herbs/numeric_pagination_args', $pagenavi_options );
