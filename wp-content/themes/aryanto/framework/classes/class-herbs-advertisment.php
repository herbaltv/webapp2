...
/**
 * HERBS_ADVERTISMENT class with filter hooks
 */
if( ! class_exists( 'HERBS_ADVERTISMENT' )){

	class HERBS_ADVERTISMENT {
		...
		function __construct() {
			...
			// Background Ad
			add_action( 'Herbs/before_wrapper', array( $this, 'background_ad' ) );

			// Before Header
			add_action( 'Herbs/before_header', array( $this, 'before_header' ) );

			// Before Header
			add_action( 'Herbs/Logo/after_wrapper', array( $this, 'beside_logo' ) );

			// After Header
			add_action( 'Herbs/after_header', array( $this, 'after_header' ) );

			// Before Footer
			add_action( 'Herbs/before_footer', array( $this, 'before_footer' ) );

			// Before Post Ad
			add_filter( 'Herbs/before_the_article', array( $this, 'before_post' ), 5 );

			// After Post Ad
			add_filter( 'Herbs/before_post_components', array( $this, 'after_post' ), 5 );

			// Before Post Content Ad
			add_filter( 'Herbs/before_post_content', array( $this, 'before_post_content' ), 10 );

			// After Post Content Ad
			add_filter( 'Herbs/after_post_content', array( $this, 'after_post_content' ), 5 );

			// After X posts in Archives
			add_action( 'Herbs/after_post_in_archives', array( $this, 'ad_after_x_posts' ), 10, 2 );

			// Below Category Slider
			add_action( 'Herbs/Category/after_slider', array( $this, 'after_category_slider' ) );

			// Above Category Title
			add_action( 'Herbs/before_archive_title', array( $this, 'before_category_title' ) );

			// Below Category Title
			add_action( 'Herbs/after_archive_title', array( $this, 'after_category_title' ) );

			// Below Category Posts
			add_action( 'Herbs/after_archive_posts', array( $this, 'after_category_posts' ) );

			// Below Category Pagination
			add_action( 'Herbs/after_archive_pagination', array( $this, 'after_category_pagination' ) );
		...
		function get_ad( $banner, $before = false, $after = false, $echo = true ){
			...
			if( ! tie_get_option( $banner ) || HERBS_HELPER::is_mobile_and_hidden( $banner ) ){
				return;
			}
			...
			elseif( $code = tie_get_option( $banner.'_adsense' ) ){
				...
				$the_ad .= do_shortcode( apply_filters( 'Herbs/custom_ad_code', $code ) );
			}
			...
			elseif( $img = tie_get_option( $banner.'_img' ) ){
				...
				$url      = apply_filters( 'Herbs/ads_url', tie_get_option( $banner.'_url' ) ? tie_get_option( $banner.'_url' ) : '' );
				...
				<div id="tie-popup-adblock" class="tie-popup is-fixed-popup">
					<div class="tie-popup-container">
						<div class="container-wrapper">
							...
							<h2><?php echo tie_get_option( 'adblock_title', esc_html__( 'Adblock Detected', HERBS_TEXTDOMAIN ) ); ?></h2>

							<div class="adblock-message">
								<?php echo tie_get_option( 'adblock_message', esc_html__( 'Please consider supporting us by disabling your ad blocker', HERBS_TEXTDOMAIN ) ); ?>
							</div>

						</div><!-- .container-wrapper  /-->
					</div><!-- .tie-popup-container /-->
				</div><!-- .tie-popup /-->
				<script type='text/javascript' src='<?php echo HERBS_TEMPLATE_URL ?>/assets/js/advertisement.js'></script>
			<?php
		}
	...
	// Single instance.
	$HERBS_ADVERTISMENT = new HERBS_ADVERTISMENT();
}
