...
if( ! class_exists( 'HERBS_INSTAGRAM' )){

	class HERBS_INSTAGRAM {
		...
		function __construct( $atts ) {
			...
			if( empty( $atts['username'] ) || empty( $atts['userid'] ) ){
				return HERBS_HELPER::notice_message( esc_html__( 'You need to set the Username and the UserID.', HERBS_TEXTDOMAIN ) );
			}
			...
			if( ! empty( $user_data['error'] ) ){
				return HERBS_HELPER::notice_message( $user_data['error'] );
			}
				...
				<?php
					if( $posts ){ ?>
						<li>
							...
							<span><?php esc_html_e( 'Posts', HERBS_TEXTDOMAIN ) ?></span>
						</li>
						<?php
					}
					...
					if( $followed ){ ?>
						<li>
							...
							<span><?php esc_html_e( 'Followers', HERBS_TEXTDOMAIN ) ?></span>
						</li>
						<?php
					}
					...
					if( $follow ){ ?>
						<li>
							...
							<span><?php esc_html_e( 'Following', HERBS_TEXTDOMAIN ) ?></span>
						</li>
						<?php
					}
				?>
		...
		private function get_by_username(){
			...
			if( empty( $request ) || is_wp_error( $request ) ){
				return array( 'error' => esc_html__( 'Can not connect to Instagram!', HERBS_TEXTDOMAIN ) );
			}
			...
			if ( empty( $matches[1] ) ){
				return array( 'error' => esc_html__( 'Can not fetch the images!', HERBS_TEXTDOMAIN ) );
			}
			else{
				...
				if( $instagram_data === null && json_last_error() !== JSON_ERROR_NONE ){
					return array( 'error' => esc_html__( 'Can not decoding the instagram json', HERBS_TEXTDOMAIN ) );
				}
				...
				if( ! empty( $instagram_data['entry_data']['LoginAndSignupPage'] ) ){
					...
					//return array( 'error' => esc_html__( 'Instagram redirected to login and signup page. Rate limiting occured. Try to use other ip or try crawling with authentication.', HERBS_TEXTDOMAIN ) );
				}

				// Check if the images set is available
				if( empty( $instagram_data['entry_data']['ProfilePage'][0]['graphql']['user'] ) ){
					return array( 'error' => esc_html__( 'Can not find the user!', HERBS_TEXTDOMAIN ) );
				}
