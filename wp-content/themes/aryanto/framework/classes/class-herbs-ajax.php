...
if( ! class_exists( 'HERBS_AJAX' )){
	...
	class HERBS_AJAX {
		...
		function block_load_more(){
			...
			if( $block['style'] == 'woocommerce' && HERBS_WOOCOMMERCE_IS_ACTIVE ){
			...
			if( $block_query->have_posts() ){
				while ( $block_query->have_posts() ){
					...
					if( $block['style'] == 'woocommerce' && HERBS_WOOCOMMERCE_IS_ACTIVE ){
						...
						HERBS_HELPER::get_template_part( 'templates/loops/loop', $style, array( 'block' => $block, 'count' => $count ) );
					}
				}
				...
				wp_send_json( wp_json_encode(
					array(
						...
						'hide_prev' => $hide_prev,
						'code'      => ob_get_clean(),
						'button'    => esc_html__( 'No More Posts', HERBS_TEXTDOMAIN ),
					)));
			}
			else{
				wp_send_json( wp_json_encode(
					array(
						'hide_next' => true,
						'hide_prev' => $hide_prev,
						'code'      => esc_html__( 'No More Posts', HERBS_TEXTDOMAIN ),
						'button'    => esc_html__( 'No More Posts', HERBS_TEXTDOMAIN ),
					)));
			}
		...
		function archive_load_more(){
			...
			if( ! empty( $settings['is_category'] ) ){
				add_filter( 'Herbs/Archive_Thumbnail/category_meta', '__return_false' );
			}
			...
			if( $block_query->have_posts() ){
				while ( $block_query->have_posts() ){
					...
					HERBS_HELPER::get_template_part( 'templates/loops/loop', $layout, array( 'block' => $settings ) );

					do_action( 'Herbs/after_post_in_archives', $layout, $latest_post );
				}
				...
				wp_send_json( wp_json_encode(
					array(
						'hide_next' => true,
						'code'      => '<li>'. esc_html__( 'No More Posts', HERBS_TEXTDOMAIN ) .'</li>',
						'button'    => esc_html__( 'No More Posts', HERBS_TEXTDOMAIN ),
					)));
			}
		...
		function mega_menu(){
			...
			$block = apply_filters( 'Herbs/mega_menu/posts_query/args', $block );

			$count      = 0;
			$is_featurd = false;
			$thumbnail  = HERBS_THEME_SLUG.'-image-large';
			...
			// Cache key
			$cache_key = apply_filters( 'Herbs/cache_key', '' ) . '_mega_'.implode( '_', $block ) .'_'. (int) $is_featurd .'_'. $thumbnail;
				...
				if( $block_query->have_posts() ){
					while ( $block_query->have_posts() ){
						...
						if( $is_featurd && $count == 1 ){
							HERBS_HELPER::get_template_part( 'templates/loops/loop', 'mega-menu-featured', array( 'media_icon' => $media_icon ) );
							echo " <div class=\"mega-check-also\">\n<ul>";
						}
						else{
							HERBS_HELPER::get_template_part( 'templates/loops/loop', 'mega-menu-default', array( 'thumbnail' => $thumbnail, 'media_icon' => $media_icon ) );
						}
					}
					...
					echo '<div class="ajax-no-more-posts">'. esc_html__( 'Nothing Found', HERBS_TEXTDOMAIN ) .'</div>';
				}
		...
		function live_search(){
			...
			$supported_post_types = apply_filters( 'Herbs/live_search/post_types', $supported_post_types );
			...
			if( $block_query->have_posts() ){
				...
				while ( $block_query->have_posts() ){
					...
					HERBS_HELPER::get_template_part( 'templates/loops/loop', 'live-search' );
				...
				$search_json['suggestions'][] = array(
					'layout'   => '<div class="widget-post-list"><a class="button fullwidth" href="'. esc_url(home_url('?s=' . urlencode( $search_qry ) )) .'">'. esc_html__( 'View all results', HERBS_TEXTDOMAIN ) .'</a></div>',
					'value'    => $search_qry,
				...
				//echo '<div>'.esc_html__( 'Nothing Found', HERBS_TEXTDOMAIN ).'</div>';
			}
