...
if( ! class_exists( 'HERBS_POSTVIEWS' )){

	class HERBS_POSTVIEWS{
		...
		function __construct(){
			...
			add_filter( 'Herbs/post_options_meta',         array( $this, 'save_custom_views' ) );
			add_filter( 'Herbs/views_meta_field',          array( $this, 'custom_views_meta_field' ) );
		...
		function set_post_views(){
			...
			// Disable via filter
			if( ! apply_filters( 'Herbs/Post_Views/increment', true ) ){
				return;
			}

			// Run only if the post views option is set to THEME's post views module
			if( tie_get_option( 'tie_post_views' ) != 'theme' || ! is_single() || HERBS_HELPER::is_bot() ){
				return;
			}
			...
			if( ! self::is_cache_enabled() || ( function_exists( 'is_amp_endpoint' ) && is_amp_endpoint() ) ){
				...
				$count     = 0;
				$post_id   = get_the_ID();
				$count_key = apply_filters( 'Herbs/views_meta_field', 'tie_views' );
		...
		function postview_cache_enqueue(){
			...
			HERBS_HELPER::inline_script( 'tie-scripts', $cache_js );
		}
		...
		function increment_views(){
			...
			// Run only if the post views option is set to THEME's post views module
			if( tie_get_option( 'tie_post_views' ) != 'theme' || HERBS_HELPER::is_bot() ){
				return;
			}
		...
		function save_custom_views( $meta_fields ){
			...
			if( tie_get_option( 'tie_post_views') == 'theme' ){
				$meta_fields[] = apply_filters( 'Herbs/views_meta_field', 'tie_views' );
			}
		...
		function posts_column_views( $defaults ){
			...
			// Run only if the post views option is set to THEME's post views module
			if( tie_get_option( 'tie_post_views' )  == 'theme' ){
				$defaults['tie_post_views'] = esc_html__( 'Views', HERBS_TEXTDOMAIN );
			}
		...
		function posts_custom_column_views( $column_name, $id ){
			...
			if( $column_name === 'tie_post_views' ){
				echo HERBS_POSTVIEWS::get_views( '', get_the_ID() );
			}
		}
		...
		public static function get_views( $text = '', $post_id = 0 ){
			...
			if( $post_views_type == 'jetpack' && HERBS_JETPACK_IS_ACTIVE ){

				$count = HERBS_JETPACK::post_views( $post_id );
			}
			else{
				$count_key = apply_filters( 'Herbs/views_meta_field', 'tie_views' );
			...
			$formated = apply_filters( 'Herbs/post_views_number', number_format_i18n( $count ) );

			$output = '<span class="meta-views meta-item '. $views_class .'"><span class="tie-icon-fire" aria-hidden="true"></span> '.$formated.' '.$text.'</span>';

			return apply_filters( 'Herbs/post_views_output', $output, $post_id, $formated, $text );
		}
