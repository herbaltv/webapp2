...
if( ! class_exists( 'HERBS_MENU_LIMIT_DETECTOR' )){

	class HERBS_MENU_LIMIT_DETECTOR{
		...
		function check_post_limits(){
			...
			if( $r['suhosin_post_maxvars'] != '' || $r['suhosin_request_maxvars'] != '' || $r['max_input_vars'] != '' ){
				...
				if( ( $r['suhosin_post_maxvars'] != ''    && $r['suhosin_post_maxvars']    < 3000 ) ||
					...
					$message[] = esc_html__( 'Your server is running Suhosin, and your current maxvars settings may limit the number of menu items you can save.', HERBS_TEXTDOMAIN );
				}
		...
		function display_message( $message ){
			...
			if( ! empty( $message ) ){
				...
				<div class="error notice">
					<h4><?php esc_html_e( 'Menu Item Limit Warning', HERBS_TEXTDOMAIN ); ?></h4>
					<ul>
						<?php
