...
if( ! class_exists( 'HERBS_WELCOME_PAGE' )){

	class HERBS_WELCOME_PAGE{
		...
		function __construct(){
			...
			add_filter( 'Herbs/panel_submenus', array( $this, '_add_options_menu' ));
			add_filter( 'Herbs/about_tabs',     array( $this, '_add_about_tabs' ), 9 );
		}
		...
		function _add_options_menu( $menus ){
			...
			$menus[] = array(
				'page_title' => esc_html__( 'Getting Started', HERBS_TEXTDOMAIN ),
				'menu_title' => esc_html__( 'Getting Started', HERBS_TEXTDOMAIN ),
		...
		function _add_about_tabs( $tabs ){
			...
			$tabs['getting_started'] = array(
				'text' => esc_html__( 'Getting Started', HERBS_TEXTDOMAIN ),
		...
		function _page_content() {
			...
			do_action( 'Herbs/welcome_splash_content' );
		...
		public static function _head_section( $current_tab = 'getting_started' ){
			...
			$welcome_args = array(
				'title'   => sprintf( esc_html__( 'Welcome to %s', HERBS_TEXTDOMAIN ), apply_filters( 'Herbs/theme_name', 'Herbs' ) ),
				'about'   => '',
				'color'   => '#000000',
				'img'     => '',
				'version' => '',
			...
			$welcome_args = apply_filters( 'Herbs/welcome_args', $welcome_args );

			$tabs = array();
			$tabs = apply_filters( 'Herbs/about_tabs', $tabs );
			...
			<p class="about-text"><?php echo esc_html( $welcome_args['about'] ); ?>
				...
				<a href="https://twitter.com/share" class="twitter-share-button"
				   ...
				   data-text="<?php printf( esc_html__( 'I just installed the amazing %s #WordPress theme #herbs', HERBS_TEXTDOMAIN ), apply_filters( 'Herbs/theme_name', 'Herbs' ) ); ?>"
				   data-via="herbs" data-size="large">Tweet</a>
				<script>
					!function (d, s, id) {
			...
			<div class="tie-badge" style="background-color: <?php echo esc_attr( $welcome_args['color'] ); ?>;">
				...
				<?php printf( esc_html__( 'Version %s', HERBS_TEXTDOMAIN  ), HERBS_DB_VERSION ); ?>
			</div>
