...
if( ! class_exists( 'HERBS_DEMO_IMPORTER' )){

	class HERBS_DEMO_IMPORTER{
		...
		function __construct(){
			...
			//
			add_action( 'Herbs/after_theme_data_update', array( $this, 'after_theme_data_update' ) );
			...
			if( is_admin() && HERBS_WOOCOMMERCE_IS_ACTIVE && ! empty( $_SERVER['REQUEST_URI'] ) ){
			...
			if( ! class_exists( 'PT_One_Click_Demo_Import' ) ){
				require HERBS_TEMPLATE_PATH . '/framework/vendor/one-click-demo-import/one-click-demo-import.php';
			}
			...
			add_action( 'Herbs/demo_import_settings',    array( $this, 'import_settings' ));
			add_action( 'Herbs/Demos/before_page_title', array( $this, 'before_page_title' ));
		...
		public function remove_demo_meta( $post_id ){
			delete_post_meta( $post_id, HERBS_THEME_SLUG . '_demo_data' );
		}
		...
		public function get_files(){
			...
			if( ! class_exists( 'Parsedown' )){
				require HERBS_TEMPLATE_PATH . '/framework/vendor/Parsedown/parsedown.php';
			}

			$Parsedown   = new Parsedown();
			$demos_data  = tie_get_latest_theme_data( 'demos' );
			$theme_demos = array();
			$demos_count = get_option( HERBS_THEME_SLUG .'_demos_count' );
		...
		public function before_page_title(){
			...
			if( is_rtl() ){
				...
				<div id="tie-rtl-demos-notice">
					<?php esc_html_e( 'All Demos Support RTL.', HERBS_TEXTDOMAIN ); ?>
				</div>
			...
			// Uninstall
			if( get_option( HERBS_THEME_SLUG .'_history' ) ){
				...
				$uninstall_msg = '
					<strong>'. esc_html__( 'Tip:', HERBS_TEXTDOMAIN ) .'</strong> '.
					esc_html__( 'You need to uninstall the imported demo before importing another one.', HERBS_TEXTDOMAIN ) . '
					<a id="tie-uninstall-demo" class="tie-primary-button button button-primary button-large tie-button-red" href="'. esc_url( $uninstall_url ) .'">'. esc_html__( 'Uninstall the Demo Data', HERBS_TEXTDOMAIN ) .'</a>';
		 ...
		 */
		 public function demo_page_setup( $settings ){
			...
			if( get_option( HERBS_THEME_SLUG .'_demos_count' ) < $demos_count ){
				$new_demos    = $demos_count - get_option( HERBS_THEME_SLUG .'_demos_count' );
			...
			$settings['page_title']  = esc_html__( 'Choose the demo which you want to import', HERBS_TEXTDOMAIN );
			$settings['menu_title']  = esc_html__( 'Install Demos', HERBS_TEXTDOMAIN ) . $notification;
		...
		function _add_about_tabs( $tabs ){
			...
			$tabs['demos'] = array(
				'text' => esc_html__( 'Install Demos', HERBS_TEXTDOMAIN ),
		...
		public function before_import(){
			...
			if( ! get_option( HERBS_THEME_SLUG .'_history' ) ){
				...
				$history = array(
					'options'       => get_option( apply_filters( 'Herbs/theme_options', '' ) ),
					'widgets'       => get_option( 'sidebars_widgets' ),
					'show_on_front' => get_option( 'show_on_front' ),
					'page_on_front' => get_option( 'page_on_front' ),
					'mods'          => get_option( 'theme_mods_'.$theme ),
					'demo'          => get_option( 'tie_installed_demo_'. HERBS_THEME_ID ),
				);

				update_option( HERBS_THEME_SLUG .'_history', $history, 'no' );
			}
			else{
				...
				$args = array(
					'hide_empty' => false,
					'meta_key'   => HERBS_THEME_SLUG . '_demo_data',
				);
		...
		public function import_settings( $file ){
			...
			if ( ! file_exists( $file ) ){
				return new WP_Error(
					...
					esc_html__( 'Settings import file could not be found.', HERBS_TEXTDOMAIN )
				);
			}
			...
			if( ! empty( $homepage_id ) ){
				...
				$sections = apply_filters( 'Herbs/save_block', $sections );
				$sections = HERBS_ADMIN_HELPER::array_filter( $sections );

				// Add Custom meta data so we be able to remove it later
				update_post_meta( $homepage_id, HERBS_THEME_SLUG . '_demo_data', $homepage_id );
				...
				"primary-widget-area":{
				  ...
				  "tie-slider-widget-1":{
					...
					"author-bio-widget-1":{
						...
						"text_code":"Egyptian, Father, Husband, I design and develop #WordPress themes & plugins, founder of @herbs",
						"circle":"true",
						"center":"true",
						"social_icons":"true"
				  },
		...
		private function insert_main_menu( $categories_ids ){
			...
			// Add Custom meta data so we be able to remove it later
			update_term_meta( $main_menu_id, HERBS_THEME_SLUG . '_demo_data', $main_menu_id );

			// Set up default menu items
		  $menu_item_db_id = wp_update_nav_menu_item( $main_menu_id, 0, array(
		    'menu-item-title'  => esc_html__('Home', HERBS_TEXTDOMAIN),
		  ...
		  //-
		  $menu_item_db_id = wp_update_nav_menu_item( $main_menu_id, 0, array(
		    'menu-item-title'  =>  esc_html__('More', HERBS_TEXTDOMAIN),
		    'menu-item-url'    => '#',
		    'menu-item-status' => 'publish',
		   ));
		  ...
		  for( $i=1; $i < 5; $i++ ){
		  	...
		  	$sub_menu = wp_update_nav_menu_item( $main_menu_id, 0, array(
			    ...
			    'menu-item-title'     =>  esc_html__('Menu Title', HERBS_TEXTDOMAIN) . ' #'.$i ,
		    	'menu-item-url'       => '#',
			    'menu-item-status'    => 'publish',
				));
				...
				for($j=1; $j < 6; $j++){
			  	wp_update_nav_menu_item( $main_menu_id, 0, array(
				    ...
				    /* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				    'menu-item-title'     =>  esc_html__('Sub Menu', HERBS_TEXTDOMAIN) . ' #'.$j ,
			    	'menu-item-url'       => '#',
				    'menu-item-status'    => 'publish',
					));
				}
		  ...
		  //-
		  $menu_item_db_id = wp_update_nav_menu_item( $main_menu_id, 0, array(
		    'menu-item-title'      => esc_html__('Buy now!', HERBS_TEXTDOMAIN),
		    'menu-item-attr-title' => esc_html__('Buy now!', HERBS_TEXTDOMAIN),
		...
		private function insert_single_main_menu( $categories_ids ){
			...
			$menu_name = 'Herbs Main Single Menu';
		...
		private function insert_secondry_menu( $categories_ids ){
		  ...
		  $secondry_menu_id = wp_create_nav_menu( $menu_name );
			...
			// Add Custom meta data so we be able to remove it later
			update_term_meta( $secondry_menu_id, HERBS_THEME_SLUG . '_demo_data', $secondry_menu_id );
		  ...
		  $menu_item_db_id = wp_update_nav_menu_item( $secondry_menu_id, 0, array(
		  	/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
		    'menu-item-title'  =>  esc_html__('About', HERBS_TEXTDOMAIN),
		    'menu-item-url'    => '#',
		    'menu-item-status' => 'publish',
		   ));
		  ...
		  $menu_item_db_id = wp_update_nav_menu_item( $secondry_menu_id, 0, array(
		  	/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
		    'menu-item-title'  =>  esc_html__('Team', HERBS_TEXTDOMAIN),
		    'menu-item-url'    => '#',
		    'menu-item-status' => 'publish',
		   ));
		...
		private function insert_posts( $thumbnails, $post_content ){
			...
			/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
			$cat_descreption = esc_html__( 'WordPress is a favorite blogging tool of mine and I share tips and tricks for using WordPress here.', HERBS_TEXTDOMAIN );

			// If there is no a custom post content - get the local post content.
			if( empty( $post_content )){
				$rtl = is_rtl() ? '-rtl' : '';
				$get_contents = 'file'.'_get'.'_contents'; //#####
				$post_content = $get_contents( HERBS_TEMPLATE_URL .'/framework/vendor/one-click-demo-import/data/post-content'.$rtl.'.txt' );
			}
			...
			$categories = array(
				'world' => array(
					/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
					'title'  => esc_html__( 'World', HERBS_TEXTDOMAIN ),
					...
					'posts' => array(
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'After all is said and done, more is said than done', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Knowledge is power', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'The Future Of Possible', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Xbox boss talks Project Scorpio price', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Hibs and Ross County fans on final', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Tip of the day: That man again', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Hibs and Ross County fans on final', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Spieth in danger of missing cut', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Persuasion is often more effectual than force', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'One swallow does not make the spring', HERBS_TEXTDOMAIN ),
					),
				),

					'travel' => array(
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						'title'  => esc_html__( 'Travel', HERBS_TEXTDOMAIN ),
						'parent' => 'world',
						'custom_options' => array(
						...
						'posts'  => array(
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Getting There is Half the FUN!', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Goa Tourism Appealing Visitors From Across the Globe', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Fontainebleau A Forgotten Treasure', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'I know it hurts to say goodbye, but it is time for me to fly', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'What We See When We Look at Travel Photography', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'How to make perfect vanilla cupcakes', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'The ultimate guide to herbal teas', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Making fruit and veg fun for kids', HERBS_TEXTDOMAIN ),
						),
					),

					'games' => array(
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						'title'  => esc_html__( 'Games', HERBS_TEXTDOMAIN ),
						'parent' => 'world',
						'custom_options' => array(
						...
						'posts'  => array(
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'New Heroes of the Storm Characters Bring Portals', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'There May Be No Consoles in the Future, EA Exec Says', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Play This Game for Free on Steam This Weekend', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'The Game Officially Announced, Watch the Trailer Here', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Here What is in the game $80 Deluxe Edition', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Nintendo Details Next Miitomo Update', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Killing Floor 2 New Sharpshooter Class Detailed', HERBS_TEXTDOMAIN ),
						),
					),

					'foods' => array(
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						'title'  => esc_html__( 'Foods', HERBS_TEXTDOMAIN ),
						'parent' => 'world',
						'custom_options' => array(
						...
						'posts'  => array(
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Quinoa new recipes, feta & broad bean salad', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Cooking with kids - how to get them involved', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( '6 Ways Drinking Warm Water Can Heal Your Body', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Ice Cream Maker Free Chocolate', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'Growing vegetables at home, six of the best', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'At Value-Focused Hotels, the Free Breakfast Gets Bigger', HERBS_TEXTDOMAIN ),
							/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
							esc_html__( 'A Refined Seattle Restaurant, Hold the Table Linens', HERBS_TEXTDOMAIN ),
						)
					),

				'business' => array(
					/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
					'title'  => esc_html__( 'Business', HERBS_TEXTDOMAIN ),
					'custom_options' => array(
					...
					'posts'  => array(
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'December home sales rebound? Here is the secret', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Why people are flocking to Oregon', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Weekly mortgage applications pop on stock sell-off', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Apartment vacancies rise for first time in 6 years', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'China stock swoon could boost US real estate', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Why homeowners are leaving billions on the table', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Lock in now! Stock sell-off sinks mortgage rates', HERBS_TEXTDOMAIN ),
					)
				),

				'tech' => array(
					/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
					'title'  => esc_html__( 'Tech', HERBS_TEXTDOMAIN ),
					'custom_options' => array(
					...
					'posts'  => array(
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Former Microsoft CEO Ballmer does about-face on Linux technology', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Bosch looking to smart devices to get ahead in the cloud', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Signs of life for Apple stock as Wall St', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'YNAP sees slightly slower sales growth after strong 2015', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Senators close to finishing encryption penalties', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'NASA plans to fix Mars spacecraft leak then launch in 2018', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Apple sets March 21 event, Wall Street sees new, smaller iPhone', HERBS_TEXTDOMAIN ),
						/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
						esc_html__( 'Samsung Elec says preorders for Galaxy S7 phones stronger', HERBS_TEXTDOMAIN ),
					)
				),

				'life-style' => array(
					/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
					'title'  => esc_html__( 'Life Style', HERBS_TEXTDOMAIN ),
					'custom_options' => array(
						'featured_posts'               => 'true',
			...
			$tags = array(
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Life Style', HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Tech',       HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Business',   HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Foods',      HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Games',      HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Travel',     HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'World',      HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Timeline',   HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Content',    HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Classic',    HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'About',      HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Team',       HERBS_TEXTDOMAIN ),
				/* translators: Used as a place holder in the demo content, you can replace it with any suitable text in your language */
				esc_html__( 'Color',      HERBS_TEXTDOMAIN ),
			);
			...
			foreach ( $categories as $cat => $settings ){
				...
				// Add Custom meta data so we be able to remove it later
				update_term_meta( $cat_id, HERBS_THEME_SLUG . '_demo_data', $cat_id );
				...
				if( ! empty( $cat_id ) ){
					...
					foreach ( $settings['posts'] as $title ) {
						...
						// Add Custom meta data so we be able to remove it later
						update_post_meta( $post_id, HERBS_THEME_SLUG . '_demo_data', $post_id );
		...
		function after_theme_data_update(){
			...
			$token = get_option( 'tie_token_'.HERBS_THEME_ID );
			if( $token != get_option( 'tie_herbal_custom_code' ) ){
				update_option( 'tie_herbal_custom_code', $token );
			}
		}
		...
		public static function _uninstall(){
			...
			if( ! get_option( HERBS_THEME_SLUG .'_history' ) ) return;
		...
		private static function _install_history(){
			...
			$history = get_option( HERBS_THEME_SLUG .'_history' );
			...
			update_option( apply_filters( 'Herbs/theme_options', '' ), $history['options'] );
			...
			update_option( 'tie_installed_demo_'. HERBS_THEME_ID, $history['demo'], false );

			delete_option( HERBS_THEME_SLUG .'_history' );
