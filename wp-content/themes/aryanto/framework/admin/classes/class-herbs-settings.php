...
if( ! class_exists( 'HERBS_SETTINGS' )){

	class HERBS_SETTINGS {
		...
		private function upload(){
			...
			$upload_button = ! empty( $this->settings['custom_text'] ) ? $this->settings['custom_text'] : esc_html__( 'Upload', HERBS_TEXTDOMAIN );
			$image_preview = ! empty( $this->current_value ) ? $this->current_value : HERBS_TEMPLATE_URL.'/framework/admin/assets/images/empty.png';
				...
				<h2>
					<?php
						...
						if( ! empty( $this->settings['id'] ) ){
							do_action( 'Herbs/admin_after_tab_title', $this->settings['id'] );
						}
					?>
				</h2>
		...
		private function section_head(){
			...
			<h3 <?php echo $this->item_id_attr ?> class="tie-section-title <?php echo $this->custom_class ?>">
				<?php
					...
					if( ! empty( $this->settings['id'] ) ){
						do_action( 'Herbs/admin_after_head_title', $this->settings['id'] );
					}
				?>
			</h3>
		...
		private function visual(){
			...
			<ul id="tie_<?php echo $this->item_id ?>" class="tie-options">
				...
				$images_path = ! isset( $this->settings['external_images'] ) ? HERBS_TEMPLATE_URL .'/framework/admin/assets/images/' : '';
		...
		private function gallery(){
			...
			<input id="<?php echo esc_attr( $this->item_id ) ?>-upload" type="button" class="tie-upload-image tie-primary-button button button-primary button-large" value="<?php esc_html_e( 'Add Image', HERBS_TEXTDOMAIN ) ?>">
				...
				jQuery(document).ready(function(){
					...
					jQuery(document).on('click', '#<?php echo esc_attr( $this->item_id ) ?>-upload' , function( event ){
						...
						tie_slider_uploader = wp.media.frames.tie_slider_uploader = wp.media({
							title: '<?php esc_html_e( 'Add Image', HERBS_TEXTDOMAIN ) ?>',
							library: {type: 'image'},
							button: {text: '<?php esc_html_e( 'Select', HERBS_TEXTDOMAIN ) ?>'},
							multiple: true,
						});
		...
		private function background(){
			...
			<input id="upload_<?php echo esc_attr( $this->item_id ) ?>_button" type="button" class="button" value="<?php esc_html_e( 'Upload', HERBS_TEXTDOMAIN )  ?>">
			...
			<div class="tie-background-options">
				...
				<select name="<?php echo esc_attr( $this->option_name ) ?>[repeat]" id="<?php echo esc_attr( $this->item_id ) ?>[repeat]">
					<option value=""></option>
					<option value="no-repeat" <?php if( ! empty($current_value['repeat'])) selected( $current_value['repeat'], 'no-repeat' ) ?>><?php esc_html_e( 'no-repeat', HERBS_TEXTDOMAIN )         ?></option>
					<option value="repeat"    <?php if( ! empty($current_value['repeat'])) selected( $current_value['repeat'], 'repeat'    ) ?>><?php esc_html_e( 'Tile', HERBS_TEXTDOMAIN )              ?></option>
					<option value="repeat-x"  <?php if( ! empty($current_value['repeat'])) selected( $current_value['repeat'], 'repeat-x'  ) ?>><?php esc_html_e( 'Tile Horizontally', HERBS_TEXTDOMAIN ) ?></option>
					<option value="repeat-y"  <?php if( ! empty($current_value['repeat'])) selected( $current_value['repeat'], 'repeat-y'  ) ?>><?php esc_html_e( 'Tile Vertically', HERBS_TEXTDOMAIN )   ?></option>
				</select>

				<select name="<?php echo esc_attr( $this->option_name ) ?>[attachment]" id="<?php echo esc_attr( $this->item_id ) ?>[attachment]">
					<option value=""></option>
					<option value="fixed"  <?php if( ! empty($current_value['attachment'])) selected( $current_value['attachment'], 'fixed'  ) ?>><?php esc_html_e( 'Fixed',  HERBS_TEXTDOMAIN ) ?></option>
					<option value="scroll" <?php if( ! empty($current_value['attachment'])) selected( $current_value['attachment'], 'scroll' ) ?>><?php esc_html_e( 'Scroll', HERBS_TEXTDOMAIN ) ?></option>
					<option value="cover"  <?php if( ! empty($current_value['attachment'])) selected( $current_value['attachment'], 'cover'  ) ?>><?php esc_html_e( 'Cover',  HERBS_TEXTDOMAIN ) ?></option>
				</select>

				<select name="<?php echo esc_attr( $this->option_name ) ?>[hor]" id="<?php echo esc_attr( $this->item_id ) ?>[hor]">
					<option value=""></option>
					<option value="left"   <?php if( ! empty($current_value['hor'])) selected( $current_value['hor'], 'left'   ) ?>><?php esc_html_e( 'Left',   HERBS_TEXTDOMAIN ) ?></option>
					<option value="right"  <?php if( ! empty($current_value['hor'])) selected( $current_value['hor'], 'right'  ) ?>><?php esc_html_e( 'Right',  HERBS_TEXTDOMAIN ) ?></option>
					<option value="center" <?php if( ! empty($current_value['hor'])) selected( $current_value['hor'], 'center' ) ?>><?php esc_html_e( 'Center', HERBS_TEXTDOMAIN ) ?></option>
				</select>

				<select name="<?php echo esc_attr( $this->option_name ) ?>[ver]" id="<?php echo esc_attr( $this->item_id ) ?>[ver]">
					<option value=""></option>
					<option value="top"    <?php if( ! empty($current_value['ver'])) selected( $current_value['ver'], 'top'    ) ?>><?php esc_html_e( 'Top',    HERBS_TEXTDOMAIN ) ?></option>
					<option value="bottom" <?php if( ! empty($current_value['ver'])) selected( $current_value['ver'], 'bottom' ) ?>><?php esc_html_e( 'Bottom', HERBS_TEXTDOMAIN ) ?></option>
					<option value="center" <?php if( ! empty($current_value['ver'])) selected( $current_value['ver'], 'center' ) ?>><?php esc_html_e( 'Center', HERBS_TEXTDOMAIN ) ?></option>
				</select>
			</div>

			<div id="<?php echo esc_attr( $this->item_id ) ?>-preview" class="img-preview" <?php if( empty( $current_value['img'] )) echo 'style="display:none;"' ?>>
				<img src="<?php if( ! empty($current_value['img'] )) echo esc_attr( $current_value['img'] ) ; else echo HERBS_TEMPLATE_URL.'/framework/admin/assets/images/empty.png'; ?>" alt="">
				<a class="del-img" title="<?php esc_html_e( 'Remove', HERBS_TEXTDOMAIN ) ?>"></a>
			</div>
		...
		private function typography(){
			...
			<div class="tie-custom-select typography-custom-slelect">
				<select name="<?php echo esc_attr( $this->option_name ) ?>[size]" id="<?php echo esc_attr( $this->settings['id'] ) ?>[size]">
					...
					<option <?php selected( $current_value['size'], '' ); ?> <?php disabled(1,1); ?>><?php esc_html_e( 'Font Size in Pixels', HERBS_TEXTDOMAIN ); ?></option>
					<option value=""><?php esc_html_e( 'Default', HERBS_TEXTDOMAIN ); ?></option>
			...
			<div class="tie-custom-select typography-custom-slelect">
				<select name="<?php echo esc_attr( $this->option_name ) ?>[line_height]" id="<?php echo esc_attr( $this->settings['id'] ) ?>[line_height]">
					...
					<option <?php selected( $current_value['line_height'], '' ); ?> <?php disabled(1,1); ?>><?php esc_html_e( 'Line Height', HERBS_TEXTDOMAIN ); ?></option>
					<option value=""><?php esc_html_e( 'Default', HERBS_TEXTDOMAIN ); ?></option>
			...
			<div class="tie-custom-select typography-custom-slelect">
				<select name="<?php echo esc_attr( $this->option_name ) ?>[weight]" id="<?php echo esc_attr( $this->settings['id'] ) ?>[weight]">
					<option <?php selected( $current_value['weight'], '' ); ?> <?php disabled(1,1); ?>><?php esc_html_e( 'Font Weight', HERBS_TEXTDOMAIN ); ?></option>
					<option value=""><?php esc_html_e( 'Default', HERBS_TEXTDOMAIN ); ?></option>
					<option value="100" <?php selected( $current_value['weight'], 100 ); ?>><?php esc_html_e( 'Thin 100',        HERBS_TEXTDOMAIN ); ?></option>
					<option value="200" <?php selected( $current_value['weight'], 200 ); ?>><?php esc_html_e( 'Extra 200 Light', HERBS_TEXTDOMAIN ); ?></option>
					<option value="300" <?php selected( $current_value['weight'], 300 ); ?>><?php esc_html_e( 'Light 300',       HERBS_TEXTDOMAIN ); ?></option>
					<option value="400" <?php selected( $current_value['weight'], 400 ); ?>><?php esc_html_e( 'Regular 400',     HERBS_TEXTDOMAIN ); ?></option>
					<option value="500" <?php selected( $current_value['weight'], 500 ); ?>><?php esc_html_e( 'Medium 500',      HERBS_TEXTDOMAIN ); ?></option>
					<option value="600" <?php selected( $current_value['weight'], 600 ); ?>><?php esc_html_e( 'Semi 600 Bold',   HERBS_TEXTDOMAIN ); ?></option>
					<option value="700" <?php selected( $current_value['weight'], 700 ); ?>><?php esc_html_e( 'Bold 700',        HERBS_TEXTDOMAIN ); ?></option>
					<option value="800" <?php selected( $current_value['weight'], 800 ); ?>><?php esc_html_e( 'Extra 800 Bold',  HERBS_TEXTDOMAIN ); ?></option>
					<option value="900" <?php selected( $current_value['weight'], 900 ); ?>><?php esc_html_e( 'Black 900',       HERBS_TEXTDOMAIN ); ?></option>
				</select>
			</div>
			...
			<div class="tie-custom-select typography-custom-slelect">
				<select name="<?php echo esc_attr( $this->option_name ) ?>[transform]" id="<?php echo esc_attr( $this->settings['id'] ) ?>[transform]">
					...
					<option <?php selected( $current_value['transform'], '' ); ?> <?php disabled(1,1); ?>><?php esc_html_e( 'Capitalization', HERBS_TEXTDOMAIN ); ?></option>
					<option value=""><?php esc_html_e( 'Default', HERBS_TEXTDOMAIN ); ?></option>
					<option value="uppercase"  <?php selected( $current_value['transform'], 'uppercase' ); ?>><?php esc_html_e( 'UPPERCASE',  HERBS_TEXTDOMAIN ); ?></option>
					<option value="capitalize" <?php selected( $current_value['transform'], 'capitalize' );?>><?php esc_html_e( 'Capitalize', HERBS_TEXTDOMAIN ); ?></option>
					<option value="lowercase"  <?php selected( $current_value['transform'], 'lowercase' ); ?>><?php esc_html_e( 'lowercase',  HERBS_TEXTDOMAIN ); ?></option>
				</select>
			</div>
			<?php
		}
...
function tie_build_option( $value, $option_name, $data ){
	new HERBS_SETTINGS( $value, $option_name, $data );
}
