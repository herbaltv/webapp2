...
if( ! class_exists( 'HERBS_THEME_UPDATER' ) ){


	class HERBS_THEME_UPDATER {
		...
		function __construct( ) {
			...
			*/
			...
			$this->current_theme_version = HERBS_DB_VERSION;
			$this->theme_changeLog_url   = apply_filters( 'Herbs/External/changelog', '' );
			...
			add_action( 'Herbs/after_theme_data_update', array( $this, 'update_cached_data' ) );
		}
		...
		function check_for_update( $transient ){
			...
			if( ! empty( $data ) ){
				$transient->response[ HERBS_THEME_SLUG ] = $data;
			}
		...
		function update_notifier_menu(){
			...
			add_submenu_page(
				'tie-theme-options',
				esc_html__( 'Theme Updates', HERBS_TEXTDOMAIN ),
				esc_html__( 'New Update', HERBS_TEXTDOMAIN ) . ' <span class="update-plugins tie-theme-update"><span class="update-count">'. $this->remote_theme_version .'</span></span>',
				'administrator',
				'theme-update-notifier',
				array( $this, 'redirect_to_update_notifier' )
			);

			add_filter( 'Herbs/options_tab_title',           array( $this, 'add_theme_updates_tab_title' ) );
		...
		function add_theme_updates_tab_title( $settings_tabs ){
			...
			$settings_tabs['theme-updates'] = array(
				'icon'  => 'update',
				'title' => esc_html__( 'New Update', HERBS_TEXTDOMAIN ) . ' <span class="tie-theme-update"><span class="update-count">'.esc_html__( 'New', HERBS_TEXTDOMAIN ).'</span></span>',
			);
		...
		function add_theme_updates_tab(){
			...
			tie_build_theme_option(
				array(
					'title' =>	esc_html__( 'New Theme Update', HERBS_TEXTDOMAIN ),
					'type'  => 'tab-title',
				));

			tie_build_theme_option(
				array(
					'text' => sprintf( esc_html__( 'There is a new version of the %s available.', HERBS_TEXTDOMAIN ), apply_filters( 'Herbs/theme_name', 'Herbs' ) ) .' <a href="'. $this->theme_changeLog_url .'" target="_blank">'. sprintf( esc_html__( 'View version %1$s details.', HERBS_TEXTDOMAIN ), $this->remote_theme_version ) .'</a>',
					'type' => 'message',
				));
			...
			if( ! empty( $support_info['status'] ) && $support_info['status'] == 'active' ){
				...
				if( get_template() != HERBS_THEME_SLUG ) {

					tie_build_theme_option(
						array(
							'text' => sprintf(
								esc_html__( 'The theme folder name does not match the original theme folder name %1$s%3$s%2$s, you need to chnage the theme folder name back to be able to update the theme automatically, or you can update the theme manually via FTP.', HERBS_TEXTDOMAIN ),
								'<strong>',
								'</strong>',
								HERBS_THEME_SLUG
							),
						...
						<div class="tie-theme-updates-buttons">
							<a class="tie-primary-button button button-primary" target="_blank" href="<?php echo apply_filters( 'Herbs/External/update_manually', '' ); ?>"><?php esc_html_e( 'How to update the theme manually?', HERBS_TEXTDOMAIN ) ?></a>
						</div>
					...
					$update_url = add_query_arg( array(
							...
							'theme'  => HERBS_THEME_SLUG,
						), self_admin_url( 'update.php' ) );
					?>

					<div class="tie-theme-updates-buttons">
						<a class="tie-primary-button button button-primary button-hero" href="<?php echo esc_url( wp_nonce_url( $update_url, 'upgrade-theme_' . HERBS_THEME_SLUG ) ) ?>"><?php esc_html_e( 'Update Automatically', HERBS_TEXTDOMAIN ) ?></a>
					</div>
			...
			<p><?php esc_html_e( 'Please Note: Any customizations you have made to theme files will be lost. Please consider using child themes for modifications.', HERBS_TEXTDOMAIN ); ?></p>
