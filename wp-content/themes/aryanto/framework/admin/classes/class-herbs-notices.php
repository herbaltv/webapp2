...
if( ! class_exists( 'HERBS_NOTICES' ) ){
	...
	class HERBS_NOTICES {
		...
		function load_notices(){
			...
			// Need Help pointer
			if( ! self::is_dismissed( 'tie_need_help_'. HERBS_THEME_SLUG ) ){
		...
		function need_help_script(){
			$pointer_content  = '<h3>'. esc_html__( 'Need Help?', HERBS_TEXTDOMAIN ) .'</h3>';
			$pointer_content .= '<p>'. sprintf( esc_html__( 'Click on the help icon %s if you need help.', HERBS_TEXTDOMAIN ), '<span class="dashicons dashicons-editor-help"></span>' ) .'</p>'; ?>
			...
			<script>
				...
				jQuery(document).ready( function($){
					$('#help-icon-general-settings-tab').pointer({
						...
						close: function(){
							$.post( ajaxurl, {
								pointer: 'tie_need_help_<?php echo esc_js( HERBS_THEME_SLUG ) ?>',
		...
		function happy_new_year(){
			...
			if ( ! self::is_dismissed( $notice_id ) && ( $new_year_dates['today_date'] >= $new_year_dates['first_congrats_day'] || $new_year_dates['today_date'] < $new_year_dates['last_congrats_day'] ) ){
				...
				$notice_content  = '<h4>'. esc_html__( 'Happy New Year!', HERBS_TEXTDOMAIN ) .'</h4>';
				$notice_content .= '<p>'. esc_html__( 'To our client who have made our progress possible, All of us at Herbs join in wishing you a Happy New Year with the best of everything in your life for you and your family and we look forward to serving you in the new year :)', HERBS_TEXTDOMAIN ) .'</p>';
				$notice_content .= '<p>'. sprintf(
					esc_html__( 'Follow us on %1$sTwitter%3$s or %2$sFacebook%3$s.', HERBS_TEXTDOMAIN ),
					'<a href="http://twitter.com/herbs" target="_blank">',
					'<a href="https://www.facebook.com/herbs" target="_blank">',
					'</a>'
				) .'</p>';
		...
		function rate_theme(){
			...
			$notice_id = 'tie_install_date_'. HERBS_THEME_ID;
			...
			if( ! get_option( $notice_id ) ){
				...
				$notice_content  = '<h4>'. sprintf( esc_html__( 'Like %s?', HERBS_TEXTDOMAIN ), apply_filters( 'Herbs/theme_name', 'Herbs' ) ) .'</h4>';
				$notice_content .= sprintf(
					esc_html__( 'We\'ve noticed you\'ve been using %1$s for some time now; we hope you love it! We\'d be thrilled if you could %2$sgive us a 5* rating on themeforest.net!%4$s If you are experiencing issues, please %3$sopen a support ticket%4$s and we\'ll do our best to help you out.', HERBS_TEXTDOMAIN ),
					apply_filters( 'Herbs/theme_name', 'Herbs' ),
					'<a href="'. tie_get_purchase_link( array( 'utm_medium' => 'rate-popup' ) ) .'" target="_blank">',
					'<a href="'. apply_filters( 'Herbs/External/open_ticket', '' ) .'" target="_blank">',
					'</a>'
				);
		...
		function theme_translation(){
			...
			$locale = get_locale();
			$notice_id = 'tie_translate_theme_'.$locale.'_'.HERBS_THEME_ID;
			...
			if( get_option( 'tie_install_date_'. HERBS_THEME_ID ) && ( time() - get_option( 'tie_install_date_'. HERBS_THEME_ID) ) < ( 3 * DAY_IN_SECONDS ) ){
				return false;
			}
			...
			$notice_content .= '
				<div class="translations-meter-outer">
					...
					sprintf( esc_html__( '%1s Translation Status: %2s Done', HERBS_TEXTDOMAIN ), $native_name, '<strong>'. $translation_meter .'%</strong>') .'
				</div>
			';

			$notice_content .= '<em><small>'. esc_html__( '* The extra support will appear in your profile on our website, support center and in the theme dashboared only. it will not appear on ThemeForest.', HERBS_TEXTDOMAIN ) .'</small></em><div class="clearfix"></div>';

			self::message( array(
				'notice_id'   => $notice_id,
				'title'       => esc_html__( 'Get Extra FREE 6 Months Support period', HERBS_TEXTDOMAIN ),
				'message'     => $notice_content,
				'color'       => '#A770EF',
				'button_text' => esc_html__( 'Go to the Translation Portal', HERBS_TEXTDOMAIN ),
				'button_url'  => apply_filters( 'Herbs/translations_panel_url', '' ),
			));
		}
		...
		function new_update(){
			...
			$notice_id = 'tie_new_updates_'. HERBS_THEME_ID;

			if ( ! self::is_dismissed( $notice_id ) && $changelog = get_option( 'tie_chnagelog_'. HERBS_THEME_ID ) ){
				...
				$notice_content  = '<h4>'. esc_html__( 'YAY, New Features', HERBS_TEXTDOMAIN ) .'</h4>';
				...
				if( ! tie_is_theme_rated() ){
					...
					$notice_content .= '<span class="awesome">'. esc_html__( 'Awesome, isn\'t it? Give us some love :)', HERBS_TEXTDOMAIN ) .'</span>';
					$button_text     = sprintf( esc_html__( 'Rate %s', HERBS_TEXTDOMAIN ), apply_filters( 'Herbs/theme_name', 'Herbs' ) );
				}
		...
		function new_milestone(){
			...
			if( class_exists( 'ARQAM_LITE_COUNTERS' ) || function_exists( 'arq_counters_data' ) ){
				...
				$notice_title   = '&#x1F3C6;';
				$notice_desc    = esc_html__( 'Followers', HERBS_TEXTDOMAIN );
				...
				if( ! empty( $arq_counters['data'] ) && is_array( $arq_counters['data'] ) ){
					...
					if( ! empty( $milestone ) ){
						...
						$the_number = sprintf( esc_html__( '%s Followers', HERBS_TEXTDOMAIN ), $milestone );
			...
			if( tie_get_option( 'tie_post_views' ) == 'theme' && ! $milestone ){
				...
				$notice_title   = '&#128079;';
				$notice_desc    = esc_html__( 'Total Articles\' Views!', HERBS_TEXTDOMAIN );

				// Let's Sum all post views for all posts
				global $wpdb;
				$number = $wpdb->get_var( $wpdb->prepare( " SELECT sum(meta_value) FROM $wpdb->postmeta WHERE meta_key = %s", apply_filters( 'Herbs/views_meta_field', 'tie_views' ) ) );
				...
				if( ! empty( $milestone ) ){
					...
					$the_number = sprintf( esc_html__( '%s articles views', HERBS_TEXTDOMAIN ), $milestone );
			...
			// The Notice message
			$the_message     = sprintf( esc_html__( 'We just reached the %s milestone! We are taking this moment to give YOU a big THANK YOU for all the support and confidence you gave us.', HERBS_TEXTDOMAIN ), $the_number ) .' #milestone';
			$notice_content  = '<h4>'. esc_html__( 'New Milestone', HERBS_TEXTDOMAIN ) .'</h4>';
			$notice_content .= '<div class="milestone-number">'. $milestone .'</div>';
			$notice_content .= '<div class="milestone-desc">'. $notice_desc .'</div>';
			$notice_content .= '<a class="tie-primary-button button button-primary tweet-milestone button-hero" target="_blank" href="https://twitter.com/intent/tweet?text='. urlencode( $the_message ).'&amp;url='. esc_url(home_url( '/' )) .'"><span class="dashicons dashicons-twitter"></span> '. esc_html__( 'Spread The Word', HERBS_TEXTDOMAIN ) .'</a>';
		...
		function happy_anniversary(){
			...
			if( ! empty( $customer_since )){
				...
				if( $current_month == $customer_month && $customer_year < $current_year ){
					...
					$years_text = sprintf( _n( '%d year', '%d years', $number_of_years, HERBS_TEXTDOMAIN ), $number_of_years );

					$notice_content  = '<h4>'. esc_html__( 'Happy Anniversary with Herbs!', HERBS_TEXTDOMAIN ) .'</h4>';
					$notice_content .= '<p>'. sprintf( esc_html__( 'Woohoo! We are so happy You have been with us for %s We are looking forward to providing an awesome WordPress theme and plugins for you for many more. Thanks for being an awesome customer!', HERBS_TEXTDOMAIN ), $years_text ) .'</p>';
					$notice_content .= '<p>'. esc_html__( 'Your friends at Herbs', HERBS_TEXTDOMAIN ) .'</p>';

					echo '<div id="tie-page-overlay" class="is-notice-dismissible" data-id="'. $notice_id .'" style="bottom: 0; opacity: 0.8;"></div>';

					self::message( array(
						'notice_id' => $notice_id,
						'title'     => '<img src="'.HERBS_TEMPLATE_URL. '/framework/admin/assets/images/badges/'. $number_of_years .'.png" alt="" />',
						'class'     => 'sucess tie-popup-block tie-popup-window tie-notice-popup tie-yay',
						'message'   => $notice_content,
					));
				}
		...
		public static function message( $args = array() ){
			...
			$defaults = array(
				'notice_id'      => '',
				'title'          => esc_html__( 'Howdy', HERBS_TEXTDOMAIN ),
				'img'            => false,
				'message'        => '',
				'dismissible'    => true,
				'color'          => '',
	...
	// Single instance.
	$HERBS_NOTICES = new HERBS_NOTICES();
}
