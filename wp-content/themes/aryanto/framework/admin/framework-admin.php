...
function tie_chnage_theme_parent_menu( $name ){
	...
	foreach ($menu as $key => $value) {
		if( ! empty( $value[0] ) && $value[0] == 'tietheme' ){
			$menu[$key][0] = apply_filters( 'Herbs/theme_name', 'Herbs' );
			break;
		}
	}
 ...
 * Theme Options tabs
 */
add_filter( 'Herbs/options_tab_title', 'tie_build_theme_options_tabs', 9 );
function tie_build_theme_options_tabs(){
	...
	$settings_tabs = array(
		...
		'header' => array(
			'icon'	=> 'schedule',
			'title'	=> esc_html__( 'Header', HERBS_TEXTDOMAIN )),
		...
		'footer' => array(
			...
			'title' => esc_html__( 'Footer', HERBS_TEXTDOMAIN )),

		'blocks' => array(
			'icon'	=> 'welcome-widgets-menus',
			'title'	=> esc_html__( 'Blocks', HERBS_TEXTDOMAIN )),

		'archives' => array(
			'icon'	=> 'exerpt-view',
			'title'	=> esc_html__( 'Archives', HERBS_TEXTDOMAIN )),
		...
		'share' => array(
			'icon'  => 'share',
			'title' => esc_html__( 'Share Buttons', HERBS_TEXTDOMAIN )),

		'sidebars' => array(
			'icon'  => 'slides',
			'title' => esc_html__( 'Sidebars', HERBS_TEXTDOMAIN )),

		'lightbox' => array(
			'icon'  => 'format-image',
			'title' => esc_html__( 'LightBox', HERBS_TEXTDOMAIN )),
		...
		'typography' => array(
			'icon'  => 'editor-italic',
			'title' => esc_html__( 'Typography', HERBS_TEXTDOMAIN )),

		'translations' => array(
			'icon'  => 'editor-textcolor',
			'title' => esc_html__( 'Translations', HERBS_TEXTDOMAIN )),
		...
		'mobile' => array(
			'icon'  => 'smartphone',
			'title' => esc_html__( 'Mobile', HERBS_TEXTDOMAIN )),

		'amp' => array(
			'icon'  => 'search',
			'title' => esc_html__( 'AMP', HERBS_TEXTDOMAIN )),
		...
		'advanced' => array(
			'icon'  => 'admin-tools',
			'title' => esc_html__( 'Advanced', HERBS_TEXTDOMAIN )),
	);
	...
	if ( HERBS_WOOCOMMERCE_IS_ACTIVE ){
		...
		$settings_tabs['woocommerce'] = array(
			'icon'  => 'woocommerce',
			'title' => esc_html__( 'WooCommerce', HERBS_TEXTDOMAIN ),
		);
	}
	...
	$settings_tabs['backup'] = array(
		'icon'  => 'migrate',
		'title' => esc_html__( 'Export/Import', HERBS_TEXTDOMAIN ),
	);
...
function tie_modify_admin_bar( $wp_admin_bar ){
	...
	if ( ! current_user_can( 'switch_themes' ) || ! tie_get_option( 'theme_toolbar' ) || HERBS_ADMIN_HELPER::is_theme_options_page() ){
		return;
	}
	...
	$wp_admin_bar->add_menu( array(
		...
		'title'  => '<span class="ab-icon"><img src="'. HERBS_TEMPLATE_URL.'/framework/admin/assets/images/tie.png" alt=""></span>' . apply_filters( 'Herbs/theme_name', 'Herbs' ),
	...
	$wp_admin_bar->add_menu( array(
		...
		'title'  => '<span class="dashicons-before dashicons-admin-generic"></span> '. esc_html__( 'Theme Options', HERBS_TEXTDOMAIN ),
		'href'   => add_query_arg( array( 'page' => 'tie-theme-options' ), admin_url( 'admin.php' ) ),
	));
	...
	if( get_option( 'tie_token_'.HERBS_THEME_ID ) ){
		$wp_admin_bar->add_menu(array(
			...
			'title'  => '<span class="dashicons-before dashicons-book"></span> '. esc_html__( 'Knowledge Base', HERBS_TEXTDOMAIN ),
			'href'   => apply_filters( 'Herbs/External/knowledge_base', '' )
		));
	}
	else{
		...
		$wp_admin_bar->add_menu(array(
			...
			'title'  => '<span class="dashicons-before dashicons-unlock"></span> '. sprintf( esc_html__( 'Activate %s', HERBS_TEXTDOMAIN ), apply_filters( 'Herbs/theme_name', 'Herbs' ) ),
		...
		$wp_admin_bar->add_menu(array(
			'parent' => 'tie-adminbar-panel',
			'id'     => 'tie-buy-theme',
			'title'  => '<span class="dashicons-before dashicons-cart"></span> '. esc_html__( 'Buy a License', HERBS_TEXTDOMAIN ),
...
function tie_get_purchase_link( $utm_data = array() ){
	...
	return add_query_arg(
		...
		'utm_medium'   => 'link',
		'utm_campaign' => HERBS_THEME_SLUG,
		'utm_content'  => false
		)),
		'https://herbaltv.co.id'
	);
}
...
/*-----------------------------------------------------------------------------------*/
require HERBS_TEMPLATE_PATH . '/framework/admin/theme-options.php';
require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-builder-widgets.php';
//require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-welcome-page.php';
//require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-menu-limit-detector.php';
//require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-required-plugins.php';
//require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-demo-importer.php';
//require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-posts-switcher.php';
//require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-system-status.php';
//require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-theme-updater.php';
require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-settings.php';
require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-settings-category.php';
require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-settings-post.php';
require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-notices.php';
//require HERBS_TEMPLATE_PATH . '/framework/admin/classes/class-herbs-verification.php';
require HERBS_TEMPLATE_PATH . '/framework/admin/page-builder.php';
...
function tie_admin_enqueue_scripts(){
	...
	wp_enqueue_script( 'tie-admin-scripts', HERBS_TEMPLATE_URL.'/framework/admin/assets/tie.js', array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-draggable', 'wp-color-picker' ), $ver, false );
	wp_enqueue_style ( 'tie-admin-style',   HERBS_TEMPLATE_URL.'/framework/admin/assets/style.css', array(), $ver, 'all' );
	wp_enqueue_style ( 'tie-fontawesome',   HERBS_TEMPLATE_URL.'/assets/fonts/fontawesome/font-awesome.min.css', array(), $ver, 'all' );
  wp_enqueue_style ( 'wp-color-picker' );

	$localize = array(
		'update' => esc_html__( 'Update', HERBS_TEXTDOMAIN ),
		'search' => esc_html__( 'Search', HERBS_TEXTDOMAIN ),
	);
...
function tie_admin_code_editor(){
	...
	$settings_post_types = apply_filters( 'Herbs/settings_post_types', array( 'post', 'page' ) );
	...
	// Theme Options Page
	elseif( HERBS_ADMIN_HELPER::is_theme_options_page() ){
...
function tie_install_theme(){
	...
	if( ! get_option( 'tie_ver_'. HERBS_THEME_ID ) && ! get_option( apply_filters( 'Herbs/theme_options', '' ) ) ){
		...
		update_option( 'tie_ver_'. HERBS_THEME_ID, HERBS_DB_VERSION );

		// Store the data of installing the theme temporarily
		update_option( 'tie_install_date_'. HERBS_THEME_ID, time(), false );
	}
	...
	update_option( 'tie_published_posts_'. HERBS_THEME_ID, $published_posts, false );
...
function tie_default_theme_settings(){
	...
	$default_settings = array(
		'tie_options' => array(
			...
			'footer_top'                        => 'true',
			'footer_social'                     => 'true',
			'footer_one'                        => sprintf( esc_html__( '%1$s Copyright %2$s, All Rights Reserved &nbsp;|&nbsp; %3$s Theme by %4$s', HERBS_TEXTDOMAIN ), '&copy;', '%year%', '<span style="color:red;" class="fa fa-heart"></span> <a href="'. apply_filters( 'Herbs/External/theme_footer', '' ) .'" target="_blank" rel="nofollow noopener">'. apply_filters( 'Herbs/theme_name', 'Herbs' ), 'Herbs</a>'  ),
...
function tie_user_profile_custom_options( $user ){ ?>
	...
	<h3><?php esc_html_e( 'Custom Author widget', HERBS_TEXTDOMAIN ) ?></h3>
	<table class="form-table">
		<tr>
			<th><label for="author_widget_content"><?php esc_html_e( 'Custom Author widget content', HERBS_TEXTDOMAIN ) ?></label></th>
			<td>
				...
				<br /><span class="description"><?php esc_html_e( 'Supports: Text, HTML and Shortcodes.', HERBS_TEXTDOMAIN ) ?></span>
			</td>
		</tr>
	</table>

	<h3><?php esc_html_e( 'Social Networks', HERBS_TEXTDOMAIN ) ?></h3>
	<table class="form-table">
...
function tie_get_latest_theme_data( $key = '', $token = false, $force_update = false, $update_files = false, $revoke = false ){
	...
	$cache_field     = 'tie-data-'.HERBS_THEME_SLUG;
	$plugins_field   = 'tie-plugins-data-'.HERBS_THEME_SLUG;
	$token_key       = 'tie_token_'.HERBS_THEME_ID;
	$token_error_key = 'tie_token_error_'.HERBS_THEME_ID;
	$request_url     = 'https://herbaltv.co.id/?envato_get_data';
	...
	// Use the given $token and force update the Herbs data from Envato
	if( $token !== false ){
	...
	if( empty( $cached_data ) && ! empty( $token ) ){
		...
		$body = array(
			...
			'theme_version'  => HERBS_DB_VERSION,
			'demo_installed' => get_option( 'tie_installed_demo_'. HERBS_THEME_ID ),
			'is_switched'    => get_option( 'tie_switch_to_'. HERBS_THEME_ID ),
			'local'          => get_locale(),
		);
		...
		$response = wp_remote_post( $request_url, array(
			...
			'body' => apply_filters( 'Herbs/api_connect_body', $body ),
			'sslverify' => false,
			//'timeout'   => 15,
		));
		...
		if ( is_wp_error( $response ) ){
			...
			if( ! empty( $cached_data['status'] ) && $cached_data['status'] == 1 ){
			  ...
			  do_action( 'Herbs/after_theme_data_update' );
			}
			else{
...
/*-----------------------------------------------------------------------------------*/
add_action( 'Herbs/Options/updated', 'tie_clear_super_cache' );
...
# Welcome Page ARGS
/*-----------------------------------------------------------------------------------*/
add_filter( 'Herbs/welcome_args', 'tie_welcome_args' );
function tie_welcome_args( $args ){

	$args['color'] = apply_filters( 'Herbs/default_theme_color', '#000' );
	$args['img']   = HERBS_TEMPLATE_URL .'/framework/admin/assets/images/herbs-logo-mini.png';
	$args['about'] = sprintf( esc_html__( 'You are awesome! Thanks for using our theme, %s is now installed and ready to use! Get ready to build something beautiful :)', HERBS_TEXTDOMAIN ), apply_filters( 'Herbs/theme_name', 'Herbs' ) );
...
/*-----------------------------------------------------------------------------------*/
add_filter( 'Herbs/welcome_splash_content', 'tie_welcome_splash_content' );
function tie_welcome_splash_content(){
	?>

	<h2><?php printf( esc_html__( 'Need help? We\'re here %s', HERBS_TEXTDOMAIN ), '&#x1F60A' ); ?></h2>

	<div class="under-the-hood">
		<div class="col column">
			<p><?php printf( wp_kses_post( 'The Theme comes with 6 months of free support for every license you purchase. Support can be <a target="_blank" href="%1s">extended through subscriptions via ThemeForest</a>. All support is handled through our <a target="_blank" href="%2s">support center</a>. Below are all the resources we offer in our support center.', HERBS_TEXTDOMAIN ), apply_filters( 'Herbs/External/renew_support_article', '' ), apply_filters( 'Herbs/External/support_center', '' ) ); ?></p>
		</div>
	</div>

	<div class="has-3-columns is-fullwidth">
		<div class="col column">
			<h3><span class="dashicons dashicons-sos"></span> <?php esc_html_e( 'Submit a Ticket', HERBS_TEXTDOMAIN ); ?></h3>
			<p><?php esc_html_e( 'Need one-to-one assistance? Get in touch with our Support team.', HERBS_TEXTDOMAIN ); ?></p>
			...
			<?php
				...
				if( get_option( 'tie_token_'.HERBS_THEME_ID ) ){
					...
					if( ! empty( $support_info['status'] ) && $support_info['status'] == 'expired' ){
						echo '<p style="font-weight:bold; color: red;">'. esc_html__( 'Your Support Period Has Expired', HERBS_TEXTDOMAIN ) .'</p>';
					}
					else{
						?>
							<a target="_blank" class="button button-primary button-hero" href="<?php echo apply_filters( 'Herbs/External/open_ticket', '' ); ?>"><?php esc_html_e( 'Submit a Ticket', HERBS_TEXTDOMAIN ); ?></a>
						<?php
					}
				}
				else{

					echo '<p style="font-weight:bold; color: red;">'. esc_html__( 'You need to validated your license to access the support system.', HERBS_TEXTDOMAIN ) .'</p>';
				}
		...
		<div class="col column">
			<h3><span class="dashicons dashicons-book"></span> <?php esc_html_e( 'Knowledge Base', HERBS_TEXTDOMAIN ); ?></h3>
			<p><?php esc_html_e( 'This is the place to go to reference different aspects of the theme.', HERBS_TEXTDOMAIN ); ?></p>
			<a target="_blank" class="button button-primary button-hero" href="<?php echo apply_filters( 'Herbs/External/knowledge_base', '' ); ?>"><?php esc_html_e( 'Browse the Knowledge Base', HERBS_TEXTDOMAIN ); ?></a>
		</div>

		<div class="col column">
			<h3><span class="dashicons dashicons-info"></span> <?php esc_html_e( 'Troubleshooting', HERBS_TEXTDOMAIN ); ?></h3>
			<p><?php esc_html_e( 'If something is not working as expected, Please try these common solutions.', HERBS_TEXTDOMAIN ); ?></p>
			<a target="_blank" class="button button-primary button-hero" href="<?php echo apply_filters( 'Herbs/External/troubleshooting', '' ); ?>"><?php esc_html_e( 'Visit The Page', HERBS_TEXTDOMAIN ); ?></a>
		</div>
	</div>

	<hr />

	<ul id="follow-herbs" class="column">
		<li class="follow-herbs-fb">
			<div id="fb-root"></div>
			...
			<div class="fb-like" data-href="https://facebook.com/herbs" data-layout="button_count" data-action="like" data-size="large" data-show-faces="false" data-share="false"></div>
		</li>

		<li class="follow-herbs-twitter">
			<a href="https://twitter.com/herbs" class="twitter-follow-button" data-size="large" data-show-count="false">Follow @herbs</a><script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>
