	 ...
	 */
	 class TIE_POSTS_LIST extends WP_Widget {
		...
		public function __construct(){
			...
			parent::__construct( 'posts-list-widget', apply_filters( 'Herbs/theme_name', 'Herbs' ) .' - '. esc_html__( 'Posts list', HERBS_TEXTDOMAIN), $widget_ops );
		}
		...
		public function widget( $args, $instance ){
			...
			if( ! empty( $instance['style'] ) && ! empty( $layouts[ $instance['style'] ] )){
				...
				elseif( $instance['style'] == 3 ){
					$style_args['thumbnail_first'] = HERBS_THEME_SLUG.'-image-large';
					$style_args['review_first']    = 'large';
				}

				elseif( $instance['style'] == 4 ){
					$style_args['thumbnail']  = HERBS_THEME_SLUG.'-image-large';
		...
		public function form( $instance ){
			$defaults = array( 'title' => esc_html__('Recent Posts', HERBS_TEXTDOMAIN) , 'no_of_posts' => '5', 'posts_order' => 'latest' );
			...
			if( tie_get_option( 'tie_post_views' ) ){
				$post_order['standard']['views'] = esc_html__( 'Most Viewed posts', HERBS_TEXTDOMAIN );
			}


			// JetPack
			$post_order['jetpack']['jetpack-7']  = esc_html__( 'Jetpack - Most Viewed for 7 days',  HERBS_TEXTDOMAIN );
			$post_order['jetpack']['jetpack-30'] = esc_html__( 'Jetpack - Most Viewed for 30 days', HERBS_TEXTDOMAIN );

			// Related Posts options
			$post_order['related']['related-cat']    = esc_html__( 'Related Posts by Categories', HERBS_TEXTDOMAIN );
			$post_order['related']['related-tag']    = esc_html__( 'Related Posts by Tags',       HERBS_TEXTDOMAIN );
			$post_order['related']['related-author'] = esc_html__( 'Related Posts by Author',     HERBS_TEXTDOMAIN );


			$post_order = apply_filters( 'Herbs/Widget/Posts/post_order_args' ,$post_order );
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'posts_order' ) ); ?>"><?php esc_html_e( 'Posts order:', HERBS_TEXTDOMAIN) ?></label>
			...
			<div class="tie-related-posts-order-option" <?php echo $related_options ?>>
				<?php
					echo '<p class="tie-message-hint">'. esc_html__( 'This Widget appears in the single post page only.', HERBS_TEXTDOMAIN) .'</p>';
				?>
			</div>


			<p class="tie-non-custom-posts-order-option" <?php echo $non_custom_options ?>>
				<label for="<?php echo esc_attr( $this->get_field_id( 'cats_id' ) ); ?>"><?php esc_html_e( 'Categories', HERBS_TEXTDOMAIN) ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'no_of_posts' ) ); ?>"><?php esc_html_e( 'Number of posts to show', HERBS_TEXTDOMAIN) ?></label>
			...
			<p class="tie-non-custom-posts-order-option" <?php echo $non_custom_options ?>>
				<label for="<?php echo esc_attr( $this->get_field_id( 'offset' ) ); ?>"><?php esc_html_e( 'Offset - number of posts to pass over', HERBS_TEXTDOMAIN ) ?></label>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'media_overlay' ) ); ?>"><?php esc_html_e( 'Media Icon', HERBS_TEXTDOMAIN) ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'exclude_current' ) ); ?>"><?php esc_html_e( 'Exclude Current Post in the single post page.', HERBS_TEXTDOMAIN) ?></label>
			</p>

			<label><?php esc_html_e( 'Style', HERBS_TEXTDOMAIN) ?></label>
					...
					<?php
						for ( $i=1; $i < 9; $i++ ){ ?>
							<label class="tie-widget-options">
								<input name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" type="radio" value="<?php echo esc_attr( $i ) ?>" <?php echo checked( $style, $i ) ?>> <img src="<?php echo HERBS_TEMPLATE_URL .'/framework/admin/assets/images/widgets/posts-'.$i.'.png'; ?>" />
							</label>
							<?php
						}
					?>
