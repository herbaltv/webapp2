<?php
/*
Weather Widget developed By : Fouad Badawy | Herbs
...
if( ! class_exists( 'TIE_WEATHER_WIDGET' )){
	...
	class TIE_WEATHER_WIDGET extends WP_Widget{
		...
		public function __construct(){
			...
			parent::__construct( 'tie-weather-widget', apply_filters( 'Herbs/theme_name', 'Herbs' ) .' - '.esc_html__( 'Weather', HERBS_TEXTDOMAIN ), $widget_ops );
		}
		...
		public function update( $new_instance, $old_instance ){
			...
			if( ! empty( $instance['location'] ) ){
				HERBS_WEATHER::clear_cache( $instance['location'] );
			}
		...
		public function form( $instance ){
			$defaults = array( 'title' => esc_html__('Weather', HERBS_TEXTDOMAIN) );
			...
			if( ! tie_get_option( 'api_openweather' ) ){
				echo '<p class="tie-message-hint">'. esc_html__( 'You need to set the Weather API Key in the theme options page > API Keys.', HERBS_TEXTDOMAIN ) .'</p>';
			}
		?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('location') ); ?>">
				<?php esc_html_e('Location', HERBS_TEXTDOMAIN); ?> - <a href="<?php echo esc_url( 'http://openweathermap.org/find' ); ?>" target="_blank" rel="nofollow noopener"><?php esc_html_e('Find Your Location', HERBS_TEXTDOMAIN); ?></a><br />
				<small><?php esc_html_e( '(i.e: London,UK or New York City)', HERBS_TEXTDOMAIN ); ?></small>
			</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('custom_name') ); ?>">
				<?php esc_html_e('Custom City Name', HERBS_TEXTDOMAIN); ?><br />
			</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('units') ); ?>"><?php esc_html_e('Units', HERBS_TEXTDOMAIN); ?></label>  &nbsp;
			<input id="<?php  echo esc_attr( $this->get_field_id('units') ); ?>-f" name="<?php echo esc_attr( $this->get_field_name('units') ); ?>" type="radio" value="F" <?php checked( $units, 'F' ); ?> /> <?php esc_html_e('F', HERBS_TEXTDOMAIN); ?> &nbsp; &nbsp;
			<input id="<?php  echo esc_attr( $this->get_field_id('units') ); ?>-c" name="<?php echo esc_attr( $this->get_field_name('units') ); ?>" type="radio" value="C" <?php checked( $units, 'C' ); ?> /> <?php esc_html_e('C', HERBS_TEXTDOMAIN); ?>
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('forecast_days') ); ?>"><?php esc_html_e('Forecast', HERBS_TEXTDOMAIN ); ?></label>
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id('forecast_days') ); ?>" name="<?php echo esc_attr( $this->get_field_name('forecast_days') ); ?>">
				<?php
					for ( $i=5; $i>0; $i-- ) {
						echo '<option value="'. $i .'"'. selected( $forecast_days, $i, false ) .'>'. sprintf( _n( '%d day', '%d days', $i, HERBS_TEXTDOMAIN ), $i ) .'</option>';
					}
				?>
				<option value="hide"<?php selected( $forecast_days, 'hide' ); ?>><?php esc_html_e('Disable', HERBS_TEXTDOMAIN); ?></option>
			</select>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'animated' ) ); ?>"><?php esc_html_e( 'Animated Icons?', HERBS_TEXTDOMAIN) ?></label>
		</p>

		<div class="weather-color tie-custom-color-picker <?php echo esc_attr( $colors_class ) ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'bg_color' ) ); ?>" style="display:block;"><?php esc_html_e( 'Background Color', HERBS_TEXTDOMAIN ); ?></label>
		...
		<div class="weather-color tie-custom-color-picker <?php echo esc_attr( $colors_class ) ?>">
			<label for="<?php echo esc_attr( $this->get_field_id( 'font_color' ) ); ?>" style="display:block;"><?php esc_html_e( 'Text Color', HERBS_TEXTDOMAIN ); ?></label>
