	 ...
	 */
	 class TIE_LATEST_TWEET_WIDGET extends WP_Widget {
		...
		public function __construct(){
			...
			parent::__construct( 'latest_tweets_widget', apply_filters( 'Herbs/theme_name', 'Herbs' ) .' - '.esc_html__( 'Twitter', HERBS_TEXTDOMAIN ), $widget_ops );
		}
		...
		public function widget( $args, $instance ){
			...
			if( ! empty( $instance['username'] ) && ! empty( $instance['consumer_key'] ) && ! empty( $instance['consumer_secret'] ) ){
				...
				$twitter_username = str_replace( '@', '', HERBS_HELPER::remove_spaces( $instance['username'] ) );
				...
				if( empty( $twitter_data )){
					...
					if( empty( $token ) ){
						...
						$data_to_send = HERBS_HELPER::api_credentials( $credentials );
					...
					$api_url = HERBS_HELPER::remove_spaces( $api_url );
				...
				if( is_array( $twitter_data ) ){
						...
						<?php
							...
							foreach( $twitter_data as $item ){
								...
								if ((abs( time() - $time) ) < 86400 ){
									$h_time = sprintf( esc_html__( '%s ago', HERBS_TEXTDOMAIN ), human_time_diff( $time ) );
								}
								else{
					...
					if( ! empty( $instance['slider'] ) ){ ?>
						<div class="slider-links">
							<ul class="tie-slider-nav"></ul><a href="https://twitter.com/<?php echo esc_attr( $twitter_username )  ?>" target="_blank" rel="nofollow noopener" class="button"><?php esc_html_e( 'Follow Us', HERBS_TEXTDOMAIN ) ?></a>
						</div>
						...
						<a href="https://twitter.com/<?php echo esc_attr( $twitter_username ) ?>" target="_blank" rel="nofollow noopener" class="button dark-btn fullwidth"><?php esc_html_e( 'Follow us on Twitter', HERBS_TEXTDOMAIN ) ?></a>
						<?php
					}
				}
			}
			else{
				HERBS_HELPER::notice_message( esc_html__( 'Error Can not Get Tweets, Incorrect account info.', HERBS_TEXTDOMAIN ) );
			}
		...
		public function form( $instance ){
			$defaults = array( 'title' => esc_html__( 'Follow Us', HERBS_TEXTDOMAIN ) , 'no_of_tweets' => '5' );
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'username' ) ); ?>"><?php esc_html_e( 'Twitter Username', HERBS_TEXTDOMAIN) ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'consumer_key' ) ); ?>"><?php esc_html_e( 'Consumer key:', HERBS_TEXTDOMAIN) ?> </label>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'consumer_secret' ) ); ?>"><?php esc_html_e( 'Consumer secret:', HERBS_TEXTDOMAIN) ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'no_of_tweets' ) ); ?>"><?php esc_html_e( 'Number of Tweets to show:', HERBS_TEXTDOMAIN) ?></label>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'slider' ) ); ?>"><?php esc_html_e( 'Slider Layout?', HERBS_TEXTDOMAIN) ?></label>
			</p>
