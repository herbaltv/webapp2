...
if( ! class_exists( 'TIE_NEWSLETTER_WIDGET' )){
	 ...
	 */
	 class TIE_NEWSLETTER_WIDGET extends WP_Widget {
		...
		public function __construct(){
			...
			parent::__construct( 'tie-newsletter', apply_filters( 'Herbs/theme_name', 'Herbs' ) .' - '.esc_html__( 'Newsletter', HERBS_TEXTDOMAIN ) , $widget_ops );
		}
		...
		public function widget( $args, $instance ){
			...
			// WPML
			$text = apply_filters( 'wpml_translate_single_string', $text, HERBS_THEME_SLUG, 'widget_content_'.$this->id );
				...
				<?php
					...
					if( ! empty( $instance['feedburner'] )){ ?>
						...
						<form action="https://feedburner.google.com/fb/a/mailverify" method="post" class="subscribe-form" target="popupwindow" onsubmit="window.open('https://feedburner.google.com/fb/a/mailverify?uri=<?php echo esc_attr( $instance['feedburner'] ); ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
							<label class="screen-reader-text" for="email"><?php esc_html_e( 'Enter your Email address', HERBS_TEXTDOMAIN ); ?></label>
							<input class="subscribe-input required email" type="text" id="email" name="email" placeholder="<?php esc_html_e( 'Enter your Email address', HERBS_TEXTDOMAIN ); ?>">
							<input type="hidden" value="<?php echo esc_attr( $instance['feedburner'] ); ?>" name="uri">
							<input type="hidden" name="loc" value="en_US">
							<input class="button subscribe-submit" type="submit" name="submit" value="<?php esc_html_e( 'Subscribe', HERBS_TEXTDOMAIN ) ; ?>">
						</form>
						<?php
					}
						...
						<div id="mc_embed_signup-<?php echo esc_attr( $args['widget_id'] ) ?>">
							<form action="<?php echo esc_attr( $instance['mailchimp'] ) ?>" method="post" id="mc-embedded-subscribe-form-<?php echo esc_attr( $args['widget_id'] ) ?>" name="mc-embedded-subscribe-form" class="subscribe-form validate" target="_blank" novalidate>
									<div class="mc-field-group">
										<label class="screen-reader-text" for="mce-EMAIL-<?php echo esc_attr( $args['widget_id'] ) ?>"><?php esc_html_e( 'Enter your Email address', HERBS_TEXTDOMAIN ); ?></label>
										<input type="email" value="" id="mce-EMAIL-<?php echo esc_attr( $args['widget_id'] ) ?>" placeholder="<?php esc_html_e( 'Enter your Email address', HERBS_TEXTDOMAIN ); ?>" name="EMAIL" class="subscribe-input required email">
									</div>
									<?php do_action( 'Herbs/Mailchimp/extra_fields' ); ?>
									<input type="submit" value="<?php esc_html_e( 'Subscribe', HERBS_TEXTDOMAIN ) ; ?>" name="subscribe" class="button subscribe-submit">
							</form>
						</div>
						<?php
					}
		...
		public function update( $new_instance, $old_instance ){
			...
			// WPML
			do_action( 'wpml_register_single_string', HERBS_THEME_SLUG, 'widget_content_'.$this->id, $new_instance['text'] );
	...
	<p>Lorem ipsum dolor sit amet, consectetur.</p>'
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php esc_html_e( 'Text above the Email input field', HERBS_TEXTDOMAIN) ?></label>
				...
				<small><?php esc_html_e( 'Supports: Text, HTML and Shortcodes.', HERBS_TEXTDOMAIN) ?></small>
			</p>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_icon' ) ); ?>"><?php esc_html_e( 'Show the icon?', HERBS_TEXTDOMAIN) ?></label>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'mailchimp' ) ); ?>"><?php esc_html_e( 'MailChimp Form Action URL', HERBS_TEXTDOMAIN) ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'feedburner' ) ); ?>"><strong><?php esc_html_e( '- OR -', HERBS_TEXTDOMAIN) ?></strong> <?php esc_html_e( 'Feedburner ID', HERBS_TEXTDOMAIN) ?></label>
