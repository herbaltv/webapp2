	 ...
	 */
	 class TIE_ABOUT_AUTHOR_WIDGET extends WP_Widget {
		...
		public function __construct(){
			...
			parent::__construct( 'author_widget', apply_filters( 'Herbs/theme_name', 'Herbs' ) .' - '.esc_html__( 'About the post&rsquo;s author' , HERBS_TEXTDOMAIN ) , $widget_ops );
		}
		...
		public function widget( $args, $instance ){
			...
			$title = ( $authors_count > 1 ) ? esc_html__( 'Authors', HERBS_TEXTDOMAIN ) : $post_authors[0]->display_name;
			$class = ( $authors_count > 1 ) ? 'multiple-authors' : 'single-author';

			/** This filter is documented in wp-includes/widgets/class-wp-widget-pages.php */
			$instance['title'] = apply_filters( 'widget_title', sprintf( esc_html__( 'About %s', HERBS_TEXTDOMAIN ), $title ), $instance, $this->id_base );
	 ...
	 */
	 class TIE_AUTHOR_POSTS_WIDGET extends WP_Widget {
		...
		public function __construct(){
			...
			parent::__construct( 'author_post_widget', apply_filters( 'Herbs/theme_name', 'Herbs' ) .' - '.esc_html__( "Posts By Post's Author" , HERBS_TEXTDOMAIN ), $widget_ops );
		}
		...
		public function widget( $args, $instance ){
			...
			/** This filter is documented in wp-includes/widgets/class-wp-widget-pages.php */
			$instance['title'] = apply_filters( 'widget_title', sprintf( esc_html__( 'By %s', HERBS_TEXTDOMAIN ), get_the_author() ) , $instance, $this->id_base );
			...
			if( $instance['see_all'] ){
				echo '<a class="button dark-btn fullwidth" href="'. get_author_posts_url( $author_id ). '">'. esc_html__( 'All', HERBS_TEXTDOMAIN ) .' ('. HERBS_WP_HELPER::count_user_posts( $author_id ) .')</a>';
			}
		...
		public function form( $instance ){
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'see_all' ) ); ?>"><?php esc_html_e( 'Display (All) link:', HERBS_TEXTDOMAIN) ?></label>
	 ...
	 */
	 class TIE_AUTHOR_CUSTOM_WIDGET extends WP_Widget {
		...
		public function __construct(){
			...
			parent::__construct( 'author-custom-widget', apply_filters( 'Herbs/theme_name', 'Herbs' ) .' - '.esc_html__( "Custom Author Content" , HERBS_TEXTDOMAIN ) , $widget_ops, $control_ops );
		}
		...
		public function form( $instance ){
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'center' ) ); ?>"><?php esc_html_e( 'Center content:', HERBS_TEXTDOMAIN) ?></label>
