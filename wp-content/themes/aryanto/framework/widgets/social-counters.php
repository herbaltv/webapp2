	 ...
	 */
	 class TIE_SOCIAL_COUNTER_WIDGET extends WP_Widget {
		...
		public function __construct(){
			...
			parent::__construct( 'social-statistics', apply_filters( 'Herbs/theme_name', 'Herbs' ) .' - '.esc_html__( 'Social Counters', HERBS_TEXTDOMAIN ), $widget_ops );
		}
			...
			<ul class="solid-social-icons <?php echo esc_attr( $class ) ?>">
				<?php
					...
					if( ! $is_installed ){
						...
						HERBS_HELPER::notice_message( esc_html__( 'This widget requries the Arqam Lite Plugin, You can install it from the Theme settings menu > Install Plugins.', HERBS_TEXTDOMAIN ) );
					}
					elseif( ! empty( $arq_counters ) && is_array( $arq_counters ) ){
						...
						HERBS_HELPER::notice_message( esc_html__( 'Go to the Arqam options page to set your social accounts.', HERBS_TEXTDOMAIN ) );
					}
				?>
			</ul>
			<?php
		...
		public function form( $instance ){
			$defaults = array( 'title' => esc_html__( 'Follow Us', HERBS_TEXTDOMAIN), 'style' => 1 );
					...
					<?php
						for ( $i=1; $i < 13; $i++ ){ ?>
							<label class="tie-widget-options">
								<input name="<?php echo esc_attr( $this->get_field_name( 'style' ) ); ?>" type="radio" value="<?php echo esc_attr( $i ) ?>" <?php echo checked( $style, $i ) ?>> <img src="<?php echo HERBS_TEMPLATE_URL .'/framework/admin/assets/images/widgets/counter-'.$i.'.png'; ?>" />
							</label>
							<?php
						}
					?>
