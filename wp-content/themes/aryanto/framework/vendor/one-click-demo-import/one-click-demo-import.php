...
class PT_One_Click_Demo_Import {
	...
	public function create_plugin_page(){
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'Choose the demo which you want to import', HERBS_TEXTDOMAIN ),
				'menu_title'  => esc_html__( 'Demo Import', HERBS_TEXTDOMAIN ),
				'capability'  => 'import',
	...
	// by Herbs ***************************************************************
		...
		if( isset( $_REQUEST['uninstall-demo'] ) ){
			...
			if( 'yes' == $_REQUEST['uninstall-demo'] && check_admin_referer( 'uninstall_demos', 'uninstall_demos_nonce' ) ){
				HERBS_DEMO_IMPORTER::_uninstall();
			}
			...
			elseif( 'done' == $_REQUEST['uninstall-demo'] ){
				...
				<div style="display: block;" class="ocdi__response js-tie-ajax-response js-ocdi-ajax-response">
					...
					<h3 class="is-done"><?php esc_html_e( 'Demo data has been successfully uninstalled', HERBS_TEXTDOMAIN ); ?></h3>
				</div>
				<?php
			}
			return;
	...
	update_option( HERBS_THEME_SLUG .'_demos_count', $demos_count, false );

	?>

	<div id="tie-import-wrap" class="ocdi wrap about-wrap tie-about-wrap">

		<?php HERBS_WELCOME_PAGE::_head_section( 'demos' ); ?>
		...
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
			...
			if( ! get_option( 'tie_token_'.HERBS_THEME_ID ) ){
				...
				do_action( 'Herbs/Demos/before_page_title' );
		...
		<div class="ocdi__intro-text">
			<p>
				<?php esc_html_e( 'Importing demo data (post, pages, images, theme settings, ...) is the easiest way to setup your theme. It will allow you to quickly edit everything instead of creating content from scratch. When you import the data, the following things might happen:', HERBS_TEXTDOMAIN ); ?>
			</p>

			<ul>
				<li><?php esc_html_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', HERBS_TEXTDOMAIN ); ?></li>
				<li><?php esc_html_e( 'Posts, pages, images, widgets and menus will get imported.', HERBS_TEXTDOMAIN ); ?></li>
				<li><?php esc_html_e( 'Please click "Import Demo Data" button only once and wait, it can take a couple of minutes.', HERBS_TEXTDOMAIN ); ?></li>
			</ul>
		</div>

		<div class="ocdi__intro-text">
			<p><?php esc_html_e( 'Before you begin, make sure all the required plugins are activated.', HERBS_TEXTDOMAIN ); ?></p>
		</div>
			...
			<?php foreach ( $this->import_files as $index => $import_file ) :	?>
				<div class="theme">
					<div class="theme-screenshot" data-value="<?php echo esc_attr( $index ); ?>">
						...
						<span class="more-details"><?php echo esc_html__( 'Import', HERBS_TEXTDOMAIN ); ?></span>
						<?php
							if( ! empty( $import_file['new_demos'] ) ){
								echo '<span class="new-demo">'. esc_html__( 'New', HERBS_TEXTDOMAIN ) .'</span>';
							}
						?>
					</div>
						...
						<?php
							if( ! empty( $import_file['import_demo'] )){ ?>
								<div class="theme-actions">
									<a class="tie-live-demo button button-secondry" target="_blank" href="<?php echo esc_url( $import_file['import_demo'] ); ?>"><?php echo esc_html__( 'Live Demo', HERBS_TEXTDOMAIN ); ?></a>
								</div>
								<?php
							}
						?>
...
<?php // by Herbs ***************************************************************?>
				...
				<div class="theme-wrap wp-clearfix">
					...
					<div class="theme-about wp-clearfix">
						...
						<div class="tie-message-hint">
							<h4>
								<?php esc_html_e( 'Important Notes:', HERBS_TEXTDOMAIN ); ?>
							</h4>

							<ol>
								<li><?php esc_html_e( 'We recommend to run Demo Import on a clean WordPress installation.', HERBS_TEXTDOMAIN ); ?></li>
								<li><?php esc_html_e( 'The Demo Import will not import the images we have used in our live demos, due to copyright / license reasons.', HERBS_TEXTDOMAIN ); ?></li>
								<li><?php esc_html_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', HERBS_TEXTDOMAIN ); ?></li>
								<li><?php esc_html_e( 'Posts, pages, images, widgets and menus will get imported.', HERBS_TEXTDOMAIN ); ?></li>
								<li><?php esc_html_e( 'Before you begin, make sure all the required plugins are activated.', HERBS_TEXTDOMAIN ); ?></li>
								<li><?php esc_html_e( 'Do not run the Demo Import multiple times one after another, it will result in double content.', HERBS_TEXTDOMAIN ); ?></li>
							</ol>
						</div>
					...
					<div class="theme-actions">
						<div class="import-button">
							<a id="tie-install-demo" class="tie-primary-button button button-primary button-hero" href="#"><?php echo esc_html__( 'Import', HERBS_TEXTDOMAIN ); ?></a>
							<a id="tie-view-demo" class="tie-primary-button button button-hero" target="_blank" href="#"><?php echo esc_html__( 'Live Demo', HERBS_TEXTDOMAIN ); ?></a>
						</div>

						<div class="imported-buttons">
							<a class="tie-primary-button button button-primary button-hero" href="<?php echo admin_url( 'admin.php?page=tie-theme-options' ) ?>"><?php echo esc_html__( 'Theme Options', HERBS_TEXTDOMAIN ); ?></a>
							<a class="tie-primary-button button button-hero" target="_blank" href="<?php echo esc_url(home_url( '/' )) ?>"><?php echo esc_html__( 'Visit Site', HERBS_TEXTDOMAIN ); ?></a>
						</div>
					</div>
					...
					<div class="ocdi__response js-tie-ajax-response js-ocdi-ajax-response">
						...
						<div class="tie-loading-wrapper">
							<h3><?php esc_html_e( 'Importing Demo Content...', HERBS_TEXTDOMAIN ); ?></h3>
							<h4><?php esc_html_e( 'Please be patient and do not navigate away from this page while the import is in progress. This can take a while if your server is slow.', HERBS_TEXTDOMAIN ); ?></h4>
							<h4><?php esc_html_e( 'You will be notified via this page when the import is completed.', HERBS_TEXTDOMAIN ); ?></h4>
						</div>
					</div>
		...
		if ( $this->plugin_page === $hook ){
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'        => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', HERBS_TEXTDOMAIN ),
					),
				)
			);
		}
	...
	public function import_demo_data_ajax_callback(){
		...
		if ( ! $use_existing_importer_data ){
			...
			if ( ! empty( $_FILES ) ){ // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', HERBS_TEXTDOMAIN );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ){ // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ){
					...
					OCDI_Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', HERBS_TEXTDOMAIN )
					);
				}
				...
				// Store the installed Demo // Herbs
				if( ! empty( $this->import_files[ $this->selected_index ]['import_file_name'] )){
					update_option( 'tie_installed_demo_'. HERBS_THEME_ID, $this->import_files[ $this->selected_index ]['import_file_name'], false );
				}
			}
			else {

				// Send JSON Error response to the AJAX call.
				wp_send_json( esc_html__( 'No import files specified!', HERBS_TEXTDOMAIN ) );
			}
		}
		 ...
		 * WooCommerce Data Import by Herbs
		 */
	...
	//Herbs *************
		...
		if ( ! empty( $this->selected_import_files['settings'] ) && empty( $this->frontend_error_messages ) ){
			...
			do_action( 'Herbs/demo_import_settings', $this->selected_import_files['settings'] );
		...
		$log_added = OCDI_Helpers::append_to_file(
			...
			$this->log_file_path,
			esc_html__( 'Completed', HERBS_TEXTDOMAIN )
		);
	//END *************
		...
		if ( empty( $this->frontend_error_messages ) ){
			...
			$response['message'] = '<h3 class="is-done">' .__( 'That\'s it, all done!', HERBS_TEXTDOMAIN ). '</h3>'.
				'<h4>'.
					sprintf(
						__( 'The demo import has finished. Now you can see the result at %1$syour site%2$s or start customize via %3$sTheme Options%2$s.', HERBS_TEXTDOMAIN ),
			...
			$response['message'] = sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sOne Click Demo Import%4$s plugin, because it has done its job.%5$s', HERBS_TEXTDOMAIN ),
			...
			*/
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', HERBS_TEXTDOMAIN ),
	...
	private function import_widgets( $widget_import_file_path ){
		...
		if ( is_wp_error( $results ) ){
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				...
				$this->log_file_path,
				esc_html__( 'Importing widgets', HERBS_TEXTDOMAIN )
			);
		}
	...
	private function import_customizer( $customizer_import_file_path ){
		...
		if ( is_wp_error( $results ) ){
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				$results->get_error_message(),
				$this->log_file_path,
				esc_html__( 'Importing customizer settings', HERBS_TEXTDOMAIN )
			);
		}
