...
if( tie_get_option( $components_id.'-components_search' ) ):
	...
	if( tie_get_option( "$components_id-components_search_layout" ) == 'compact' ):?>
		...
		<li class="search-bar menu-item custom-menu-link" aria-label="<?php esc_html_e( 'Search', HERBS_TEXTDOMAIN ); ?>">
			<form method="get" id="search" action="<?php echo esc_url(home_url( '/' )); ?>/">
				<input id="search-input" <?php echo ( $live_search_class ); ?>type="text" name="s" title="<?php esc_html_e( 'Search for', HERBS_TEXTDOMAIN ) ?>" placeholder="<?php esc_html_e( 'Search for', HERBS_TEXTDOMAIN ) ?>" />
...
if( tie_get_option( $components_id.'-components_slide_area' ) ):?>
	<li class="side-aside-nav-icon menu-item custom-menu-link">
		<a href="#">
			...
			<span class="screen-reader-text"><?php esc_html_e( 'Sidebar', HERBS_TEXTDOMAIN ) ?></span>
		</a>
	</li>
	<?php
endif;
...
if( tie_get_option( $components_id.'-components_random' ) ):?>
	<li class="random-post-icon menu-item custom-menu-link">
		<a href="<?php echo esc_url( add_query_arg( 'random-post', 1 ) ); ?>" class="random-post" title="<?php esc_html_e( 'Random Article', HERBS_TEXTDOMAIN ) ?>" rel="nofollow">
			<span class="fa fa-random" aria-hidden="true"></span>
			<span class="screen-reader-text"><?php esc_html_e( 'Random Article', HERBS_TEXTDOMAIN ) ?></span>
		</a>
	</li>
	<?php
endif;
...
// Cart
if( tie_get_option( $components_id.'-components_cart' ) && HERBS_WOOCOMMERCE_IS_ACTIVE ):?>
	<li class="shopping-cart-icon menu-item custom-menu-link">
		<a href="<?php echo wc_get_cart_url() ?>" title="<?php esc_html_e( 'View your shopping cart', HERBS_TEXTDOMAIN ); ?>">
			...
			<span class="screen-reader-text"><?php esc_html_e( 'View your shopping cart', HERBS_TEXTDOMAIN ) ?></span>
		</a>

		<?php if( $components_id != 'mobile' ) { ?>
			<div class="components-sub-menu comp-sub-menu">
				<div class="shopping-cart-details">
					<?php do_action( 'Herbs/wc_cart_menu_content' ) ?>
...
if( tie_get_option( $components_id.'-components_bp_notifications' ) && is_user_logged_in() && HERBS_BUDDYPRESS_IS_ACTIVE ):

	$notification = apply_filters( 'Herbs/BuddyPress/notifications', '' ); ?>

	<li class="notifications-icon menu-item custom-menu-link">
		<a href="<?php echo esc_url( $notification['link'] ) ?>" title="<?php esc_html_e( 'Notifications', HERBS_TEXTDOMAIN ); ?>">
			...
			<span class="screen-reader-text"><?php esc_html_e( 'Notifications', HERBS_TEXTDOMAIN ) ?></span>
		</a>
	...
	<?php if( is_user_logged_in() ){ ?>
		...
		<li class="profile-icon menu-item custom-menu-link">
			<a href="#" class="profile-btn">
				<?php
					...
					echo get_avatar( $current_user->ID, apply_filters( 'Herbs/Login/avatar_size', 30 ) );
				?>
				<span class="screen-reader-text"><?php esc_html_e( 'Your Profile', HERBS_TEXTDOMAIN ) ?></span>
			</a>
		...
		<li class="popup-login-icon menu-item custom-menu-link">
			<a href="#" class="lgoin-btn tie-popup-trigger">
				...
				<span class="screen-reader-text"><?php esc_html_e( 'Log In', HERBS_TEXTDOMAIN ) ?></span>
			</a>
		</li>
	...
	if( tie_get_option( "$components_id-components_social_layout" ) == 'list' ):?>
		<li class="list-social-icons menu-item custom-menu-link">
			<a href="#" class="follow-btn">
				...
				<span class="follow-text"><?php esc_html_e( 'Follow', HERBS_TEXTDOMAIN ) ?></span>
			</a>
			<?php
				tie_get_social(
					array(
