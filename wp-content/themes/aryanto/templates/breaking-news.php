...
if( $type == 'header' && HERBS_HELPER::is_mobile_and_hidden( 'breaking_news' ) ){
	return;
}

// Cache field key
$cache_key = apply_filters( 'Herbs/cache_key', '' );
...
$breaking_class = join( ' ', array_filter( apply_filters( 'Herbs/Breaking_news/class', $breaking_class ) ) );
$breaking_attr  = join( ' ', array_filter( apply_filters( 'Herbs/Breaking_news/attr',  $breaking_attr  ) ) );
...
<div class="<?php echo esc_attr( $breaking_class ) ?>">
	...
	<span class="breaking-title">
		...
		<span class="breaking-title-text"><?php echo ! empty( $breaking_title ) ? $breaking_title : esc_html__( 'Trending', HERBS_TEXTDOMAIN ); ?></span>
	</span>
				...
				if( ! empty( $breaking_custom ) && is_array( $breaking_custom ) ){
					...
					foreach ($breaking_custom as $custom_text){
						...
						if( ! empty( $custom_text['text'] ) ){
							$text = ( $type == 'block' ) ? $custom_text['text'] : apply_filters( 'wpml_translate_single_string', $custom_text['text'], HERBS_THEME_SLUG, 'Breaking News Custom Text #'.$count );
						}

						if( ! empty( $custom_text['link'] ) ){
							$link = ( $type == 'block' ) ? $custom_text['link'] : apply_filters( 'wpml_translate_single_string', $custom_text['link'], HERBS_THEME_SLUG, 'Breaking News Custom Link #'.$count );
						}
						?>
