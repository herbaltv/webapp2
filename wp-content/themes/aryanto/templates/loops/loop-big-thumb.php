...
if( $count == 1 ) :
	...
	$background = '';
	$thumbnail  = tie_thumb_src_bg( HERBS_THEME_SLUG.'-image-post' );

	if( ! empty( $thumbnail ) ){
		$background = tie_get_option( 'lazy_load' ) ? 'data-lazy-bg="'. esc_attr( tie_thumb_src( HERBS_THEME_SLUG.'-image-post' ) ) .'"' : 'style="'. esc_attr( tie_thumb_src_bg( HERBS_THEME_SLUG.'-image-grid' ) ) .'"';
	}
	...
	<li <?php tie_post_class( 'post-item '. $no_thumb ); ?>>
		...
		<?php
			...
			if ( has_post_thumbnail() && empty( $block['thumb_small'] ) ){
				tie_post_thumbnail( HERBS_SMALL_IMAGE, 'small', false, true, $block['media_overlay'] );
			}
		?>
