...
elseif( is_category() ){
	...
	# Cache field key
	$cache_key = apply_filters( 'Herbs/cache_key', '-slider-cat-'.$category_id );
...
if( ! empty( $slider_settings ) ){
	...
	# Get the LayerSlider
	if( $lsslider && HERBS_LS_Sliders_IS_ACTIVE ){
	...
	elseif( $revslider && HERBS_REVSLIDER_IS_ACTIVE ){
		echo '<div class="third-party-slider">';
	...
	elseif( $featured_posts ){
		...
		if( tie_get_option( 'jso_cache' ) && ! empty( $cache_key ) && false !== get_transient( $cache_key ) && ! ( defined( 'WP_CACHE' ) && WP_CACHE ) ){
			...
			# Default Images Size
			$image_size    = apply_filters( 'Herbs/Sliders/img_grid_size', HERBS_THEME_SLUG.'-image-post' );
			$full_img_size = apply_filters( 'Herbs/Sliders/img_full_size', HERBS_THEME_SLUG.'-image-full' );
			...
			if( $slider != 'videos-list' && $slider != 'videos_list' ){
				...
				if( ! empty( $query_type ) && $query_type == 'custom' ){
					...
					while ( $slider_query->have_posts() ){
						...
						$slide_meta  = apply_filters( 'Herbs/Slider/post_meta/before_date', $slide_meta );
						$slide_meta .= $show_date ? tie_get_time( true ) : '';
						$slide_meta  = apply_filters( 'Herbs/Slider/post_meta/after_date', $slide_meta );
						...
						if( $is_do_not_dublicate ){
							HERBS_HELPER::do_not_dublicate( $slider_post_id );
						}
			...
			if( tie_get_option( 'lazy_load' ) ){
				...
				if( ! empty( $is_first_slider ) ){
					...
					$before_slider .= '<img data-lazy="%1$s" src="'. HERBS_TEMPLATE_URL .'/assets/images/tie-empty-slider.png" width="780" height="500" alt="">';
			...
			<section id="tie-<?php echo esc_attr( $slider_id ) ?>" class="slider-area mag-box<?php echo esc_attr( $slider_wrapper_class ) ?>">
				...
				if( $slider == 'videos-list' || $slider == 'videos_list' ){
					...
					if( $videos_data ){
						...
						HERBS_HELPER::get_template_part( 'templates/videos-list', '', $args );
					}
