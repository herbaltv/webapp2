...
if( $post_format ){ // $post_format == 'standard' and no featured image
	...
	if( $post_layout == 6 || $post_layout == 7 ){
		...
		$image_size = ( tie_get_object_option( 'sidebar_pos', 'cat_posts_sidebar_pos', 'tie_sidebar_pos' ) == 'full' ) ? 'full' : HERBS_THEME_SLUG.'-image-post';
	}

	$image_size = apply_filters( 'Herbs/post_featured_area/image_size', $image_size, $post_format, $post_layout );
	...
	elseif( $post_format == 'slider' ){
		...
		if( $post_layout == 6 || $post_layout == 7 ){
			...
			HERBS_HELPER::inline_script( 'tie-js-sliders', $post_slider );
		}
		else{
