...
if( HERBS_BBPRESS_IS_ACTIVE && is_bbpress() ){
	return;
}

// Check if the share buttons is hidden on mobiles
if( HERBS_HELPER::is_mobile_and_hidden( 'share_post_'.$share_position ) ){
	return;
}
...
if( tie_get_postdata( 'tie_hide_share_'.$share_position ) == 'no' ||
	...
	( HERBS_HELPER::is_supported_post_type() && tie_get_option( 'share_post_'.$share_position ) && ! tie_get_postdata( 'tie_hide_share_'.$share_position ) )){
	...
	$share_buttons = array(
		...
		'facebook' => array(
			...
			'text' => esc_html__( 'Facebook', HERBS_TEXTDOMAIN ),
		),

		'twitter' => array(
			'url'   => 'https://twitter.com/intent/tweet?text='. $post_title .'&amp;url='. $post_link . $share_twitter_username,
			'text'  => esc_html__( 'Twitter', HERBS_TEXTDOMAIN ),
		),

		'linkedin' => array(
			'url'  => 'https://www.linkedin.com/shareArticle?mini=true&amp;url='. get_permalink() .'&amp;title='. $post_title, // Shortlink causes issue
			'text' => esc_html__( 'LinkedIn', HERBS_TEXTDOMAIN ),
		),

		'tumblr' => array(
			'url'  => 'https://www.tumblr.com/share/link?url='. $post_link .'&amp;name='. $post_title,
			'text' => esc_html__( 'Tumblr', HERBS_TEXTDOMAIN ),
		),

		'pinterest' => array(
			'url'  => 'https://pinterest.com/pin/create/button/?url='. $post_link .'&amp;description='. $post_title .'&amp;media='. tie_thumb_src( HERBS_THEME_SLUG.'-image-post' ),
			'text' => esc_html__( 'Pinterest', HERBS_TEXTDOMAIN ),
		),

		'reddit' => array(
			'url'  => 'https://reddit.com/submit?url='. $post_link .'&amp;title='. $post_title,
			'text' => esc_html__( 'Reddit', HERBS_TEXTDOMAIN ),
		),

		'vk' => array(
			'url'  => 'https://vk.com/share.php?url='. $post_link,
			'text' => esc_html__( 'VKontakte', HERBS_TEXTDOMAIN ),
		),

		'odnoklassniki' => array(
			'url'  => 'https://connect.ok.ru/dk?st.cmd=WidgetSharePreview&st.shareUrl='. $post_link .'&amp;description='. $post_title .'&amp;media='. tie_thumb_src( HERBS_THEME_SLUG.'-image-post' ),
			'text' => esc_html__( 'Odnoklassniki', HERBS_TEXTDOMAIN ),
		),

		'pocket' => array(
			'url'  => 'https://getpocket.com/save?title='. $post_title .'&amp;url='.$post_link,
			'text' => esc_html__( 'Pocket', HERBS_TEXTDOMAIN ),
			'icon' => 'fa fa-get-pocket',
		),

		'skype' => array(
			'url'   => 'https://web.skype.com/share?url='. $post_link .'&text='. $post_title,
			'text'  => esc_html__( 'Skype', HERBS_TEXTDOMAIN ),
			'icon'  => 'fa fa-skype',
		),

		'whatsapp' => array(
			'url'   => 'https://api.whatsapp.com/send?text='. $post_title .'%20'.$post_link,
			'text'  => esc_html__( 'WhatsApp', HERBS_TEXTDOMAIN ),
			'avoid_esc' => true,
		),

		'telegram' => array(
			'url'   => 'https://telegram.me/share/url?url='. $post_link .'&text='. $post_title,
			'text'  => esc_html__( 'Telegram', HERBS_TEXTDOMAIN ),
		...
		'viber' => array(
			...
			'text'  => esc_html__( 'Viber', HERBS_TEXTDOMAIN ),
		...
		'email' => array(
			...
			'text' => esc_html__( 'Share via Email', HERBS_TEXTDOMAIN ),
			'icon' => 'fa fa-envelope',
		),

		'print' => array(
			'url'  => '#',
			'text' => esc_html__( 'Print', HERBS_TEXTDOMAIN ),
		),
	);


	$share_buttons = apply_filters( 'Herbs/share_buttons', $share_buttons );
			...
			<div class="share-links <?php echo esc_attr( $share_class ) ?>">
				<?php
					if( tie_get_option( 'share_title_'.$share_position ) ){ ?>
						...
						<span> <?php esc_html_e( 'Share', HERBS_TEXTDOMAIN ); ?></span>
						</div>
						<?php
					}
