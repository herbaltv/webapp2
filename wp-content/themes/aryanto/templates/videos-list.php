...
if( get_option( 'tie_youtube_api_error' ) ){
	...
	HERBS_HELPER::notice_message( esc_html__( 'YouTube API ERROR, Go to the Youtube API Console on Google Cloud and remove any restrictions on the API key, Then edit the current page and click on the Update/Save button to re-connect to the YouTube servers to load the videos.', HERBS_TEXTDOMAIN ) .'<br /><br /><em>'. get_option( 'tie_youtube_api_error' ) .'</em>' );
}
?>
		...
		<?php if( ! empty( $title ) ): ?>
			<div class="playlist-title">
				...
				<span class="videos-number">
					<span class="video-playing-number">1</span> / <span class="video-totlal-number"><?php echo ( $videos_count ) ?></span> <?php esc_html_e( 'Videos', HERBS_TEXTDOMAIN ); ?>
				</span>
			</div> <!-- .playlist-title /-->
		<?php endif; ?>
